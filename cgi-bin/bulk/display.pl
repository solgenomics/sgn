...
sub new {
    ...
    $args->{pagesize}=50;
    $args->{content}="";
    $args->{tempdir}=$vhost_conf->get_conf('basepath').$vhost_conf->get_conf('tempfiles_subdir')."/bulk";
    # get cgi arguments
...
sub render_html_page {
    my $self = shift;
    my $style = shift;
    if ($style =~/HTML/i || !defined($style)) {  $self -> render_html_table_page(); }
...
sub render_html_table_page {
    ...
    # open the file
    #
    if (!exists($self->{page_number}) || ($self->{page_number}==0)) { $self->{page_number}=1; }
    if (!exists($self->{page_size}))   { $self->{page_size} = 50; }

    $self -> {debug} = 0;
    my $line_count = $self -> getFileLines($self->{tempdir}."/".$self->{dumpfile});
    $self -> {line_count} = $line_count;
    $self -> debug("Line Count in the file: $line_count");
    if ($line_count<2) { $self->{content}.="No data was retrieved. Please verify your input parameters. Thanks.<br /><br />\n"; }
    else {
	open (F, "<".$self->{tempdir}."/".$self->{dumpfile}) || $self -> {page} -> error_page("Can't open $self->{dumpfile}");
	#
	# read the column definitions
	#
	my @output_fields;
	my $defs=<F>; if ($defs) { chomp($defs); @output_fields=split/\t/, $defs; }
	$self -> debug("column definitions: ".(join " ", @output_fields));
	# define the links
	my %links = (clone_name => "/search/est.pl?request_type=10&amp;search=Search&amp;request_id=",
	...
	# print table header
	$self->{content} .= "<tr><td>line</td><td>".(join "</td><td>", @output_fields)."</td></tr>";
	
	while (<F>) {
	    chomp;
	    $line++;
	    if ($line >= ((($self->{page_number}-1)*$self->{page_size})+1) &&  ($line <= (($self->{page_number})*$self->{page_size}))) {
		my @fields = split/\t/;
		my %row;
		...
		$row{est_seq} = html_break_string ($row{est_seq}, $breakspace_num);
		$row{est_seq} = "<span class=\"sequence\" style=\"font-size: smaller;\"> $row{est_seq}</span>";
		
		$row{unigene_seq} = html_break_string ($row{unigene_seq}, $breakspace_num);
		$row{unigene_seq} = "<span class=\"sequence\" style=\"font-size: smaller;\"> $row{unigene_seq}</span>";

		$row{protein_seq} = html_break_string ($row{protein_seq}, $breakspace_num);
		$row{protein_seq} = "<span class=\"sequence\" style=\"font-size: smaller;\"> $row{protein_seq}</span>";
		$row{estscan_seq} = html_break_string ($row{estscan_seq}, $breakspace_num);
		$row{estscan_seq} = "<span class=\"sequence\" style=\"font-size: smaller;\"> $row{estscan_seq}</span>";
		$row{longest6frame_seq} = html_break_string ($row{longest6frame_seq}, $breakspace_num);
		$row{longest6frame_seq} = "<span class=\"sequence\" style=\"font-size: smaller;\"> $row{longest6frame_seq}</span>";
		$row{preferred_protein_seq} = html_break_string ($row{preferred_protein_seq}, $breakspace_num);
		$row{preferred_protein_seq} = "<span class=\"sequence\" style=\"font-size: smaller;\"> $row{preferred_protein_seq}</span>";




		$row{bac_end_sequence} = html_break_string ($row{bac_end_sequence}, $breakspace_num);
		$row{bac_end_sequence} = "<span class=\"sequence\" style=\"font-size: smaller;\"> $row{bac_end_sequence}</span>";
		...
		$row{qual_value_seq} = "<span class=\"sequence\" style=\"font-size: smaller;\"> $row{qual_value_seq}</span>";
		...
		foreach my $f (@output_fields) {
		    ...
		    if (!exists($row{$f}) || $row{$f} eq undef) { $row{$f}="N.A."; }
		    #
		    # add links as required. Links for each output field are stored in the %links hash.
		    #
		    if (exists($links{$f}) && $row{$f} ne "N.A.") { $row{$f} = "<a href=\"$links{$f}$row{$f}\">$row{$f}</a>"; }
		    if ($f eq "clone_id") {
...
sub render_fasta_page {
	...
	if ((join " ", @output_fields) =~ /est_seq/i) { $self -> {seq_type} = "est_seq"; }
	elsif ((join " ", @output_fields) =~ /unigene_seq/i)  { $self -> {seq_type} = "unigene_seq"; }
	elsif ((join " ", @output_fields) =~ /^protein_seq$/i)  { $self -> {seq_type} = "protein_seq"; } #added for protein
	elsif ((join " ", @output_fields) =~ /estscan_seq/i) { $self->{seq_type} = "estscan_seq"; }
	elsif ((join " ", @output_fields) =~ /longest6frame_seq/i) { $self->{seq_type}="longest6frame_seq"; }
	elsif ((join " ", @output_fields) =~ /preferred_protein_seq/i) { $self->{seq_type} = "preferred_protein_seq"; }
	...
	# est vs. unigene seq
	$data{est_seq} = html_break_string ($data{est_seq}, $breakspace_num, "\n") if defined($data{est_seq});
	$data{unigene_seq} = html_break_string ($data{unigene_seq}, $breakspace_num, "\n") if defined($data{unigene_seq});
	$data{protein_seq} = html_break_string ($data{protein_seq}, $breakspace_num, "\n") if defined($data{protein_seq});
	$data{estscan_seq} = html_break_string ($data{estscan_seq}, $breakspace_num, "\n") if defined($data{estscan_seq});
	$data{longest6frame_seq} = html_break_string($data{longest6frame_seq}, $breakspace_num, "\n") if defined($data{longest6frame_seq});
	$data{preferred_protein_seq} = html_break_string($data{preferred_protein_seq}, $breakspace_num, "\n") if defined($data{preferred_protein_seq});

	if ($self->{seq_type} eq "est_seq") {
	    $seq = "\n".$data{est_seq}."\n";
	    warn "NOTE: est sequence given, seq_type: ".$self->{seq_type}."\n";
	    $data{est_seq}="";
	}
	elsif ($self -> {seq_type} eq "unigene_seq") {
	    $seq = "\n".$data{unigene_seq}."\n";
	    warn "NOTE: unigene sequence given, seq_type: ".$self->{seq_type}."\n";
	    $data{unigene_seq}="";
	}
	#added for protein
	elsif ($self -> {seq_type} eq "protein_seq") {
	    $seq = "\n".$data{protein_seq}."\n";
	    warn "NOTE: protein sequence given, seq_type: ".$self->{seq_type}."\n";
	    $data{protein_seq}="";
	}
	...
	foreach my $o (@output_fields) {
	    if (exists($data{$o}) && $data{$o} ne "") { $output .= "$o:$data{$o}\t"; }
	}
	
	s/ +/ /g foreach ($output.$seq);
	$output =~ s/.*?\://;  # remove the first label (>SGN_U:SGN_U738473 becomes simply >SGN_U738473)
...
sub nextButton {
    ...
    if (($self -> {line_count} + $self->{page_size}) > (($self -> {page_number}+1)*$self->{page_size})) {
	$self->{content} .= "<a href=\"display.pl?dumpfile=".($self->{dumpfile})."&amp;page_number=".($self->{page_number}+1)."\">Next Page</a>";
    }
    else {
...
sub previousButton {
	...
	$self -> {content} .="<a href=\"display.pl?dumpfile=".($self->{dumpfile})."&amp;page_number=".($self->{page_number}-1)."\">Previous Page</a>";
    }
    else {
...
sub summaryButton {
    my $self = shift;
    $self -> {content} .= "<a href=\"display.pl?summary=1&amp;dumpfile=".($self->{dumpfile})."&amp;idType=".($self->{idType})."\">Summary Page</a>";
}
...
sub no_data_error_page {
    print "The results of that search or not available anymore. Please repeat your search.";
}
