...
my ( $organism_id, $force ) =
  $page->get_encoded_arguments( "organism_id", "force" );
...
if ( !$organism ) {
    $page->message_page("No organism exists for identifier $organism_id!");
}
if ( !$organism_id || ( !$organism_id =~ /^\d+$/ ) ) {
    $page->message_page("'$organism_id' is not a valid identifier!");
}
...
my $genomic = info_table_html(
    'Ploidy'            => $organism->get_ploidy()            || $na,
...
my $map_list = info_table_html(
    'Available maps' => $maps || $na,
    '__border' => 0,
);

my $loci_count = $organism->get_loci_count();
my $loci_list =
qq|<a href= "/search/locus_search.pl?&w8e4_common_name=$common_name">$loci_count loci</a>|;
$loci_list = $na if !$loci_count;
my $loci = info_table_html(
    "Available loci (for $common_name species)" => $loci_list,
    "__border"                                  => 0
);

print info_section_html(
    title       => 'Genomic details',
...
my $phenotypes = info_table_html(
    "Available phenotypes (for $common_name species)" => $pheno_list,
    "__border"                                        => 0
);

print info_section_html(
    title       => 'Phenomic details',
    contents    => $phenotypes,
    collapsible => 1,
    collapsed   => 0
);
