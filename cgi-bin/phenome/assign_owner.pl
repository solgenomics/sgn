...
if ($login_user_type eq 'curator') {
	...
	eval{
		...
		$query->execute($sp_person_id, $object_id);
                ...
                my $stock = $c->dbic_schema('Bio::Chado::Schema', 'sgn_chado')->resultset("Stock::Stock")->find( { stock_id => $object_id });
                $stock->create_stockprops( { 'sp_person_id' => $sp_person_id} , { 'cv_name' => 'local' , autocreate=>1, 'allow_duplicate_values'=>1 } );
	...
	if ($@) {
	    ...
	    my $subject="[New $object_type owner assigned] $object_type $object_id";
	    my $person= CXGN::People::Person->new($dbh, $login_person_id);
	    my $user=$person->get_first_name()." ".$person->get_last_name();
	    my $user_link = qq |/solpeople/personal-info.pl?sp_person_id=$login_person_id|;

            my %links = ( 'locus'=> qq |/phenome/locus_display.pl?locus_id=$object_id|,
                          'individual' =>  qq |/phenome/individual.pl?individual_id=$object_id|,
	    ...
	    my $owner_link = qq |/solpeople/personal-info.pl?sp_person_id=$sp_person_id|;
	    my $fdbk_body="curator $user ($user_link) has assigned a new owner ($owner_name, $owner_link) for $object_type $object_link \n ";
