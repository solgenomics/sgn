...
qq |<a href="/phenome/population.pl?population_id=$population_id">$pop_name</a> |;
    ...
    my $submitter =
      CXGN::People::Person->new($self->get_dbh(), $population->get_sp_person_id() );
    my $submitter_name =
...
HTML
    ...
    if ($population_name) {
        ...
        my $page_comment_obj =
          CXGN::People::PageComment->new($self->get_dbh(), "population", $population_id );
...
sub qtl_plot {
    ...
    my ($qtl_image, $image, $image_t, $image_url, $image_html, $image_t_url, $thickbox, $title);
	...
	open MARKERS, "<$flanking_markers" or die "can't open $flanking_markers: !$\n";
	...
	for ( my $i = 0 ; $i < @left ; $i++ ) {
		...
		my $l_m   = $left[$i];
		my $r_m   = $right[$i];
		my $l_pos = $population->get_marker_position( $mapversion, $l_m );
		my $r_pos = $population->get_marker_position( $mapversion, $r_m );
	...
	my $old_chr_chr = 1;
	my (
	    $c,       $chr_chr,   $image,        $image_t,   $thickbox,
	    $max_chr, $chr_chr_e, $marker_chr_e, $pos_chr_e, $lod_chr_e
	    );
...
###########thickbox	
		...
		$cache_qtl_plot_t->set_image_data( $graph_t->plot( \@data )->png );
...
sub infile_list {
    ...
    my $input_file_list_temp = File::Temp->new(
...
sub outfile_list {
    ...
    my $output_file_list_temp = File::Temp->new(TEMPLATE => "outfile_list_${ac}_$pop_id-XXXXXX",
 						    DIR      => $prod_temp_path,
 						    UNLINK   => 0,
 						    );
    ...
    my $qtl_temp = File::Temp->new(TEMPLATE => "qtl_summary_${ac}_$pop_id-XXXXXX", 
				     DIR=>$prod_temp_path, 
				     UNLINK=>0);
    my $qtl_summary = $qtl_temp->filename;
    
    my $marker_temp = File::Temp->new(TEMPLATE => "flanking_markers_${ac}_$pop_id-XXXXXX", 
				     DIR=>$prod_temp_path, 
				     UNLINK=>0);
...
sub cache_temp_path {
    ...
    my $tempimages_path = File::Spec->catfile($basepath, $tempfile_dir, "temp_images");
...
sub run_r {
	...
	map {
	    my (undef,$filename) = tempfile( File::Spec->catfile( CXGN::Tools::Run->temp_base(),
...
sub permu_values_exist {
    ...
    my ($size, $permu_file, $permu_data, $tempimages_path, $prod_cache_path, $prod_temp_path);
    
    if ($prod_permu_file) {
	
	$permu_file = fileparse($prod_permu_file);    
	($prod_cache_path, $prod_temp_path, $tempimages_path) = $self->cache_temp_path();
    }		
