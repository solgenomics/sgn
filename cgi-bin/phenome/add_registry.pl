...
if ( $login_user_type eq 'curator' || $login_user_type eq 'submitter' ) {
    ...
    my ( $registry_symbol, $registry_name, $registry_description, $sp_person_id,
      ...
      = $doc->get_encoded_arguments(
        "registry_symbol",      "registry_name",
        "registry_description", "sp_person_id",
        "locus_id"
      );
...
#I've changed the query to search only for existing registered symbols. There may be multiple symbols in the database, but users shouldn't be allowed to store more duplicates.
    my $registry_exists = $dbh->prepare(
"SELECT symbol, name FROM phenome.registry WHERE symbol ilike '$registry_symbol' "
    );
    $registry_exists->execute();

    if ( !$registry_exists->fetchrow_array() ) {
        my $registry_query = $dbh->prepare(
"INSERT INTO phenome.registry (symbol, name, description, sp_person_id, status) VALUES (?, ?, ?, ?, 'registered')"
        );
        ...
        my $locus_registry_insert = $dbh->prepare(
"INSERT INTO phenome.locus_registry (locus_id, registry_id, sp_person_id) VALUES (?, ?, ?)"
        );
        $locus_registry_insert->execute( $locus_id, $registry_id,
            $sp_person_id );
        $response = "success";
    }
