...
my ($login_person_id, $login_user_type)=CXGN::Login->new($dbh)->has_session();
...
if ($type eq 'network') {
	...
	GROUP: foreach my $group(@locus_groups) {
	  ...
	  MEMBER: foreach my $member(@members) { 
	      ...
	      my ($organism, $associated_locus_name, $gene_activity,$lgm_obsolete_link);
		  ...
		  $al_count++;
		  my $associated_locus= CXGN::Phenome::Locus->new($dbh, $member_locus_id);
		  $associated_locus_name= $associated_locus->get_locus_name();
		  $gene_activity= $associated_locus->get_gene_activity();
		  $organism = $associated_locus->get_common_name;
		  
		  ##obsolete link
		  $lgm_obsolete_link = qq | <span class="ghosted">[Remove]</span> |; 
		  if ($login_user_type) {
		      if (($login_user_type eq 'submitter') || ($login_user_type eq 'curator') || ($login_user_type eq 'sequencer')) {
			  $lgm_obsolete_link= CXGN::Phenome::Locus::LocusPage::obsolete_locusgroup_member($lgm_id);
		      }
		  }
		  ###########
		  		  
		  $members_info .= qq|$organism <a href="/phenome/locus_display.pl?locus_id=$member_locus_id">$associated_locus_name</a> $gene_activity $lgm_obsolete_link <br /> | if ($associated_locus->get_obsolete() eq 'f') ;
	...
	foreach my $r (keys %rel) {
	    $associated_loci .= info_table_html($r=>$rel{$r}, __border=>0,__tableattrs=>'width="100%"') ;
	}
    ...
    if ($@) { 
	$error{"error"} =  $@;
	CXGN::Contact::send_email('print_locus_page.pl died',$error{"error"}, 'sgn-bugs@sgn.cornell.edu');
...
if ($type eq 'ontology') {
	...
	foreach my $a (@alleles) { 
	    ...
	    foreach my $a_db_name (keys %a_dbs) { #add allele_dbxrefs to the locus_dbxrefs list
		my %seen=(); #hash for assisting filtering of duplicated dbxrefs (from allele annotation) 
		foreach (@{$dbs{$a_db_name} }) { $seen{$_->[0]->get_accession()}++; }#populate with the locus_dbxrefs
		foreach  ( @{$a_dbs{$a_db_name} } ) {  #and filter duplicates
		    push @ {$dbs{$a_db_name} }, $_ unless $seen{$_->[0]->get_accession()}++ ;
		}
	    }
	}
	...
	foreach (@ont_annot ) {  
		...
		if ($href->{obsolete} eq 't') { 
		    my $unobsolete;
		    $unobsolete= CXGN::Phenome::Locus::LocusPage::unobsolete_evidence($href->{dbxref_ev_object}->get_object_dbxref_evidence_id) if (($login_user_type eq 'curator' || $login_user_type eq 'submitter' || $login_user_type eq 'sequencer') );
		    push @obs_annot, $href->{relationship}." ".$cvterm_link." (".$href->{ev_code}.")". $unobsolete ;
		}else {
		    $c++;
		    ...
		    my $obsolete_link= CXGN::Phenome::Locus::LocusPage::obsolete_evidence($href->{dbxref_ev_object}->get_object_dbxref_evidence_id) if (($login_user_type eq 'curator' || $login_user_type eq 'submitter' || $login_user_type eq 'sequencer') );
		    my $ev_string;
		    $ev_string .= "<br /><hr>" if $ont_hash{$cv_name}{$ontology_details};
		    $ev_string .= $href->{ev_code}."<br />". $href->{ev_desc}."<br /><a href=\"". $href->{ev_with_url}. "\">" . $href->{ev_with_acc} . "</a><br /><a href=\"" . $href->{reference_url} ."\">" . $href->{reference_acc} . "</a><br />".$href->{submitter} . $obsolete_link ;	
...
if ($type eq 'unigenes') {
	...
	if ( !@unigenes ) { $unigenes = qq|<span class=\"ghosted\">none</span>|; }
	my $unigene_count=0;
	my @solcyc;
	...
	foreach my $unigene (@unigenes) {
	    ...
	    my $unigene_obsolete_link = qq | <span class="ghosted">[Remove]</span> |;
	    my $locus_unigene_id = $locus->get_locus_unigene_id($unigene_id);
	    if (   ( $login_user_type eq 'submitter' )
		   || ( $login_user_type eq 'curator' )
		   || ( $login_user_type eq 'sequencer' ) )
	    {  $unigene_obsolete_link = CXGN::Phenome::Locus::LocusPage::obsolete_locus_unigene($locus_unigene_id);
	    }
...
sub print_obsoleted {
    ...
    my $print_obsoleted= 
	html_alternate_show('obsoleted_terms',
			   'Show obsolete',
			    '',
