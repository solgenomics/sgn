...
my (
     $action,                           $tree_string,           $file,
     $shared_file,                      $hilite,                $ops,
     $term,                             $title,                 $tree_style,
     $show_blen,                        $preset,                $height,
     $reroot,                           $show_orthologs,        $show_species,
     $collapse_single_species_subtrees, $show_standard_species, $use_html_path,
     $show_skip_form,                   $align_format,          $align_seq_data,
     $align_temp_file,                  $align_type,            $hide_alignment,
     $tree_from_align,                  $show_sigpep,           $show_domains,
     $stored_genes,                     $family_nr,             $i_value
   )
     ...
     && $i_value )
{
    ...
    unless ( -d $family_basedir ) {
        input_err_page( $page, "No current family build corresponds to an i_value of $i_value" );
    }
    ...
    unless ( -f "$family_basedir/newick/$family_nr.newick" ) {
        ...
        my $tmp_fh = File::Temp->new( DIR    => $PATH,
                                      UNLINK => 0 );
    ...
    $tmp_fh->close();
    my @out = `cp $family_basedir/pep_align/$family_nr.pep.aligned.fasta $align_temp_file 2>/dev/stdout`;
}
...
if ($show_skip_form) {
    ...
    my $temp_fh = File::Temp->new( DIR    => $PATH,
                                   UNLINK => 0 );
...
if ( $align_seq_data ne '' ) {
    $align_tmp_fh = File::Temp->new( DIR    => $PATH,
                                     UNLINK => 0, );
...
if ( -f $align_temp_file ) {
    my ( $align_seq_count, $align_max_lengthh ) = fasta_check( $align_temp_file, $page )
      if $align_temp_file;
}
...
if ($tree_from_align) {
    ...
    input_err_page( $page, "Utility \"sreformat\" didn't work, or no alignment" )
      unless (@sre);
    ...
    input_err_page( $page, "Program \"quicktree\" didn't work, or no alignment" )
      unless (@newick);
    $tree_string = "";
...
#
if ($preset) {
    my $plants_file = CXGN::Phylo::File->new( $page->path_to("data/asterids.newick.txt") );
...
my ( $temp_fh, $temp_file ) = File::Temp::tempfile( "tree-XXXXXX", DIR => $temp_dir );
...
if ($shared_file) {
    if ( $shared_file =~ /\/\./ ) {
        $page->error_page( 'Invalid file location.', "Invalid file location: $shared_file" );
    }
    ...
    $shared_file =~ /^$shared_data_url\/cosii\/[\w\-\/]+\.\d\.ml\.tre$/
      or $page->error_page( "Invalid file location", "Invalid file location: $shared_file" );
    $title = $shared_file;
...
"This does not seem to be a legal Newick formatted tree.<br />Error possibly near highlighted token:<br />$error"
    ...
    $tree->impose_branch_length_minimum();    # impose the default minimum branch length (0.0001),
    ...
    if ($show_orthologs) {
        ...
        my $root_spec = $tree->get_root()->speciation_at_this_node($species_tree);

        # print "root_spec: $root_spec \n";
        if ( $root_spec < 0 ) {    # if trifurcation at root, reroot to one of the neighboring branches
    ...
    eval {
        ...
        if ($stored_genes) {
            ...
            unlink $filepath;    #we will always store to a new file, so this one must go!
        }
...
#        my $link_tail = "&amp;hilite=$key\&amp;height=$height&amp;file=$file&amp;action=display&amp;term=$term&amp;title=$urlencode{$title}&amp;style=$style&amp;show_blen=$show_blen&amp;align_temp_file=$align_temp_name&align_type=$align_type&hide_alignment=$hide_alignment";
    ...
    #
    my @search_nodes = $browser->get_tree()->search_label_name("$term");    # search_node_name("$term");
    ...
    if ( $tree_style eq "round" ) {
        ...
        $renderer = CXGN::Phylo::PNG_tree_renderer->new($tree);
    }

    $renderer->set_show_branch_length($show_blen);

    my @model_colors = ( [ 100, 40, 100 ], [ 50, 140, 50 ], [ 40, 40, 170 ], [ 150, 90, 10 ], [ 100, 100, 40 ] );
    ...
    my $new_align_temp_fh = File::Temp->new( DIR    => $PATH,
                                             UNLINK => 0, );
    my $new_align_temp = $new_align_temp_fh->filename;
    my $new_hilite_temp_fh = File::Temp->new( DIR    => $PATH,
                                              UNLINK => 0, );
    ...
    foreach my $n ( $tree->get_leaf_list ) {
        ...
        if ( $s % 2 == 1 ) {    # alignments shown with alternating darker and lighter colors...
            @mem_color          = ( 60,  60, 140 );
    ...
    print page_title_html( qq|<a href="?align_temp_show=$align_filename&newick_temp_show=$filename">Tree browser:</a> |
                           . $tree->get_name() );

    print $renderer->get_html_image_map( "tree_image_map", $new_align_temp, $new_hilite_temp, $align_type );

    my $file_for_link = File::Basename::basename( $browser->get_temp_file() );

    my $node_operations              = join( " ", $browser->get_node_operations() );
    my $new_rotated_node_operations  = join( " ", $node_operations, ( "r" . $browser->get_hilite() ) );
    my $new_hidden_node_operations   = join( " ", $node_operations, ( "h" . $browser->get_hilite() ) );
    my $new_set_root_node_operations = join( " ", $node_operations, ( "s" . $browser->get_hilite() ) );
    my $new_reset_root_operations    = join( " ", $node_operations, ( "t" . $browser->get_hilite() ) );

    my $active_if_hilite_button              = "";
    my $not_show_blen                        = !$show_blen;
    my $not_show_orthologs                   = !$show_orthologs;
    my $not_collapse_single_species_subtrees = !$collapse_single_species_subtrees;
    ...
    my ( $original_selected, $midpoint_selected, $minvar_selected, $mindl_selected, $maxmin_selected ) =
      ( "", "", "", "", "" );
    ...
    my $param_collapse_toggle = hash2param(
                                            \%PARAM,
                                            {
                                               collapse_single_species_subtrees => $not_collapse_single_species_subtrees
                                            }
                                          );
    ...
    my ( $param_smaller, $param_larger, $param_autosize ) =
      map { hash2param( \%PARAM, { height => $_ } ) } ( $smaller_height, $larger_height, "stretch" );
...
HTML
    ...
    my $non_species_tree_leaf_node_names = $browser->get_tree()->non_speciestree_leafnode_names();
    if ($show_orthologs) {
...
sub show_form {
    ...
    if ($example_show) {
        ...
        if ( open my $af, '<', "$HTML_ROOT_PATH/cgi-bin/tools/tree_browser/data/example_align_preset.txt" ) {
            ...
            $align_preset = "Example File Not Found";
        }
    ...
    print table(
        ...
        Tr(
            td(
                'Enter a tree in',
                a( { href => 'http://evolution.genetics.washington.edu/phylip/newicktree.html' }, 'newick', ),
...
sub convert_clustal_fasta {
    ...
    my $in = Bio::AlignIO->new( -file   => $clustal_file,
                                -format => 'clustalw' );
    my $cl_out = Bio::AlignIO->new( -file   => ">$fasta_file",
                                    -format => 'fasta', );
...
sub html_tableize {
    ...
    $outstring =~ s!\s{2,}!</td><td>!g;         # >= 2 of any whitespace char -> new cell
    ...
    if ( scalar @oglist == 0 ) {
        ...
        foreach my $o (@oglist) {
            ...
            while ( $og_row_string =~ m/<tr><td>([a-z]*)_(.*?)$cell_spacer/ ) {
                $og_row_string =~
                  s{<tr><td([a-z]*)_(.*?)$cell_spacer}{<tr><td>$ognumber$1$cell_spacer$2$cell_spacer};    #just 1 subst.

                my $implname = $2;    #this is ", " separated
                $implname =~ s/,\s*/\t/g;    # make it space separated
                my $ogletter = $1;
                my ( $key, $node ) = $tree->node_from_implicit_name_string($implname);
                ...
                $og_row_string =~
                  s{<tr><td>($ognumber$ogletter)?$cell_spacer}{<tr><td>$lnkstr$1$cell_spacer};    #put in the links
