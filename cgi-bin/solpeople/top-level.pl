...
unless($sp_person_id){$page->error_page("Login ID not found.","","","Login ID not found: $sp_person_id");}
unless($sp->get_username()){$page->error_page("Username not found.","","", "Username not found for sp_person_id '$sp_person_id'");}
...
print qq|<div align="right" style="margin-bottom: 0.5em">Not $username? <a href="login.pl?logout=yes">[log out]</a></div>\n|;
...
EOHTML
my $queries = qq|<div id="queryTable">|.CXGN::BlastWatch::get_queries($dbh,$sp_person_id).'</div>';
...
sub in(@){my $t = shift; (grep {$t eq $_} @_) ? 1 : 0}
...
EOHTML
	      ...
	      and my @projects = grep {$_ >= 1 && $_ <= 12} $sp->get_projects_associated_with_person
	    ) {
      my $chrinput = @projects > 1 ?
	simple_selectbox_html( name => 'chr',
			       choices => \@projects,
...
if ($sp->get_user_type() =~ /curator/i) { 
    my $publications=qq| <a href= "/search/pub_search.pl">Search the SGN publication database </a><br />|;
    $publications .= qq| <a href= "/search/pub_search.pl?w9b3_assigned_to=$sp_person_id">See your assigned publications</a><br />|;
    $publications .= qq| <a href= "/search/pub_search.pl?w9b3_status=pending">See publications pending curation</a><br />|;
    $publications .= qq| <a href= "/chado/fetch_pubmed.pl">Load new publications from PubMed</a><br />|;

    print info_section_html(title => 'Literature mining', contents => $publications);
}
    
if ($sp->get_user_type() =~ /submitter|curator|sequencer/i) { 

    my @loci = CXGN::Phenome::Locus::get_locus_ids_by_editor($dbh, $sp_person_id);
    my $top= 50;
    my $more = 0;
    my $max = @loci;
    if (@loci>24) { 
	...
	if ($i<$max) {$locus_annotations .= qq | <a href="/phenome/locus_display.pl?locus_id=$locus_id&amp;action=view">$symbol</a> | }  
	else {
	...
	$locus_annotations .=  html_optional_show('locus_annotations',
						  'Show more',
    ...
    print info_section_html(title => 'Annotated Loci', contents => $locus_annotations);
    ...
    if (@user_maps>0) {
	$html .= "<table alt=\"\" ><tr><td><i>Name</i></td><td></td><td></td><td><i>status</i></td></tr>"; 
	foreach my $um (@user_maps) { 
	    my $public = " (not public) ";
	    if ($um->get_map()->get_is_public()) { $public="(public)"; }
	    $html .= "<tr><td><b>".$um->get_short_name() ."</b></td><td> <a href=\"/cview/umap.pl?action=view&amp;user_map_id=".$um->get_id()."\" >[view]</a></td><td><a href=\"/cview/umap.pl?action=edit&amp;user_map_id=".$um->get_id()."\">[configure]</a></td><td>$public</td></tr>";
...
my $user_status_info = qq { Your current user status is <b>user</b>. Please contact <a href="mailto:sgn-feedback\@sgn.cornell.edu">SGN</a> to upgrade to a <b>submitter</b> account with more privileges. Submitters can upload user maps, EST data, and become locus editors. };

if ($sp->get_user_type() eq "submitter") { 
    $user_status_info = qq { Your current user status is <b>submitter</b>. You have the maximum user privileges on SGN. Please contact <a href="mailto:sgn-feedback\@sgn.cornell.edu">SGN</a> if you would like to change your user status.};
...
if ($sp->get_user_type() eq "sequencer") { 
    $user_status_info = qq { Your current user status is <b>sequencer</b>. You have maximum user privileges on SGN. };
}
print info_section_html(title=>'User Status', contents=>$user_status_info);
