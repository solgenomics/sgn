...
{
    $page->message_page("Sorry, but you cannot log in to this site.","This site is a mirror of <a href=\"http://sgn.cornell.edu\">sgn.cornell.edu</a>. To log in to SGN, go to <a href=\"http://sgn.cornell.edu/solpeople/login.pl\">SGN's login page</a>.");
}
if(!$login_controller->login_allowed())
{
    $page->message_page("Sorry, but logins are disabled while our server undergoes maintenance.","Logins should be available again within 24 hours. Should this condition persist, please contact <a href=\"mailto:sgn-feedback\@sgn.cornell.edu\">sgn-feedback\@sgn.cornell.edu</a>.");
}
$page->{request}->no_cache(1);#not sure if this is needed--just a bit of old code which probably can't hurt
my($username,$password,$goto_url,$logout)=$page->get_arguments("username","pd","goto_url","logout");
...
unless($page->{request}->is_initial_req() && ($goto_url!~/account-confirm/)) {  #if they were redirected here by a page, then save where they came from so we can send them back
...
if ($logout eq "yes")#if we are in the process of logging out
{
    ...
    $page->message_page('You have successfully logged out. Thanks for using SGN.');
}
if($username && $password)#else if we are in the process of logging in
{
    ...
    {
        $page->message_page("Account for user $username is disabled for reason '$account_disabled'.","If your account has not been confirmed, check your email for a confirmation from SGN.");
    }
    elsif($incorrect_password)#if their password is wrong, print a message
    {
        $page->message_page("Incorrect username or password.","<a href=\"send-password.pl\">[Lost password]</a>");
    } 
    elsif($duplicate_cookie)#if we couldn't generate a unique cookie string
    {
        $page->error_page("Sorry but the login system failed.","Please try again.","failed to generate new cookie string","Our random login cookie generator generated a duplicate value!");
    }    
    elsif($person_id)#if their username and password matched
    {        
        if($goto_url)#if they came from trying to work somewhere else, send them back
        {
...
{
    if($login_controller->has_session())#if there's no good reason for them to be here, send them to the menu page
    {
        $page->client_redirect("top-level.pl");
    }
}
