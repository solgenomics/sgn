...
# Hey kids! when you get a funny error message, add it here!
our @funny_error_messages =
  (
   #Found by john in perl documentation, not an actual error though.
   "Parser may be in an inconsistent state",
   #Found by marty in postgres. suggested solution he found online: "try again."
   "No parser with ID 0",
   # Python
   ...
   "Do _not_ leave metal bits visible!",
...
sub rube_goldberg {
    my $random_index=int(rand(@funny_error_messages));
    die($funny_error_messages[$random_index]);
}
