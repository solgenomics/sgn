...
{
    my($name)=$page->get_encoded_arguments("organization_name");
    if($name)
    {
        my $org_query=$dbh->prepare("select sp_organization_id from sp_organization where name=?");
        ...
        {
            my $org_insert=$dbh->prepare("insert into sp_organization (name) values (?)");
            $org_insert->execute($name);        
