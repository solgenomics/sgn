...
if($params{id}) {
   $clone = CXGN::Genomic::Clone->retrieve($params{id}+0) #the +0 makes sure it's numeric
	or clone_not_found_page($page, "No clone with id $params{id} could be found.", $clonequery);

} elsif($params{bac_id}) {

    $clone = CXGN::Genomic::Clone->retrieve($params{bac_id}+0)
	or clone_not_found_page($page, "No clone with id $params{bac_id} could be found.", $clonequery);
  ...
  $result->total_results > 1
    and $page->error_page("SGN bug: multiple clones with clone name $orig_clonename were found.");
#clone_not_found_page($page, "Multiple clones with clone name $orig_clonename (a.k.a. $clonename) were found.  That's an internal bug.", $link_pages, \%params,$clonequery);
  $result->total_results < 1
    and clone_not_found_page($page, "No clones with clone name $orig_clonename were found.", $clonequery);
...
unless ($clone && $clone->clone_id) {
  clone_not_found_page($page, "No clones found matching the search criteria.", $clonequery);
}
...
EOSQL
  ...
  if( $mas and @$mas ) {
    join '', map "$_\n", map {
      my ($marker_id,$pubmed_id,$sp_person_id,$comment_text,$fname,$lname,$org) = @$_;
      ...
      .info_table_html( Comment => $comment_text || '<span class="ghosted">none</span>',
			__border => 0,
		      )
      .'</div>'
    } @$mas;
...
my $physical_map_link = do {
  if( $overgo_html || $computational_associations_html || $manual_associations_html ) {
    my $chr = $clone->chromosome_num;
    my $url = "/cview/view_chromosome.pl?map_id=p9&hilite=".$clone->clone_name."&chr_nr=$chr";
...
my $agp_map_link = do {
  if( my @agp_pos = agp_positions($clone) ) {
    my $url = "/cview/view_chromosome.pl?map_id=agp&hilite=".$clone->clone_name."&chr_nr=".$agp_pos[0][0];
...
my $fish_link = do {
  my $on_fish_map = $dbh->selectall_arrayref(<<EOQ,undef,$clone->clone_id);
select map_id, chromo_num
from sgn.fish_result
where clone_id = ?
EOQ
  if($on_fish_map && @$on_fish_map) {
    my $url = "/cview/view_chromosome.pl?map_id=$on_fish_map->[0][0]&hilite=".$clone->clone_name."&chr_nr=$on_fish_map->[0][1]";
...
if( $self->_is_tomato( $clone ) ) {
    print info_section_html(title    => 'Physical mapping',
			    ...
			    .info_section_html( title         => "FISH images",
						contents      => CXGN::Fish::fish_image_html_table(CXGN::DB::Connection->new, $clone->clone_id),
						is_subsection => 1,
					      )
			    .info_section_html( title         => "IL Mapping",
						subtitle      => do {
						...
						is_empty      => !any(map $_->{val},@{$reg_info}{qw/il_proj il_chr il_bin/}),
						contents      =>
						info_table_html(__multicol => 3, __border => 0,
								'Assigned to project' => qq|<span id="clone_il_mapping_project">$reg_info->{il_proj}{disp}</span>|,
								'IL-mapped to chromosome' => qq|<span id="clone_il_mapping_chr">$reg_info->{il_chr}{disp}</span>|,
								'IL-mapped to IL segment' => qq|<span id="clone_il_mapping_line">$reg_info->{il_bin}{disp}</span>|,
							       )
						.info_table_html( __border => 0,
								  'IL mapping notes' => qq|<span id="clone_il_mapping_notes">$reg_info->{il_notes}{disp}</span>|,
							    )
					    ,
					    is_subsection => 1,
					  )
...
print info_section_html( title => 'Search again',
			 collapsible => 1,
			 contents => CXGN::Search::CannedForms::clone_search_form($page,$clonequery),
		       );
...
sub clone_not_found_page {
    ...
    print info_section_html(title => 'Search again',
			    contents => CXGN::Search::CannedForms::clone_search_form($page,$query),
			   );
...
sub sequencing_content {
  ...
  my $welcome_message_html = do {
    if ( $person && str_in($person->get_user_type,qw/sequencer curator/) ) {
      "Welcome, <b>".$person->get_first_name.' '.$person->get_last_name."</b>. You are logged in as a $user_type.\n";
    }
  } || '';
  ...
  my $is_finished      = $latest_seq && !($latest_seq =~ /-\d+$/) && $clone->seq !~ /N/;
  my %sequencing_files = $self->_is_tomato($clone) ? CXGN::TomatoGenome::BACPublish::sequencing_files( $clone, $c->config->{'ftpsite_root'} )
...
EOHTML
  ...
  my $restriction_frags_html = do {
	...
	$low_restriction_match_score = 1 if $match_score < $restriction_match_threshold;
	my $frag_listing = info_table_html(__sub => 1, __border => 0,
					   "$enzyme <i>in vitro</i> (".scalar(@$iv_frags).')'
					   => join(', ', @$iv_frags),
					   "$enzyme <i>in silico</i> (".scalar(@$is_frags).')'
					   => join(', ', @$is_frags),
...
EOQ
	...
	if (sum_ranges(@$matches)/$seqlen > 0.1) {
	  push @warnings, "more than 10% of sequence matches cloning vector";
	}
	if ($is_finished) {
	  my $seq_middle = Bio::Range->new(-start => int(0.1*$seqlen), -end => int(0.9*$seqlen));
	  #convert the vector matches to ranges so we can work with them more easily
	  my @vector_ranges = map { Bio::Range->new(-start => $_->[0]+1, -end => $_->[1]) } @$matches;
	  if (any(map {$_->overlaps($seq_middle)} @vector_ranges)) {
	    push @warnings, "vector found in middle 80% of sequence";
	  }
	}
      }
      ...
      if ($matches && @$matches) {
	push @warnings, "full sequence contains matches to E. coli K12 genome";
      }

      #check for tomato chloroplast hits
      $feature_query->execute('BLAST_tomato_chloroplast',$clone->clone_id);
      $matches = $feature_query->fetchall_arrayref;
      if ($matches && @$matches) {
	push @warnings, "full sequence contains matches to Tomato chloroplast genome";
      }

      #check for hits to other bacs
      $feature_query->execute('BLAST_tomato_bacs',$clone->clone_id);
      $matches = $feature_query->fetchall_arrayref;
      $matches = [grep {index($_->[2],$clone->clone_name_with_chromosome) != 0} @$matches];
      if ($matches && @$matches && sum_ranges(@$matches)/$seqlen > 0.2 ) {
	push @warnings, "more than 20% of full sequence matches other tomato genomic clones";
      }

      #check for the low restriction match score flag
      if ($low_restriction_match_score) {
	push @warnings, "poor correspondence with <i>in vitro</i> restriction fragments<br/>(match score below $restriction_match_threshold)";
      }

      #check this sequence's length vs. estimated length
      if ( $clone->estimated_length
	   and (my $pdiff = abs($seqlen - $clone->estimated_length)/$clone->estimated_length) > 0.4
	 ) {
	push @warnings, sprintf("sequence length %s is %0.1f%% different from estimated length %s",
...
my $htgs_phase_html = ('none','1 - fragmented, unordered','2 - fragmented, ordered','3 - finished')[$clone->seqprops->{htgs_phase}] || 'not sequenced';
my $sequenced_by_html = do {
  if( my $seq_shortname = $clone->seqprops->{sequenced_by} ) {
    #look up the organization for that shortname
    my $matching_orgs = $dbh->selectcol_arrayref('select name from sgn_people.sp_organization where shortname = ?',undef,$seq_shortname);
    if(@$matching_orgs == 1) {
      $matching_orgs->[0]
    }
    else {
...
return
  ...
  .info_table_html(
		   ...
		   ( $self->_is_tomato( $clone )
		       ? ('Chromosome Assembly' => $agp_map_link || 'this clone is not part of a chromosome assembly')
		       : ()
		   ),
		   ...
		   'End Sequences'     => do {
		     ...
		     if (my @chromats = $clone->chromat_objects) {
		       join '',map {"$_\n"} ('<ul style="margin: 0; padding-left: 2em">',
					     ( map {
					       '<li>'.$_->read_link_html($link_pages{read_info_page}).'</li>'
					     } @chromats
					     ),
					     '</ul>',
					    );
...
sub render_tomato_bac_annot_download {
    ...
    my %sequencing_files = CXGN::TomatoGenome::BACPublish::sequencing_files( $clone, $c->config->{'ftpsite_root'} );
    my @formats = distinct(grep {$_} map {my ($k) = /_([^_]+)$/; $k} keys %sequencing_files);

    my @available_analyses = grep { #find analyses that have all their files available
      my $a = $_ eq 'all' ? '' : $_.'_';
      all(map {$sequencing_files{$a.$_}} @formats)
    } 'all',CXGN::TomatoGenome::BACSubmission->list_analyses;

    if (@formats && @available_analyses) {
      my $set_select = simple_selectbox_html(choices => \@available_analyses,
					     name    => 'annot_set',
					     id      => 'annot_set_selector',
					    );
      my $type_select = simple_selectbox_html(choices => \@formats,
					      name    => 'annot_format',
...
sub agp_positions {
  ...
  my (undef,$agp_file) = CXGN::TomatoGenome::BACPublish::tpf_agp_files($chr);
