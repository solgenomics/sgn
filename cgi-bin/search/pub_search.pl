######################################################################
#
#  Search tsgn publications and displays the results.
#
######################################################################
...
if(%params) {
    ...
    my $has_permission=0;
    if ($login_user_type eq 'curator' || $login_user_type eq 'submitter' || $login_user_type eq 'sequencer') { $has_permission=1;}
    #####
	...
	my $assigned_to=CXGN::People::Person->new($dbh, $assigned_to_id)->get_first_name();
	...
	my $stats=$stat;
	$stats=  qq|<select id="pub_stat" onchange="Publication.updatePubCuratorStat(this.value, $pub_id)">
                    $stat_options
                    </select> 
                   | if $has_permission;
	...
	$curators=qq|<select id="pub_curator_select" onchange="Publication.updatePubCuratorAssigned(this.value, $pub_id)">
                     $curator_options
                     </select>
                    | if $has_permission ;
	
	my $pub=CXGN::Chado::Publication->new($dbh, $pub_id);
	push @results, [map {$_} (qq|<a href="/chado/publication.pl?pub_id=$pub_id">$pub_ref</a>|, 
...
$results_html .= columnar_table_html(headings => [
					 'Title',
					 'Status',
					 'Assigned to',
					 'Stored'
...
if (@results) {
    print blue_section_html('Publication search results', ,sprintf('<span class="paginate_summary">%s matches </span>', $result->total_results,$result->time),$results_html);
}else {
    print '<h4>No matches found</h4>';
}
    
    print info_section_html(title    => 'Search again', 
			    contents =>CXGN::Search::CannedForms::publication_search_form($page,$query)
	);
