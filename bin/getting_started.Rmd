...
# create plot!
ggplot()+ggtitle("W8848–3 Plot recreated")+labs(x="Additive relationship",y="Marker-based distance")+geom_text(aes(x=x,y=y,label=names(x)))
...
ped <- read.table ("~/sgn/bin/ped.txt", header = FALSE, sep = "\t", check.names = FALSE, stringsAsFactors = FALSE)
colnames(ped)[1] <- "Name"
...
markers <- read.table ("~/sgn/bin/267genotypes-p3.txt", header = TRUE, check.names = FALSE, stringsAsFactors = FALSE)
...
# We have too many genotypes, so make a list of all the accessions in the pedigree matrix, and use it to subset the genetic distance matrix
include_list <- labels(Amat[,"COB1103"])
include_list
subset_matrix <- dist_matrix[rownames(dist_matrix) %in% include_list, colnames(dist_matrix) %in% include_list] 
...
x <- Amat[!rownames(Amat) %in% "COB1103","COB1103"]
y <- subset_matrix["COB1103",!colnames(subset_matrix) %in% "COB1103"]
