...
upload_metabolomics_phenotypes.pl

=head1 SYNOPSIS

upload_metabolomics_phenotypes.pl -H [dbhost] -D [dbname] -U [dbuser] -P [dbpass] -w [basepath] -ap [archive_path] -tf [tempfiles_subdir] -m [mode] -j [sp_job_id]
...
ARGUMENTS
 ...
 -m mode (required), either "verify" to check the file without saving anything, or "store" to save
    its metabolite values to the database
 -j sp_job_id of the job that submitted this script (required)

=head2 DESCRIPTION

perl bin/upload_metabolomics_phenotypes.pl -H breedbase_db -D breedbase -U postgres -P postgres -w /home/cxgn/sgn -ap /archive -tf /static/documents/tempfiles -m store -j 17

Reads an already archived metabolomics spreadsheet, together with the metabolite details file that
goes with it, and either checks it or saves the metabolite values in it.
...
# Connect to databases. Everything shares one handle so that the metabolite values, the protocol
...
try {
    ...
    upload_metabolomics_phenotypes();
} catch {
    push @error_status, $_;
...
sub upload_params {
    ...
    if (!$upload_params) {
        die "Job $sp_job_id does not describe a metabolomics upload.\n";
    }

    return $upload_params;
}

=head2 upload_metabolomics_phenotypes()

Reads the uploaded spreadsheet and then either checks the metabolite values in it or saves them,
depending on the mode.

=cut

sub upload_metabolomics_phenotypes {
    ...
    if ($mode eq 'verify') {
        verify_metabolomics_phenotypes($parsed_file);
    } else {
        store_metabolomics_phenotypes($parsed_file);
    }
}
...
sub validate_and_parse {
    ...
    my $metabolites_file_path = $params->{archived_metadata_filename};
    my $upload_original_name = $params->{upload_original_name};

    my $validate_file = $parser->validate($validate_type, $file_path, undef, $params->{data_level}, $chado_schema, undef, $params->{protocol_id}, $metabolites_file_path);
    if (!$validate_file) {
    ...
    my $parsed_file = $parser->parse($validate_type, $file_path, undef, $params->{data_level}, $chado_schema, undef, $params->{user_id}, $context, $params->{protocol_id}, $metabolites_file_path);
    if (!$parsed_file) {
...
=head2 verify_metabolomics_phenotypes($parsed_file)

Checks the metabolite values against the database without storing any of them.

=cut

sub verify_metabolomics_phenotypes {
...
=head2 store_metabolomics_phenotypes($parsed_file)

Saves the metabolite values to the database, creating the protocol they were read against if the
uploader described a new one.

=cut

sub store_metabolomics_phenotypes {
    my $parsed_file = shift;

    my %parsed_data = %{$parsed_file->{'data'}};
    my @plots = @{$parsed_file->{'units'}};
    my @metabolites = @{$parsed_file->{'variables'}};
    my %metabolites_details = %{$parsed_file->{'variables_desc'}};

    my $protocol_id = $params->{protocol_id};
    if (!$protocol_id) {
        $protocol_id = create_protocol(\@metabolites, \%metabolites_details);
    }
    ...
    my $store_phenotypes = build_store_phenotypes(\@plots, \%parsed_data_agg, \%phenotype_metadata, 1);
...
=head2 create_protocol($metabolites, $metabolites_details)

Saves the protocol the uploader described, and returns its id. The metabolites the file turned out
to hold are recorded on it, so that later uploads can be checked against them.

The mass spectrometry details are only recorded for an MS protocol, since they say nothing about
any other kind of equipment.

=cut

sub create_protocol {
    my $metabolites = shift;
    my $metabolites_details = shift;

    my $high_dim_metabolomics_protocol_cvterm_id = SGN::Model::Cvterm->get_cvterm_row($chado_schema, 'high_dimensional_phenotype_metabolomics_protocol', 'protocol_type')->cvterm_id();
    my $high_dim_metabolomics_protocol_prop_cvterm_id = SGN::Model::Cvterm->get_cvterm_row($chado_schema, 'high_dimensional_phenotype_protocol_properties', 'protocol_property')->cvterm_id();

    my %metabolomics_protocol_prop = (
        header_column_names => $metabolites,
        header_column_details => $metabolites_details,
        equipment_type => $params->{protocol_equipment_type},

        target => $params->{protocol_target},
        sample_collection_description => $params->{protocol_sample_collection},
        sample_extraction_description => $params->{protocol_sample_extraction},
        rawdata_transformation_descripson => $params->{protocol_rawdata_transformation},
        metabolite_identification_description => $params->{protocol_metabolite_identification},

        equipment_description => $params->{protocol_equipment_desc},
        data_process_description => $params->{protocol_data_process_desc},
        phenotype_type => $params->{protocol_phenotype_type},
        phenotype_units => $params->{protocol_phenotype_units},

        protocol_publication => $params->{protocol_publication}
    );
    if ($params->{protocol_equipment_type} eq 'MS') {
        $metabolomics_protocol_prop{chromatography_system_brand} = $params->{protocol_chromatography_system_brand};
        $metabolomics_protocol_prop{chromatography_column_brand} = $params->{protocol_chromatography_column_brand};
        $metabolomics_protocol_prop{ms_brand} = $params->{protocol_ms_brand};
        $metabolomics_protocol_prop{ms_type} = $params->{protocol_ms_type};
        $metabolomics_protocol_prop{ms_instrument_type} = $params->{protocol_ms_instrument_type};
        $metabolomics_protocol_prop{ms_ion_mode} = $params->{protocol_ms_ion_mode};

        $metabolomics_protocol_prop{chromatography_type} = $params->{protocol_chromatography_type};
        $metabolomics_protocol_prop{chromatography_autosampler_model} = $params->{protocol_chromatography_autosampler_model};
        $metabolomics_protocol_prop{chromatography_column_type} = $params->{protocol_chromatography_column_type};
        $metabolomics_protocol_prop{chromatography_protocol_description} = $params->{protocol_chromatography_protocol};
        $metabolomics_protocol_prop{mass_spectrometry_protocol_description} = $params->{protocol_mass_spectrometry_protocol};
        $metabolomics_protocol_prop{ms_scan_mz_range} = $params->{protocol_ms_scan_mz_range};
    }
...
Builds the object that checks and saves the metabolite values. Checking a file does not need to know
