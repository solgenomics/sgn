...
#### Step 7. Saving new trial in the database {-}
...
```{r echo=FALSE, out.width='95%', fig.align='center'}
knitr::include_graphics(c('assets/images/manage_trials_upload_trial_2.png','assets/images/manage_trials_upload_trial_template.png'))
```
...
| **plot\_name** | **accession\_name** | **plot\_number** | **block\_number** | **is\_a\_control** | **rep\_number** | **range\_number** | **row\_number** | **col\_number** | **seedlot\_name** | **num\_seed\_per\_plot** | **weight\_gram\_seed\_per\_plot** | **entry\_number** |
|----------------|---------------------|------------------|-------------------|--------------------|-----------------|-------------------|-----------------|-----------------|-------------------|--------------------------|-----------------------------------|------------------|
...
In the case where you have uploaded an experiment using seedlot_names that are not already present in the database, you will be taken to the next screen. If the seedlots in your file are all already in the database, this step will be skipped.
The reason it is necessary for your seedlots to be in the database before you can add a trial using them is that a single seedlot can be used among many trials and therefore must exist as a separate entity in the database. Once all your seedlots are in the database, click to move to the next step.
...
```{r echo=FALSE, out.width='95%', fig.align='center'}
knitr::include_graphics('assets/images/manage_trials_upload_trial_7.png')
```
...
```{r echo=FALSE, out.width='95%', fig.align='center'}
knitr::include_graphics('assets/images/manage_trials_upload_trial_8.png')
```
...
For trials with spatial layout information and stored phenotypes, you can check and correct for spatial autocorrelation by clicking the "Calculate Spatial Correction" button above the field map. Doing so will open the spatial corrections dialog. For an in-depth review, visit the chapter on [*data analysis tools*](#data-analysis-tools). 

```{r echo=FALSE, out.width='95%', fig.align='center'}
knitr::include_graphics('assets/images/heatmap_spatial_correction_button.png')
```
...
```{r echo=FALSE, out.width='75%', fig.align='center'}
knitr::include_graphics('assets/images/spatial_layout_usage_help.png')
```

```{r echo=FALSE, out.width='95%', fig.align='center'}
knitr::include_graphics(c('assets/images/spatial_layout_usage_help_1.png','assets/images/spatial_layout_usage_help_2.png','assets/images/spatial_layout_usage_help_3.png'))
```
...
```{r echo=FALSE, out.width='95%', fig.align='center'}
knitr::include_graphics(c('assets/images/fieldmap_view_plot_image.png','assets/images/replace_plot_accession_form.png'))
```
...
### Adding Management Regimes

In addition to trial designs and trial layouts, you can specify the specific ways a trial was managed using the *Management Regime* section of the *Experimental Design* tab of the trial detail page.


```{r echo=FALSE, out.width='95%', fig.align='center'}
knitr::include_graphics('assets/images/management_regime_section.png')
```
...
```{r echo=FALSE, out.width='95%', fig.align='center'}
knitr::include_graphics('assets/images/management_regime_details.png')
```
...
### Adding Experimental Treatments

In contrast to management regimes, which describe how a field trial as a whole was managed, you may wish to store information regarding treatments applied differentially to the stocks of a trial. Treatments may be added from the *Treatments* section of the *Experimental design* tab.

```{r echo=FALSE, out.width='95%', fig.align='center'}
knitr::include_graphics('assets/images/trial_treatment_section.png')
```
...
Adding treatments is very similar to [uploading phenotypic data](#managing-phenotypic-data): click on the 'Add Treatments' button to view the treatment upload dialog:

```{r echo=FALSE, out.width='95%', fig.align='center'}
knitr::include_graphics('assets/images/trial_add_treatment_dialog.png')
```

Just as with phenotype uploads, you can supply a spreadsheet specifying the stocks and the treatments with optional timestamps. To ensure the treatment column headers are correct, consult the [treatment search page](#managing-observation-variables) or the ontology browser. 
...
Plant entries can be added to your trial in four ways:
...
(3) Uploaded using plant index number instead of name (the name is automatically generated)
(4) Uploaded using the number of plants per plot. This allows different numbers of plant per plot, as opposed to option 1.
...
The trial upload template (both the single- and multiple-trial design formats) has an `accession_name` column, which is used to specify the name of the primary accession for each plot (each plot must have a primary accession).  The upload templates now support an optional `intercrop_accession_name` column, which can be used to add one or more additional accessions to a plot.  You can include more than one intercropping accession by separating their names with a comma or leave the column empty when there is no intercropped accession in the plot.


| trial_name  | plot_name             | plot_number | accession_name | intercrop_accession_name |
|-------------|-----------------------|-------------|----------------|--------------------------|
| B4I_2025_NY | B4I_2025_NY-PLOT_5001 | 5001        | IL22-5365      | NDP160034Y               |
| B4I_2025_NY | B4I_2025_NY-PLOT_5002 | 5002        | IL22-5365      | NDP250123Y,NDP250155Y    |
| B4I_2025_NY | B4I_2025_NY-PLOT_5003 | 5003        | IL21-4364      | NDP250178Y               |
| B4I_2025_NY | B4I_2025_NY-PLOT_5004 | 5004        | IL21-4020      |                          |
| B4I_2025_NY | B4I_2025_NY-PLOT_5005 | 5005        | IL23-1619      | NDP150221G               |

**An example multi-trial upload template that includes multiple accessions per plot**

*NOTE: some required columns are left out for clarity*

### Trial Details

The intercropped accessions are displayed as metadata in various parts of the trial detail page.

The *Experimental Design* section displays the intercropped accessions in the Accessions table as well as the Plots table.

```{r echo=FALSE, out.width='95%', fig.align='center'}
knitr::include_graphics('assets/images/intercropping_plots_table.png')
```
...
```{r echo=FALSE, out.width='95%', fig.align='center'}
knitr::include_graphics('assets/images/intercropping_fieldmap_tooltip.png')
```

### Phenotype Downloads

All of the phenotype trait data downloads (from the trial detail page, search wizard, etc) still include the `germplasmDbId` and `germplasmName` columns, which include the the database id and name of the primary accession.  The `intercropGermplasmDbId` and `intercropGermplasmName` columns will include the id(s) and name(s) of any intercropped germplasm added to the plots.  If there are no intercropped germplasm for a particular plot, the columns will be empty.  If there are more than one intercropped germplasm added to a plot, the IDs and names will be separated by a comma.

```{r echo=FALSE, out.width='95%', fig.align='center'}
knitr::include_graphics('assets/images/intercropping_phenotype_download.png')
```

**An example phenotype download that includes multiple accessions per plot**

*NOTE: some columns are left out for clarity*
...
```{r echo=FALSE, out.width='95%', fig.align='center'}
knitr::include_graphics('assets/images/intercropping_trait_search.png')
```