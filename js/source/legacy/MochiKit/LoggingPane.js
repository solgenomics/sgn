...
MochiKit.LoggingPane.createLoggingPane = function (inline/* = false */, windowHeight) {
    ...
    if (!m._loggingPane || m._loggingPane.closed) {
        m._loggingPane = new m.LoggingPane(inline, MochiKit.Logging.logger, windowHeight);
    }
    return m._loggingPane;
};

/** @id MochiKit.LoggingPane.LoggingPane */
MochiKit.LoggingPane.LoggingPane = function (inline/* = false */, logger/* = MochiKit.Logging.logger */, windowHeight) {
	...
	if(!windowHeight) windowHeight = 200;
    if (!inline) {
        // name the popup with the base URL for uniqueness
        var url = win.location.href.split("?")[0].replace(/[#:\/.><&-]/g, "_");
        var name = uid + "_" + url;
        var nwin = win.open("", name, "dependent,resizable,height=" + windowHeight);
        if (!nwin) {
    ...
    if (inline) {
        ...
        style += "; height: 98%; top:0px;"; //100%, top-alignment causes the bottom line to be partially hidden --ccarpita
    }
