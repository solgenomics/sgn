...
jQuery(document).ready(function ($) {
    ...
    jQuery('#trial_upload_breeding_program').change(function(){
        ...
        load_upload_trial_plot_name_templates();
    });

    jQuery('#trial_upload_add_plot_name_template_button').click(function(e){
        e.preventDefault();
        var breeding_program_id = jQuery('#trial_upload_breeding_program option:selected').data('program-id');
        ...
        jQuery('#add_autogenerated_name_metadata_dialog').data('refresh_callback', load_upload_trial_plot_name_templates);
    ...
    function format_plot_name_template_preview(name_attributes) {
        return name_attributes.map(function(attr) {
    ...
    function load_upload_trial_plot_name_templates() {
        var breeding_program_id = jQuery('#trial_upload_breeding_program option:selected').data('program-id');
        var $select = jQuery('#trial_upload_plot_name_template');

        $select.empty();
        $select.append(jQuery('<option>').val('').text('Default plot naming: {trialName}-PLOT_{plotNumber}'));
        jQuery('#trial_upload_plot_name_template_preview').html('');
            ...
            success: function(response) {
                if (response && response.data && response.data.length > 0) {
                    response.data.forEach(function(template_obj) {
                        ...
                        var preview = format_plot_name_template_preview(tmpl.name_attributes || []);
                        var $option = jQuery('<option>').val(JSON.stringify(template_obj)).text(format_name + ': ' + preview);
                        $select.append($option);
                    });
                }
            },
            error: function() {
                jQuery('#trial_upload_plot_name_template_preview').html('<span class="text-danger">Error loading plot name templates.</span>');
            }
        });
    }
