...
jQuery(document).ready(function($) {
    ...
    jQuery('#unobsolete_stock_submit').click( function() {
        const note = jQuery('#unobsolete_note').val();
        const stock_id = jQuery('#unobsolete_stock_id').val();
        if (!stock_id) {
            alert ("Error retrieving stock id");
            return;
        }

        const stock_name = jQuery('#unobsolete_stock_name').val();
        if (!stock_name) {
            alert ("Error retrieving stock name");
            return;
        }

        const confirmation = confirm('Are you sure you want to un-obsolete this stock?' + "  " + stock_name);
