...
export function init(main_div){
    ...
    $.ajax({
      ...
      success : function(data) {    
        ...
        boxplot.getVariables().then(vs=>{
          if(vs.length<1){
            bp.find('.boxplotter-result').html('<strong class="not-enough-data">Not Enough Data with the Given Constraints</strong>');
  ...
  function drawVariableSelect(){
    boxplot.getVariables().then(vs=>{
      var vars = d3.select(main_div).select(".boxplotter-variable-select").selectAll("option").data(vs);
      vars.exit().remove();
  ...
  function drawGroupBys(){
    boxplot.getGroupings().then(grps=>{
      ...
      var optSelects = d3.select(main_div).select(".boxplotter-group-list")
        .selectAll(".groupBy")
        .on("change",function(){
          readGrouping();
        })
