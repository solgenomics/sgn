...
export function process_file(file_data, upload_type, config) {
    ...
    switch(upload_type) {
        ...
        case 'genotyping_plate' :
            ...
            jQuery('#genotype_plate_upload_additional_info_div').show();
        ...
        case 'genotyping_data' : 
            ...
            jQuery('#genotype_location_data_div').show();
            jQuery('#genotype_tissue_sample_data_div').show();
            ...
            let kasp_info_select = jQuery('#genotype_data_kasp_info_select');
            kasp_info_select.empty();
            archived_files_list.forEach(item => {
                const opt = jQuery('<option></option>').val(item.file_id).text(item.filename);
                kasp_info_select.append(opt);
            });
...
function display_genotyping_plate_upload_choices() {
    jQuery('#upload_type_choices_div').html('<select style="width:50%" class="form-control form-control-sm" id="genotype_plate_upload_type_choice_select">'+
                    ...
                    '</select>' +
                    '<br><br>' +
                    '<div id="genotype_plate_upload_additional_info_div" hidden>' +
                    '<div class="form-group clearfix">' +
                        '<label class="col-sm-5 control-label">Genotyping Project Name: </label>' +
                        '<div class="col-sm-7">' +
                            '<span id="genotype_plate_upload_project_select_div"></span>' +
                        '</div>' +
                    '</div><br><br>' +
                    '<div class="form-group clearfix">' +
                        ...
                        '<div class="col-sm-7">' +
                            '<input class="form-control" id="genotype_plate_upload_name" name="genotype_plate_upload_name" type="text" placeholder=" e.g. 18DNA00001"/>' +
                        '</div>' +
                    '</div><br><br>' +
                    '<div class="form-group clearfix">' +
                        '<label class="col-sm-5 control-label">Plate Format: </label>' +
                        '<div class="col-sm-7">' +
                            '<select class="form-control" id="genotype_plate_upload_format">' +
                    ...
                    '<div class="form-group clearfix">' +
                        ...
                        '<div class="col-sm-7">' +
                            '<select class="form-control" id="genotype_plate_upload_sample_type">' +
                    ...
                    '<div class="form-group clearfix">' +
                        ...
                        '<div class="col-sm-7">' +
                            '<textarea class="form-control" id="genotype_plate_upload_description"></textarea>' +
                        '</div>' +
                    '</div><br><br>' +
                    '<div class="form-group clearfix">' +
                        '<label class="col-sm-5 control-label">Include Identifiers Generated by Genotyping Facility: </label>' +
                        '<div class="col-sm-7">' +
                            '<input type="checkbox" id="genotype_plate_upload_include_facility_identifiers">' +
                        '</div>' +
                    '</div><br><br>' +
                    '</div>' +
    ...
    get_select_box('projects', 'genotype_plate_upload_project_select_div', { 'name' : 'genotype_plate_upload_project_id', 'id' : 'genotype_plate_upload_project_id', 'get_genotyping_projects' : '1', 'empty' : 1 });
}
...
function display_genotyping_data_upload_choices() {
    jQuery('#upload_type_choices_div').html('<select style="width:50%" class="form-control form-control-sm" id="genotype_data_upload_type_choice_select">'+
            ...
            '</div><br>'+
            '<div class="form-group" id="genotype_location_data_div" hidden>' +
            ...
            '</div><br>' + 
            '<br>' + 
            '<div class="form-group" id="genotype_tissue_sample_data_div" hidden>' + 
                '<label class="col-sm-5 control-label">Exported Tissue Sample Names Include Numbers Generated by Genotyping Facility (e.g. sample_name:IGD1001:09):<small>The generated number is separated from the tissue sample name in the database by a colon character.</small> </label>' +
                '<div class="col-sm-7">' + 
            ...
            '</div><br>'+
            '<div id="genotype_data_kasp_info_div" hidden>'+
                '<br><br><p><b>Select KASP marker information file:</b></p><br><select id="genotype_data_kasp_info_select"></select>'+
            '</div>'+
            '<br><br><br><div id="genotype_data_choices_next_btn_div" hidden><button class="btn btn-primary" id="genotype_data_upload_choices_next_btn">Next</button></div>');
}
...
function display_genotyping_data_upload_additional_choices() {
    ...
    switch (genotyping_data_type) {
        case 'genotype_data_intertek' : 
            ...
            jQuery('#genotype_data_kasp_info_div').hide();
            break;
        case 'genotype_data_kasp' :
            jQuery('#genotype_data_kasp_info_div').show();
...
function populate_genotyping_plate_validate_submit_data(event) {
    ...
    let genotyping_project_id = jQuery('#genotype_plate_upload_project_id').val();
    let genotyping_project_name = jQuery('#genotype_plate_upload_project_id option:selected').text();
    if (!genotyping_project_id) {
        alert("Please select a genotyping project.");
        return;
    }

    let genotyping_plate_name = jQuery('#genotype_plate_upload_name').val();
    if (!genotyping_plate_name) {
        alert("Please provide a genotyping plate ID.");
        return;
    }
    ...
    populate_validate_submit_data(genotype_plate_upload_type, file_data, {
        ...
        genotyping_project_name : genotyping_project_name,
        genotyping_plate_name : genotyping_plate_name,
        plate_format : plate_format,
        sample_type : sample_type,
        description : description,
        include_facility_identifiers : include_facility_identifiers
    });
}
...
function populate_genotyping_data_validate_submit_data(event) {
    ...
    let tissue_sample_names_include_numbers = jQuery('#upload_genotype_vcf_include_igd_numbers').prop('checked');
    if (tissue_sample_names_include_numbers == null) {
    ...
    if (!data_location_id) {
        alert("Error: Could not read location value.");
        return;
    }
    ...
    if (genotype_data_format == "genotype_data_intertek") {
        ...
        populate_validate_submit_data(genotype_data_format, file_data, {
            ...
            tissue_sample_names_include_numbers : tissue_sample_names_include_numbers,
            data_location_id : data_location_id,
            data_location : data_location
        });
    } else if (genotype_data_format == "genotype_data_kasp") {
        let kasp_info_file = {id: jQuery('#genotype_data_kasp_info_select').val(), name : jQuery('#genotype_data_kasp_info_select option:selected').text()};
        populate_validate_submit_data(genotype_data_format, file_data, {
            kasp_info_file_id : kasp_info_file.id,
            kasp_info_file_name : kasp_info_file.name,
            genotyping_protocol_id : selected_protocols[0].id,
            genotyping_project_id : selected_projects[0].id,
            genotyping_protocol_name : selected_protocols[0].name,
            genotyping_project_name : selected_projects[0].name,
...
export function submit_upload_job() {
    ...
    switch(submit_params.upload_type) {
        ...
        case 'genotyping_plate_custom_android' :
            jQuery.ajax({
                url : '/ajax/breeders/parsegenotypetrial',
                type : 'POST',
                data : {
                    'archived_file_id' : submit_params.file_id,
                    'genotyping_plate_upload_type' : submit_params.upload_type,
                    'genotyping_trial_name' : submit_params.additional_args.genotyping_plate_name,
                    'upload_include_facility_identifiers' : submit_params.additional_args.include_facility_identifiers,
                    'genotyping_project_id' : submit_params.additional_args.genotyping_project_id,
                    'genotyping_plate_format' : submit_params.additional_args.plate_format,
                    'genotyping_plate_sample_type' : submit_params.additional_args.sample_type,
                    'genotyping_plate_description' : submit_params.additional_args.description
                },
                success : function(response) {
                    if (response.error || response.error_string) {
                        console.log(response.error || response.error_string);
                    }
                    refresh_upload_tables();
                },
                error : function() {
                    alert("An error occurred uploading the genotyping plate. Check console.");
                }
            });
            break;
        case 'genotype_data_vcf' :
            jQuery.ajax({
                url : '/ajax/genotype/upload',
                type : 'POST',
                data : {
                    'archived_vcf_file_id' : submit_params.file_id,
                    'upload_genotype_vcf_include_igd_numbers' : submit_params.additional_args.tissue_sample_names_include_numbers,
                    'upload_genotype_protocol_id' : submit_params.additional_args.genotyping_protocol_id,
                    'upload_genotype_project_id' : submit_params.additional_args.genotyping_project_id,
                    'upload_genotype_location_select' : submit_params.additional_args.data_location_id,
                    'upload_genotype_accept_warnings': ignore_warnings
                },
                success : function(response) {
                    if (response.error) {
                        console.log(response.error);
                ...
                error : function() {
                    alert("An error occurred uploading VCF data. Check console.");
                }
            });
            break;
        case 'genotype_data_tassel' :
            jQuery.ajax({
                ...
                type : 'POST',
                data : {
                    'archived_tassel_file_id' : submit_params.file_id,
                    'upload_genotype_vcf_include_igd_numbers' : submit_params.additional_args.tissue_sample_names_include_numbers,
                    'upload_genotype_protocol_id' : submit_params.additional_args.genotyping_protocol_id,
                    'upload_genotype_project_id' : submit_params.additional_args.genotyping_project_id,
                    'upload_genotype_location_select' : submit_params.additional_args.data_location_id,
                ...
                error : function() {
                    alert("An error occurred uploading Tassel HDF5 data. Check console.");
                }
            });
            break;
        case 'genotype_data_intertek' :
            jQuery.ajax({
                ...
                type : 'POST',
                data : {
                    'archived_intertek_file_id' : submit_params.file_id,
                    'archived_intertek_marker_info_file_id' : submit_params.additional_args.intertek_info_file_id,
                    'upload_genotype_vcf_include_igd_numbers' : submit_params.additional_args.tissue_sample_names_include_numbers,
                    'upload_genotype_protocol_id' : submit_params.additional_args.genotyping_protocol_id,
                    'upload_genotype_project_id' : submit_params.additional_args.genotyping_project_id,
                    'upload_genotype_location_select' : submit_params.additional_args.data_location_id,
                ...
                error : function() {
                    alert("An error occurred uploading Intertek data. Check console.");
                }
            });
            break;
        case 'genotype_data_kasp' :
            jQuery.ajax({
                ...
                type : 'POST',
                data : {
                    'archived_kasp_file_id' : submit_params.file_id,
                    'archived_kasp_marker_info_file_id' : submit_params.additional_args.kasp_info_file_id,
                    'upload_genotype_vcf_include_igd_numbers' : submit_params.additional_args.tissue_sample_names_include_numbers,
                    'upload_genotype_protocol_id' : submit_params.additional_args.genotyping_protocol_id,
                    'upload_genotype_project_id' : submit_params.additional_args.genotyping_project_id,
                    'upload_genotype_location_select' : submit_params.additional_args.data_location_id,
                ...
                error : function() {
                    alert("An error occurred uploading KASP data. Check console.");
                }
            });
            break;
        case 'genotype_data_ssr' :
            jQuery.ajax({
                ...
                type : 'POST',
                data : {
                    'archived_ssr_file_id' : submit_params.file_id,
                    'upload_genotype_vcf_include_igd_numbers' : submit_params.additional_args.tissue_sample_names_include_numbers,
                    'upload_genotype_protocol_id' : submit_params.additional_args.genotyping_protocol_id,
                    'upload_genotype_project_id' : submit_params.additional_args.genotyping_project_id,
                    'upload_genotype_location_select' : submit_params.additional_args.data_location_id,
                ...
                error : function() {
                    alert("An error occurred uploading SSR data. Check console.");
                }
            });
            break;
        case 'locations' :
...
export function commit_upload_job(job_id) {
    ...
    switch(upload_type) {
        ...
        case 'genotyping_plate_custom_android' :
            let plate_data = {
                design : job.args.additional_args.design,
                genotyping_facility_submit : 'no',
                genotyping_project_id : job.args.additional_args.genotyping_project_id,
                name : job.args.additional_args.genotyping_plate_name,
                sample_type : job.args.additional_args.genotyping_plate_sample_type,
                plate_format : job.args.additional_args.genotyping_plate_format,
                description : job.args.additional_args.genotyping_plate_description
            };
            jQuery.ajax({
                ...
                data : {
                    ...
                    'archived_file_id' : job.args.additional_args.file_id,
                    'genotyping_plate_upload_type' : job.args.additional_args.file_type
                },
                success : function(response) {
                    if (response.error) {
                        console.log(response.error);
                ...
                error : function() {
                    alert("An error occurred storing the genotyping plate, check console.");
                    return;
                }
            });
            break;
