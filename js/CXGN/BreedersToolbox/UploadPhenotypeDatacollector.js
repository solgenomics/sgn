...
jQuery( document ).ready( function() {
	...
	var uploadFile = jQuery("#upload_datacollector_phenotype_file_input").val();
	if (uploadFile === '') {
	    alert("Please select a file");
	} else {
	    jQuery('#working_msg').html("Verifying Phenotype Spreadsheet and Data");
	    jQuery('#working_modal').modal("show");
	    jQuery('#upload_datacollector_phenotype_file_form').attr("action", "/ajax/phenotype/upload_verify/datacollector");
            jQuery('#upload_datacollector_phenotype_file_form').submit();
	}
    });
	...
	complete: function (response) {
	    ...
	    jQuery("#upload_datacollector_phenotype_submit_verify").attr('disabled', true);
		...
		jQuery("#upload_datacollector_phenotype_submit_store").attr('disabled', false);
            }
            if (response.error) {
	    ...
	    message_text += "</ul>";
	    jQuery("#upload_phenotype_datacollector_verify_status").html(message_text);
		...
		if (uploadFile === '') {
	    	    ...
	    	    jQuery('#working_modal').modal("show");
	    	    jQuery('#upload_datacollector_phenotype_file_form').attr("action", "/ajax/phenotype/upload_store/datacollector");
            	    jQuery('#upload_datacollector_phenotype_file_form').submit();
		}
            });
		...
		post: function () { },
	    	complete: function (response) {
	    	    jQuery('#working_modal').modal("hide");
		    jQuery("#upload_phenotype_datacollector_verify_status").empty();
		    jQuery("#upload_datacollector_phenotype_submit_store").attr('disabled', true);
	    	    var message_text = "<hr><ul class='list-group'>";
	    	    if (response.success) {
		        var arrayLength = response.success.length;
