...
$mech->with_test_level(
    local => sub {
        ...
        my ($first_sample_row) = $bs_schema->resultset('BsSample')->search(
            undef,
            {
        ...
        if ( defined $first_sample_row ) {
            ...
            my $first_sample_name =
              $first_sample_row->get_column('sample_name');

            $urls{'biosource sample page'} =
              "/biosource/sample.pl?id=$first_sample_id";
            $urls{'biosource sample page'} =
              "/biosource/sample.pl?name=$first_sample_name";
