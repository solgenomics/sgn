...
is(@$test_basic_h2_file[2]->[1], 'fresh.root.weight', "check value of row name in a table");
is(@$test_basic_h2_file[2]->[4], '15.147', "check value of Vres fresh.root.weight in a table");

# run test for dataset with outliers but with false outliers parameter
my $outliers_included_dataset_id = 1;
my $outliers_included_trait_id = "fresh root weight"; # wyglada na to ze nie ma to wiekszego znaczena dla tego testu
...
ok($rdata_full_set->{figure3}, "figure 3 returned");
ok($rdata_full_set->{figure4}, "figure 4 returned");
ok($rdata_full_set->{h2Table}, "h2Table returned");
...
ok($rdata_excluded_set->{figure3}, "figure 3 returned");
ok($rdata_excluded_set->{figure4}, "figure 4 returned");
ok($rdata_excluded_set->{h2Table}, "h2Table returned");
ok($rdata_excluded_set->{h2CsvTable}, "h2CsvTable returned");

# check if files were created
ok( -e "static/".$rdata_excluded_set->{figure3}, "figure 3 created");
