...
use CXGN::AddAutogeneratedNameMetadata;
...
#test adding project
$mech->post_ok('http://localhost:3010/ajax/transformation/add_transformation_project', [ 'project_name' => 'transformation_project_1', 'project_program_id' => $breeding_program_id,
...
my $add_autogenerated_name_metadata = CXGN::AddAutogeneratedNameMetadata->new({
    schema => $schema,
    dbh => $dbh,
...
my $vector_stock = CXGN::Stock::Vector->new({
                    schema => $schema,
                    check_name_exists => 0,
...
my $vector_stock_id = $vector_stock->store();
push @all_new_stocks, $vector_stock_id;

#adding transformation ID
...
my $control_vector_rs = $schema->resultset('Stock::Stock')->find({ name => 'TTC' });
my $control_vector_stock_id = $control_vector_rs->stock_id();
push @all_new_stocks, $control_vector_stock_id;

#adding control experiment
$mech->post_ok('http://localhost:3010/ajax/transformation/add_transformation_identifier', [ 'transformation_identifier' => 'UG1TTC', 'plant_material' => 'UG120001', 'vector_construct' => 'TTC', 'notes' => 'test', 'transformation_project_id' => $project_id, 'is_a_control' => 1]);

$response = decode_json $mech->content;
is($response->{'success'}, '1');

my $control_transformation_rs = $schema->resultset('Stock::Stock')->find({name => 'UG1TTC'});
my $control_transformation_stock_id = $control_transformation_rs->stock_id();
push @all_new_stocks, $control_transformation_stock_id;

#setting control for UG1TT1
$mech->post_ok('http://localhost:3010/ajax/transformation/set_transformation_control', [ 'transformation_stock_id' => $transformation_stock_id, 'control_stock_id' => $control_transformation_stock_id, 'program_name' => 'test']);

$response = decode_json $mech->content;
is($response->{'success'}, '1');

#checking if UG1TTC is a control
my $control_transformation_obj = CXGN::Transformation::Transformation->new({schema=>$schema, dbh=>$dbh, transformation_stock_id=>$control_transformation_stock_id});
my $control_info = $control_transformation_obj->get_transformation_info();
my $is_a_control = $control_info->[0]->[5];
is ($is_a_control, '1');

#retrieve control experiment info for UG1TT1
...
#test_qPCR upload
my $file = $f->config->{basepath} . "/t/data/stock/qPCR_upload.xlsx";
my $ua = LWP::UserAgent->new;
$response = $ua->post(
    'http://localhost:3010/ajax/transformation/upload_qPCR_data',
    ...
    Content      => [
        ...
        "qPCR_data_vector_id" => $vector_stock_id,
        "qPCR_data_vector_name" => 'TT1',
        "qPCR_tissue_type" => 'leaf',
        "qPCR_endogenous_control" => 'GAPDH',
        "qPCR_normalization_method" => 'CASS_Delta_Cq',
...
#check calculated normalized values after uploading Cq values
my $related_accessions = CXGN::Stock::RelatedStocks->new({dbic_schema => $schema, stock_id =>$vector_stock_id});
my $result = $related_accessions->get_vector_related_accessions();
my @sorted_result = natkeysort {($_->[1])} @$result;
my $transformant_name_1 = $sorted_result[0][1];
my $expression_data_1 = $sorted_result[0][7];
my $transformant_name_2 = $sorted_result[1][1];
my $expression_data_2 = $sorted_result[1][7];
my $expression_data_1_hash = decode_json $expression_data_1;
my $relative_expression_data_1 = $expression_data_1_hash->{'leaf'}->{'2026-03-01'}->{'geneA'}->{'relative_expression_values'};
my $expression_data_2_hash = decode_json $expression_data_2;
my $relative_expression_data_2 = $expression_data_2_hash->{'leaf'}->{'2026-03-01'}->{'geneA'}->{'relative_expression_values'};

is($transformant_name_1, 'TT1_BTI1');
is($relative_expression_data_1->{'relative_expression'}, 0.000211);
is($relative_expression_data_1->{'number_of_replicates'}, 4);
is($relative_expression_data_1->{'stdevp'}, 0.000101);

is($transformant_name_2, 'TT1_BTI2');
is($relative_expression_data_2->{'relative_expression'}, 0.002041);
is($relative_expression_data_2->{'number_of_replicates'}, 2);
is($relative_expression_data_2->{'stdevp'}, 0.000141);
