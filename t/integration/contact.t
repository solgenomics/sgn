...
sub send_complete_form_and_check {
    ...
    #print $mech->content;
    $mech->submit_form_ok(
        { 'form_name' => $form_name, 'fields' => \%entry_for_field },
...
sub send_blank_form_and_check {
    ...
    foreach my $field (@fields) {
        $mech->text_like(
            qr/($field)|(message) is required/i,
...
sub test_currently_filled_and_oppositely_filled_forms {
    my ( $mech, $form_name, $field, $filledEntry, $testDesc, %entry_for_field )
      = @_;
    for my $i ( 0 .. 1 ) {
        ...
        for my $j ( 0 .. 1 ) {
            ...
            foreach my $otherFields ( keys %entry_for_field ) {
                unless ( $$filledEntry{$otherFields} ) {
                    $$filledEntry{$otherFields} =
                      $entry_for_field{$otherFields};
                    $otherFields = ucfirst $otherFields;
                    $mech->text_like(
                        qr/($otherFields)|(Message) is required/i,
                    ...
                    $mech->text_unlike( qr/$otherFields is required/i,
                        "$otherFields should not show error message" );
                }
            }
            $mech->back();
            $mech->submit_form_ok( { 'form_name' => $form_name },
                "Check submitting; blank form was sent." )
              if $j == 1;
        }
    }
}
