...

{
        ...
        $mech->follow_link_ok( { text_regex => qr/QTL Tomato/, n => 1 }, 'a link from the QTL searcg page to a QTL population works' );
        ...
        # population page
        $mech->get_ok('/phenome/population.pl?population_id=12', 'Population page');
        ...
        my $phenotype_download_link = $mech->find_link( text_regex => qr/phenotype raw data/i );
        ok( $phenotype_download_link, 'got a phenotype data  download link' );
        $mech->links_ok( [$phenotype_download_link], 'phenotype data download link works' );
        cmp_ok( length( $mech->content ), '>=', 1000, 'got at least 1KB of data from the phenotype data download' );

        # Genotype data download page
        my $genotype_download_link = $mech->find_link( text_regex => qr/genotype raw data/i );
        ok( $genotype_download_link, 'got a genotype data  download link' );
        $mech->links_ok( [$genotype_download_link], 'genotype data download link works' );
        cmp_ok( length( $mech->content ), '>=', 1000, 'got at least 1KB of data from the genotype data download' );
        ...
        my $qtl_analysis_link = $mech->find_link( url_regex => qr !population_indls.pl?population_id=12&cvterm_id=39945!);
        ...
        $mech->get_ok('/phenome/population_indls.pl?population_id=12&cvterm_id=39945', ' a population_indls.pl page');
        $mech->content_contains($_) for (
        'Population Details',
        'QTLs',
       ...
       # check a qtl.pl page
       my $qtl_link = $mech->find_link(url_regex => qr !/phenome/qtl.pl?population_id=12&term_id=39945&chr=3! );
       $mech->links_ok([$qtl_link], 'qtl.pl page');
       $mech->get_ok($qtl_link, 'a qtl.pl page link from the population_indls.pl');
