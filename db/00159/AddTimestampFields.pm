...
Add timestamp fields to sgn_people.list table

    mx-run AddTimestampFields [options] -H hostname -D dbname -u username [-F]
...
sub init_patch {
    ...
    my $description = 'Add timestamp fields to sgn_people.list';
...
ALTER TABLE sgn_people.list ADD COLUMN "modify_timestamp" VARCHAR;
SQL
