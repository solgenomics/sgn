...
UpdateTraitsViewForMultipleTraitOntologies.pm

=head1 SYNOPSIS

mx-run UpdateTraitsViewForMultipleTraitOntologies [options] -H hostname -D dbname -u username [-F]
...
This patch:
 - updates the traits view to support more than one trait ontology
...
This patch updates the traits view to support more than one trait ontology
...
CREATE OR REPLACE VIEW public.traits AS
  ...
  (((cvterm.name::text || '|'::text) || db.name::text) || ':'::text) || dbxref.accession::text AS trait_name
    ...
    LEFT JOIN cvterm_relationship is_variable ON cvterm.cvterm_id = is_variable.subject_id AND is_variable.type_id IN (SELECT cvterm_id FROM cvterm WHERE name = 'VARIABLE_OF' AND cv_id = (SELECT cv_id FROM cv where cv.name = 'relationship') )
