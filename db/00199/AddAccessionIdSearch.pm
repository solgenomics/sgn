...
AddAccessionIdSearch.pm


=head1 SYNOPSIS

mx-run AddAccessionIdSearch [options] -H hostname -D dbname -u username [-F]
...
-- ADDING ACCESSION IDS searh --

DROP VIEW IF EXISTS public.accessions_ids CASCADE;
CREATE VIEW public.accessions_ids AS
  SELECT stock.stock_id AS accessions_id_id,
  stock.stock_id AS accessions_id_name
  FROM stock
  WHERE stock.type_id = (SELECT cvterm_id from cvterm where cvterm.name = 'accession') AND is_obsolete = 'f'
  GROUP BY stock.stock_id, stock.uniquename;
ALTER VIEW accessions OWNER TO web_usr;
...
SELECT public.materialized_phenoview.accession_id as accessions_id_id,
    public.stock.stock_id AS seedlot_id
   FROM public.materialized_phenoview
   LEFT JOIN stock_relationship seedlot_relationship ON materialized_phenoview.accession_id = seedlot_relationship.subject_id AND seedlot_relationship.type_id IN (SELECT cvterm_id from cvterm where cvterm.name = 'collection_of')
   LEFT JOIN stock ON seedlot_relationship.object_id = stock.stock_id AND stock.type_id IN (SELECT cvterm_id from cvterm where cvterm.name = 'seedlot')
  GROUP BY 1,2;
ALTER VIEW accessions_idsXseedlots OWNER TO web_usr;

DROP VIEW IF EXISTS public.accessionsXaccessions_ids CASCADE;
...
DROP VIEW IF EXISTS public.accessions_idsXbreeding_programs CASCADE;
CREATE VIEW public.accessions_idsXbreeding_programs AS
SELECT public.materialized_phenoview.accession_id AS accessions_id_id,
       public.materialized_phenoview.breeding_program_id
   FROM public.materialized_phenoview
  GROUP BY 1,2;
  ALTER VIEW public.accessions_idsXbreeding_programs OWNER TO web_usr;

DROP VIEW IF EXISTS public.accessions_idsXgenotyping_protocols CASCADE;
CREATE VIEW public.accessions_idsXgenotyping_protocols AS
SELECT public.materialized_genoview.accession_id as accessions_id_id,
    public.materialized_genoview.genotyping_protocol_id
   FROM public.materialized_genoview
  GROUP BY 1,2;
  ALTER VIEW public.accessions_idsXgenotyping_protocols OWNER TO web_usr;

DROP VIEW IF EXISTS public.accessions_idsXlocations CASCADE;
CREATE VIEW public.accessions_idsXlocations AS
SELECT public.materialized_phenoview.accession_id as accessions_id_id,
    public.materialized_phenoview.location_id
   FROM public.materialized_phenoview
  GROUP BY 1,2;
...
DROP VIEW IF EXISTS public.accessions_idsXplants CASCADE;
CREATE VIEW public.accessions_idsXplants AS
SELECT public.materialized_phenoview.accession_id as accessions_id_id,
    public.stock.stock_id AS plant_id
   FROM public.materialized_phenoview
   JOIN public.stock ON(public.materialized_phenoview.stock_id = public.stock.stock_id AND public.stock.type_id = (SELECT cvterm_id from cvterm where cvterm.name = 'plant'))
  GROUP BY 1,2;
  ALTER VIEW accessions_idsXplants OWNER TO web_usr;

DROP VIEW IF EXISTS public.accessions_idsXtissue_sample CASCADE;
CREATE VIEW public.accessions_idsXtissue_sample AS
SELECT public.materialized_phenoview.accession_id as accessions_id_id,
  ...
  GROUP BY 1,2;
ALTER VIEW accessions_idsXtissue_sample OWNER TO web_usr;

DROP VIEW IF EXISTS public.accessions_idsXplots CASCADE;
CREATE VIEW public.accessions_idsXplots AS
SELECT public.materialized_phenoview.accession_id as accessions_id_id,
    public.stock.stock_id AS plot_id
   FROM public.materialized_phenoview
   JOIN public.stock ON(public.materialized_phenoview.stock_id = public.stock.stock_id AND public.stock.type_id = (SELECT cvterm_id from cvterm where cvterm.name = 'plot'))
  GROUP BY 1,2;
ALTER VIEW accessions_idsXplots OWNER TO web_usr;

DROP VIEW IF EXISTS public.accessions_idsXsubplots CASCADE;
CREATE VIEW public.accessions_idsXsubplots AS
SELECT public.materialized_phenoview.accession_id as accessions_id_id,
    public.stock.stock_id AS subplot_id
   FROM public.materialized_phenoview
   JOIN public.stock ON(public.materialized_phenoview.stock_id = public.stock.stock_id AND public.stock.type_id = (SELECT cvterm_id from cvterm where cvterm.name = 'subplot'))
  GROUP BY 1,2;
ALTER VIEW accessions_idsXsubplots OWNER TO web_usr;

DROP VIEW IF EXISTS public.accessions_idsXtrial_designs CASCADE;
CREATE VIEW public.accessions_idsXtrial_designs AS
SELECT public.materialized_phenoview.accession_id as accessions_id_id,
    trialdesign.value AS trial_design_id
   FROM public.materialized_phenoview
   JOIN public.projectprop trialdesign ON materialized_phenoview.trial_id = trialdesign.project_id AND trialdesign.type_id = (SELECT cvterm_id from cvterm where cvterm.name = 'design' )
  GROUP BY 1,2;
ALTER VIEW accessions_idsXtrial_designs OWNER TO web_usr;

DROP VIEW IF EXISTS public.accessions_idsXtrial_types CASCADE;
CREATE VIEW public.accessions_idsXtrial_types AS
SELECT public.materialized_phenoview.accession_id as accessions_id_id,
    trialterm.cvterm_id AS trial_type_id
   FROM public.materialized_phenoview
   JOIN projectprop trialprop ON materialized_phenoview.trial_id = trialprop.project_id AND trialprop.type_id IN (SELECT cvterm_id from cvterm JOIN cv USING(cv_id) WHERE cv.name = 'project_type' )
   JOIN cvterm trialterm ON trialprop.type_id = trialterm.cvterm_id
  GROUP BY 1,2;
ALTER VIEW accessions_idsXtrial_types OWNER TO web_usr;

DROP VIEW IF EXISTS public.accessions_idsXtrials CASCADE;
CREATE VIEW public.accessions_idsXtrials AS
SELECT public.materialized_phenoview.accession_id as accessions_id_id,
    public.materialized_phenoview.trial_id
   FROM public.materialized_phenoview
  GROUP BY 1,2;
ALTER VIEW accessions_idsXtrials OWNER TO web_usr;

DROP VIEW IF EXISTS public.accessions_idsXyears CASCADE;
CREATE VIEW public.accessions_idsXyears AS
SELECT public.materialized_phenoview.accession_id as accessions_id_id,
    public.materialized_phenoview.year_id
   FROM public.materialized_phenoview
  GROUP BY 1,2;
ALTER VIEW accessions_idsXyears OWNER TO web_usr;

DROP VIEW IF EXISTS public.accessions_idsXgenotyping_projects CASCADE;
CREATE VIEW public.accessions_idsXgenotyping_projects AS 
    SELECT accession_id as accessions_id_id,
           genotyping_project_id
    FROM materialized_genoview
    GROUP BY 1,2;
ALTER VIEW public.accessions_idsXgenotyping_projects OWNER TO web_usr;

DROP VIEW IF EXISTS public.accessions_idsXorganisms CASCADE;
CREATE VIEW public.accessions_idsXorganisms AS
select s.stock_id as accessions_id_id,
       s.organism_id
from stock s 
where s.type_id = (select cvterm_id from cvterm where cvterm.name = 'accession')
group by 1,2;
ALTER VIEW accessions_idsXorganisms OWNER TO web_usr;

DROP VIEW IF EXISTS public.accessions_idsXtraits CASCADE;
CREATE VIEW public.accessions_idsXtraits as
SELECT public.materialized_phenoview.accession_id as accessions_id_id,
    public.materialized_phenoview.trait_id
   FROM public.materialized_phenoview
  GROUP BY 1,2;
ALTER VIEW accessions_idsXtraits OWNER TO web_usr;

DROP VIEW IF EXISTS public.accessions_idsXtrait_components CASCADE;
CREATE VIEW public.accessions_idsXtrait_components AS
SELECT public.materialized_phenoview.accession_id as accessions_id_id,
    trait_component.cvterm_id AS trait_component_id
   FROM public.materialized_phenoview
   JOIN cvterm trait ON(materialized_phenoview.trait_id = trait.cvterm_id)
   ...
   JOIN cvterm trait_component ON(cvterm_relationship.subject_id = trait_component.cvterm_id)
  GROUP BY 1,2;
ALTER VIEW accessions_idsXtrait_components OWNER TO web_usr;
...
ALTER VIEW organismsXseedlots OWNER TO web_usr;
...
SELECT  gp.genotyping_project_id, s.stock_id AS tissue_sample_id
from genotyping_projects gp
join nd_experiment_project nep on nep.project_id  = gp.genotyping_project_id  
...
SELECT nep.nd_protocol_id as genotyping_protocol_id, s.stock_id AS tissue_sample_id
from nd_experiment_protocol nep
join nd_experiment ne on ne.nd_experiment_id = nep.nd_experiment_id 
join nd_experiment_stock nes on nes.nd_experiment_id = nep.nd_experiment_id  
join stock s on s.stock_id = nes.stock_id 
where ne.type_id = (select cvterm_id from cvterm where name = 'genotyping_experiment')
and s.type_id = (select cvterm_id from cvterm where name = 'tissue_sample')
 GROUP BY 1, 2;
