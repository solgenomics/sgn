...
UpdatePhenotypeJsonbTableMaterializedViewWithPlantedCrosses.pm

=head1 SYNOPSIS

mx-run UpdatePhenotypeJsonbTableMaterializedViewWithPlantedCrosses [options] -H hostname -D dbname -u username [-F]
...
This patch adds planted crosses into the materialized_phenotype_jsonb_table view. 
...
package UpdatePhenotypeJsonbTableMaterializedViewWithPlantedCrosses;
...
This patch adds planted crosses into the materialized view phenotype jsonb table.
...
SELECT observationunit.stock_id AS observationunit_stock_id, observationunit.uniquename AS observationunit_uniquename, observationunit_cvterm.name AS observationunit_type_name, stock_relationship.type_id, germplasm_or_planted_cross.uniquename AS germplasm_or_planted_cross_uniquename, germplasm_or_planted_cross.stock_id AS germplasm_or_planted_cross_stock_id, germplasm_or_planted_cross.type_id AS germplasm_or_planted_cross_type_id, germplasm_or_planted_cross_type_name.name AS germplasm_or_planted_cross_type_name, rep.value AS rep, block_number.value AS block, plot_number.value AS plot_number, row_number.value AS row_number, col_number.value AS col_number, plant_number.value AS plant_number, is_a_control.value AS is_a_control, project.project_id AS trial_id, project.name AS trial_name, project.description AS trial_description, plot_width.value AS plot_width, plot_length.value AS plot_length, field_size.value AS field_size, field_trial_is_planned_to_be_genotyped.value AS field_trial_is_planned_to_be_genotyped, field_trial_is_planned_to_cross.value AS field_trial_is_planned_to_cross, breeding_program.project_id AS breeding_program_id, breeding_program.name AS breeding_program_name, breeding_program.description AS breeding_program_description, year.value AS year, design.value AS design, location_id.value AS location_id, planting_date.value AS planting_date, harvest_date.value AS harvest_date, folder.project_id AS folder_id, folder.name AS folder_name, folder.description AS folder_description, seedplot_planted.value AS seedlot_transaction, seedlot.stock_id AS seedlot_stock_id, seedlot.uniquename AS seedlot_uniquename, seedlot_current_weight.value AS seedlot_current_weight_gram, seedlot_current_count.value AS seedlot_current_count, seedlot_seedlot_box.value AS seedlot_box_name,
    ...
    COALESCE(
        jsonb_agg(jsonb_build_object('trait_id', phenotype.cvalue_id, 'trait_name', (((cvterm.name::text || '|'::text) || db.name::text) || ':'::text) || dbxref.accession::text, 'value', phenotype.value, 'phenotype_id', phenotype.phenotype_id, 'outlier', outlier.value, 'create_date', phenotype.create_date, 'uniquename', phenotype.uniquename, 'phenotype_location_id', nd_geolocation.nd_geolocation_id, 'phenotype_location_name', nd_geolocation.description, 'collect_date', phenotype.collect_date, 'operator', phenotype.operator))
    ...
    JOIN stock_relationship ON(observationunit.stock_id=stock_relationship.subject_id AND stock_relationship.type_id IN ($plot_of_type_id, $plant_of_type_id, $subplot_of_type_id, $tissue_sample_of_type_id))
    JOIN stock AS germplasm_or_planted_cross ON(stock_relationship.object_id=germplasm_or_planted_cross.stock_id AND (germplasm_or_planted_cross.type_id = $accession_type_id OR germplasm_or_planted_cross.type_id = $cross_type_id))
    JOIN cvterm AS germplasm_or_planted_cross_type_name ON(germplasm_or_planted_cross_type_name.cvterm_id=germplasm_or_planted_cross.type_id)
    LEFT JOIN stock_relationship AS available_seedlot_rel ON (available_seedlot_rel.subject_id=germplasm_or_planted_cross.stock_id AND available_seedlot_rel.type_id=$seedlot_collection_of_type_id)
...
CREATE INDEX materialized_phenotype_jsonb_table_germplasm_or_cross_stock_idx ON public.materialized_phenotype_jsonb_table(germplasm_or_planted_cross_stock_id) WITH (fillfactor=100);
CREATE INDEX materialized_phenotype_jsonb_table_germplasm_or_cross_uniquename_idx ON public.materialized_phenotype_jsonb_table(germplasm_or_planted_cross_uniquename) WITH (fillfactor=100);
