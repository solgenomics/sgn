...
MatviewTimstampsAndImprovedSubqueries.pm

=head1 SYNOPSIS

mx-run MatviewTimstampsAndImprovedSubqueries [options] -H hostname -D dbname -u username [-F]
...
This patch updates the materialized views to specify cv name in subqueries used to retrieve cvterm_ids, and add operator and timestamps
...
package MatviewTimstampsAndImprovedSubqueries;
...
CREATE MATERIALIZED VIEW public.seedlots AS
    ...
    FROM stock
    WHERE stock.type_id = (SELECT cvterm_id FROM cvterm join cv using(cv_id) WHERE cvterm.name = 'seedlot' AND cv.name = 'stock_type') AND is_obsolete = 'f'
    GROUP BY 1,2
WITH DATA;
...
(((cvterm.name::text || '|'::text) || db.name::text) || ':'::text) || dbxref.accession::text AS trait_component_name
    FROM cv
    JOIN cvprop ON(cv.cv_id = cvprop.cv_id AND cvprop.type_id IN (SELECT cvterm_id FROM cvterm join cv using(cv_id) WHERE cvterm.name = ANY ('{object_ontology,attribute_ontology,method_ontology,unit_ontology,time_ontology}') AND cv.name = 'composable_cvtypes'))
...
CREATE MATERIALIZED VIEW public.traits AS
  ...
  (((cvterm.name::text || '|'::text) || db.name::text) || ':'::text) || dbxref.accession::text AS trait_name
	FROM cv
    JOIN cvprop ON(cv.cv_id = cvprop.cv_id AND cvprop.type_id IN (SELECT cvterm_id FROM cvterm join cv using(cv_id) WHERE cvterm.name = 'trait_ontology' AND cv.name = 'composable_cvtypes'))
    JOIN cvterm ON(cvprop.cv_id = cvterm.cv_id)
	  JOIN dbxref USING(dbxref_id)
    JOIN db ON(dbxref.db_id = db.db_id)
    LEFT JOIN cvterm_relationship is_variable ON cvterm.cvterm_id = is_variable.subject_id AND is_variable.type_id = (SELECT type.cvterm_id FROM cv JOIN cvterm ON (cv.cv_id = cvterm.cv_id) JOIN cvprop ON (cvterm.cvterm_id = cvprop.type_id) JOIN cvterm AS type ON (cvprop.cv_id = type.cv_id) WHERE cv.name = 'composable_cvtypes' AND cvterm.name = 'trait_ontology' AND type.name = 'VARIABLE_OF')
  ...
  (((cvterm.name::text || '|'::text) || db.name::text) || ':'::text) || dbxref.accession::text AS trait_name
  FROM cv
    JOIN cvprop ON(cv.cv_id = cvprop.cv_id AND cvprop.type_id IN (SELECT cvterm_id FROM cvterm join cv using(cv_id) WHERE cvterm.name = 'composed_ontology' AND cv.name = 'composable_cvtypes'))
    JOIN cvterm ON(cvprop.cv_id = cvterm.cv_id)
    JOIN dbxref USING(dbxref_id)
    JOIN db ON(dbxref.db_id = db.db_id)
    LEFT JOIN cvterm_relationship is_subject ON cvterm.cvterm_id = is_subject.subject_id
...
SELECT trial.project_id AS trial_id,
    ...
    JOIN project_relationship ON(breeding_program.project_id = object_project_id AND project_relationship.type_id = (SELECT cvterm_id FROM cvterm join cv using(cv_id) WHERE cvterm.name = 'breeding_program_trial_relationship' AND cv.name = 'project_relationship'))
...
SELECT public.materialized_phenoview.breeding_program_id,
   ...
   LEFT JOIN nd_experiment ON nd_experiment_project.nd_experiment_id = nd_experiment.nd_experiment_id AND nd_experiment.type_id IN (SELECT cvterm_id FROM cvterm join cv using(cv_id) WHERE cvterm.name = 'seedlot_experiment' AND cv.name = 'experiment_type')
...
LEFT JOIN stock_relationship seedlot_relationship ON materialized_genoview.accession_id = seedlot_relationship.subject_id AND seedlot_relationship.type_id IN (SELECT cvterm_id FROM cvterm join cv using(cv_id) WHERE cvterm.name = 'collection_of' AND cv.name = 'stock_relationship')
LEFT JOIN stock ON seedlot_relationship.object_id = stock.stock_id AND stock.type_id IN (SELECT cvterm_id FROM cvterm join cv using(cv_id) WHERE cvterm.name = 'seedlot' AND cv.name = 'stock_type')
  GROUP BY 1,2
WITH DATA;
...
FROM nd_experiment
LEFT JOIN nd_experiment_stock ON nd_experiment.nd_experiment_id = nd_experiment_stock.nd_experiment_id
WHERE nd_experiment.type_id IN (SELECT cvterm_id FROM cvterm join cv using(cv_id) WHERE cvterm.name = 'seedlot_experiment' AND cv.name = 'experiment_type')
GROUP BY 1,2
WITH DATA;
...
DROP MATERIALIZED VIEW IF EXISTS public.breeding_programsXtrait_components CASCADE;
...
DROP MATERIALIZED VIEW IF EXISTS public.locationsXtrait_components CASCADE;
...
DROP MATERIALIZED VIEW IF EXISTS public.plantsXtrait_components CASCADE;
...
DROP MATERIALIZED VIEW IF EXISTS public.plotsXtrait_components CASCADE;
...
DROP MATERIALIZED VIEW IF EXISTS public.trait_componentsXtraits CASCADE;
CREATE MATERIALIZED VIEW public.trait_componentsXtraits AS
SELECT traits.trait_id,
trait_component.cvterm_id AS trait_component_id
FROM traits
JOIN cvterm_relationship ON(traits.trait_id = cvterm_relationship.object_id AND cvterm_relationship.type_id = (SELECT cvterm_id FROM cvterm join cv using(cv_id) WHERE cvterm.name = 'contains' AND cv.name = 'relationship'))
JOIN cvterm trait_component ON(cvterm_relationship.subject_id = trait_component.cvterm_id)
GROUP BY 1,2
WITH DATA;
...
DROP MATERIALIZED VIEW IF EXISTS public.trait_componentsXtrials CASCADE;
...
DROP MATERIALIZED VIEW IF EXISTS public.trait_componentsXtrial_designs CASCADE;
...
DROP MATERIALIZED VIEW IF EXISTS public.trait_componentsXtrial_types CASCADE;
...
DROP MATERIALIZED VIEW IF EXISTS public.trait_componentsXyears CASCADE;
...
DROP MATERIALIZED VIEW IF EXISTS public.breeding_programsXlocations CASCADE;
...
DROP MATERIALIZED VIEW IF EXISTS public.breeding_programsXplants CASCADE;
...
DROP MATERIALIZED VIEW IF EXISTS public.breeding_programsXplots CASCADE;
...
DROP MATERIALIZED VIEW IF EXISTS public.breeding_programsXtrial_designs CASCADE;
...
DROP MATERIALIZED VIEW IF EXISTS public.breeding_programsXtrial_types CASCADE;
...
DROP MATERIALIZED VIEW IF EXISTS public.breeding_programsXtrials CASCADE;
...
DROP MATERIALIZED VIEW IF EXISTS public.breeding_programsXyears CASCADE;
...
DROP MATERIALIZED VIEW IF EXISTS public.locationsXplants CASCADE;
...
DROP MATERIALIZED VIEW IF EXISTS public.locationsXplots CASCADE;
...
DROP MATERIALIZED VIEW IF EXISTS public.locationsXtrial_designs CASCADE;
...
DROP MATERIALIZED VIEW IF EXISTS public.locationsXtrial_types CASCADE;
...
DROP MATERIALIZED VIEW IF EXISTS public.locationsXtrials CASCADE;
...
DROP MATERIALIZED VIEW IF EXISTS public.locationsXyears CASCADE;
...
DROP MATERIALIZED VIEW IF EXISTS public.plantsXplots CASCADE;
...
DROP MATERIALIZED VIEW IF EXISTS public.plantsXtrials CASCADE;
...
DROP MATERIALIZED VIEW IF EXISTS public.plantsXtrial_designs CASCADE;
...
DROP MATERIALIZED VIEW IF EXISTS public.plantsXtrial_types CASCADE;
...
DROP MATERIALIZED VIEW IF EXISTS public.plantsXyears CASCADE;
...
DROP MATERIALIZED VIEW IF EXISTS public.plotsXtrials CASCADE;
...
DROP MATERIALIZED VIEW IF EXISTS public.plotsXtrial_designs CASCADE;
...
DROP MATERIALIZED VIEW IF EXISTS public.plotsXtrial_types CASCADE;
...
DROP MATERIALIZED VIEW IF EXISTS public.plotsXyears CASCADE;
...
DROP MATERIALIZED VIEW IF EXISTS public.trial_designsXtrial_types CASCADE;
...
DROP MATERIALIZED VIEW IF EXISTS public.trial_designsXtrials CASCADE;
...
DROP MATERIALIZED VIEW IF EXISTS public.trial_designsXyears CASCADE;
...
DROP MATERIALIZED VIEW IF EXISTS public.trial_typesXtrials CASCADE;
...
DROP MATERIALIZED VIEW IF EXISTS public.trial_typesXyears CASCADE;
...
DROP MATERIALIZED VIEW IF EXISTS public.trialsXyears CASCADE;
