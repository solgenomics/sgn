...
UpdateMaterializedViewTrialDesigns.pm

=head1 SYNOPSIS

mx-run UpdateMaterializedViewTrialDesigns [options] -H hostname -D dbname -u username [-F]
...
This patch fixes materialized view for trial designs
...
package UpdateMaterializedViewTrialDesigns;
...
sub patch {
    ...
    my $drone_run_band_on_drone_run_rel_type_id = SGN::Model::Cvterm->get_cvterm_row($schema, 'drone_run_band_on_drone_run', 'project_relationship')->cvterm_id();
...
SELECT observationunit.stock_id AS observationunit_stock_id, observationunit.uniquename AS observationunit_uniquename, observationunit_cvterm.name AS observationunit_type_name, observationunit.is_private AS observationunit_is_private, observationunit.private_company_id AS observationunit_private_company_id, germplasm.uniquename AS germplasm_uniquename, germplasm.stock_id AS germplasm_stock_id, germplasm.is_private AS germplasm_is_private, germplasm.private_company_id AS germplasm_private_company_id, rep.value AS rep, block_number.value AS block, plot_number.value AS plot_number, row_number.value AS row_number, col_number.value AS col_number, plant_number.value AS plant_number, is_a_control.value AS is_a_control, string_agg(distinct(notes.value), ', ') AS notes, project.project_id AS trial_id, project.name AS trial_name, project.description AS trial_description, project.is_private AS trial_is_private, project.private_company_id AS trial_private_company_id, plot_width.value AS plot_width, plot_length.value AS plot_length, field_size.value AS field_size, field_trial_is_planned_to_be_genotyped.value AS field_trial_is_planned_to_be_genotyped, field_trial_is_planned_to_cross.value AS field_trial_is_planned_to_cross, breeding_program.project_id AS breeding_program_id, breeding_program.name AS breeding_program_name, breeding_program.description AS breeding_program_description, breeding_program.is_private AS breeding_program_is_private, breeding_program.private_company_id AS breeding_program_private_company_id, year.value AS year, design.value AS design, location_id.value AS location_id, planting_date.value AS planting_date, harvest_date.value AS harvest_date, folder.project_id AS folder_id, folder.name AS folder_name, folder.description AS folder_description, folder.is_private AS folder_is_private, folder.private_company_id AS folder_private_company_id, seedplot_planted.value AS seedlot_transaction, seedlot.stock_id AS seedlot_stock_id, seedlot.uniquename AS seedlot_uniquename, seedlot_current_weight.value AS seedlot_current_weight_gram, seedlot_current_count.value AS seedlot_current_count, seedlot_seedlot_box.value AS seedlot_box_name,
    ...
    COALESCE(
        jsonb_agg(jsonb_build_object('trait_id', phenotype.cvalue_id, 'trait_name', (((cvterm.name::text || '|'::text) || db.name::text) || ':'::text) || dbxref.accession::text, 'value', phenotype.value, 'phenotype_id', phenotype.phenotype_id, 'outlier', outlier.value, 'create_date', phenotype.create_date, 'uniquename', phenotype.uniquename, 'phenotype_location_id', nd_geolocation.nd_geolocation_id, 'phenotype_location_name', nd_geolocation.description, 'collect_date', phenotype.collect_date, 'operator', phenotype.operator, 'associated_image_id', md_image.image_id, 'associated_image_type', project_md_image_type.name, 'associated_image_project_id', drone_image_project.project_id, 'associated_image_project_name', drone_image_project.name, 'associated_image_project_time_json', drone_image_project_time_json.value, 'additional_info', pheno_additional_info.value, 'is_private', phenotype.is_private, 'company_id', phenotype.private_company_id))
    ...
    LEFT JOIN project AS drone_image_band_project ON (project_md_image.project_id = drone_image_band_project.project_id)
    LEFT JOIN project_relationship AS drone_band_relationship ON (drone_band_relationship.subject_project_id=drone_image_band_project.project_id AND drone_band_relationship.type_id=$drone_run_band_on_drone_run_rel_type_id)
    LEFT JOIN project AS drone_image_project ON(drone_image_project.project_id = drone_band_relationship.object_project_id)
    ...
    LEFT JOIN phenotypeprop AS pheno_additional_info ON (phenotype.phenotype_id=pheno_additional_info.phenotype_id AND pheno_additional_info.type_id = $additional_info_type_id)
...
SELECT stock.stock_id AS accession_id,
    ...
    FROM stock
      ...
      JOIN nd_experiment_stock ON (stock.stock_id = nd_experiment_stock.stock_id AND nd_experiment_stock.type_id IN (SELECT cvterm_id from cvterm where cvterm.name='genotyping_experiment'))
      JOIN nd_experiment_protocol ON nd_experiment_stock.nd_experiment_id = nd_experiment_protocol.nd_experiment_id
      JOIN nd_protocol ON (nd_experiment_protocol.nd_protocol_id = nd_protocol.nd_protocol_id AND nd_protocol.type_id IN (SELECT cvterm_id from cvterm where cvterm.name='genotyping_experiment'))
      LEFT JOIN nd_experiment_project ON nd_experiment_stock.nd_experiment_id = nd_experiment_project.nd_experiment_id
    ...
    WHERE stock.type_id IN (SELECT cvterm_id from cvterm where cvterm.name = 'accession')
   GROUP BY 1,2,3,4,5
UNION
SELECT accession.stock_id AS accession_id,
    ...
    FROM stock AS accession
     ...
     LEFT JOIN nd_experiment_project ON nd_experiment_stock.nd_experiment_id = nd_experiment_project.nd_experiment_id
     JOIN nd_experiment_genotype ON nd_experiment_stock.nd_experiment_id = nd_experiment_genotype.nd_experiment_id
    WHERE accession.type_id IN (SELECT cvterm_id from cvterm where cvterm.name = 'accession')
...
WHERE cvterm.name = 'design' AND projectprop.value != 'drone_run' AND projectprop.value != 'drone_run_rover_collection' AND projectprop.value != 'drone_run_band'
...
SELECT project_relationship.object_project_id AS breeding_program_id, project.project_id AS genotyping_project_id
FROM public.project
...
WHERE projectprop.type_id = (SELECT cvterm_id FROM public.cvterm WHERE name = 'design') AND projectprop.value = 'genotype_data_project'
GROUP BY 1,2
WITH DATA;
...
DROP MATERIALIZED VIEW IF EXISTS public.genotyping_projectsXplants CASCADE;
CREATE MATERIALIZED VIEW public.genotyping_projectsXplants AS
    ...
    GROUP BY 1,2
WITH DATA;
CREATE UNIQUE INDEX genotyping_projectsXplants_idx ON public.genotyping_projectsXplants(genotyping_project_id, plant_id) WITH (fillfactor=100);
ALTER MATERIALIZED VIEW public.genotyping_projectsXplants OWNER TO web_usr;

DROP MATERIALIZED VIEW IF EXISTS public.genotyping_projectsXplots CASCADE;
CREATE MATERIALIZED VIEW public.genotyping_projectsXplots AS
    ...
    GROUP BY 1,2
WITH DATA;
CREATE UNIQUE INDEX genotyping_projectsXplots_idx ON public.genotyping_projectsXplots(genotyping_project_id, plot_id) WITH (fillfactor=100);
ALTER MATERIALIZED VIEW public.genotyping_projectsXplots OWNER TO web_usr;

DROP MATERIALIZED VIEW IF EXISTS public.genotyping_projectsXseedlots CASCADE;
CREATE MATERIALIZED VIEW public.genotyping_projectsXseedlots AS
...
GROUP BY 1,2
WITH DATA;
CREATE UNIQUE INDEX genotyping_projectsXseedlots_idx ON public.genotyping_projectsXseedlots(genotyping_project_id, seedlot_id) WITH (fillfactor=100);
ALTER MATERIALIZED VIEW public.genotyping_projectsXseedlots OWNER TO web_usr;

DROP MATERIALIZED VIEW IF EXISTS public.genotyping_projectsXtrait_components CASCADE;
CREATE MATERIALIZED VIEW public.genotyping_projectsXtrait_components AS
    ...
    GROUP BY 1,2
WITH DATA;
CREATE UNIQUE INDEX genotyping_projectsXtrait_components_idx ON public.genotyping_projectsXtrait_components(genotyping_project_id, trait_component_id) WITH (fillfactor=100);
ALTER MATERIALIZED VIEW public.genotyping_projectsXtrait_components OWNER TO web_usr;

DROP MATERIALIZED VIEW IF EXISTS public.genotyping_projectsXtrial_designs CASCADE;
CREATE MATERIALIZED VIEW public.genotyping_projectsXtrial_designs AS
    ...
    GROUP BY 1,2
WITH DATA;
CREATE UNIQUE INDEX genotyping_projectsXtrial_designs_idx ON public.genotyping_projectsXtrial_designs(genotyping_project_id, trial_design_id) WITH (fillfactor=100);
ALTER MATERIALIZED VIEW public.genotyping_projectsXtrial_designs OWNER TO web_usr;

DROP MATERIALIZED VIEW IF EXISTS public.genotyping_projectsXtrial_types CASCADE;
CREATE MATERIALIZED VIEW public.genotyping_projectsXtrial_types AS
    ...
    GROUP BY 1,2
WITH DATA;
CREATE UNIQUE INDEX genotyping_projectsXtrial_types_idx ON public.genotyping_projectsXtrial_types(genotyping_project_id, trial_type_id) WITH (fillfactor=100);
ALTER MATERIALIZED VIEW public.genotyping_projectsXtrial_types OWNER TO web_usr;
...
REFRESH MATERIALIZED VIEW public.genotyping_projectsXplants;
REFRESH MATERIALIZED VIEW public.genotyping_projectsXplots;
REFRESH MATERIALIZED VIEW public.genotyping_projectsXseedlots;
REFRESH MATERIALIZED VIEW public.genotyping_projectsXtrait_components;
REFRESH MATERIALIZED VIEW public.genotyping_projectsXtrial_designs;
REFRESH MATERIALIZED VIEW public.genotyping_projectsXtrial_types;
...
REFRESH MATERIALIZED VIEW CONCURRENTLY public.genotyping_projectsXplants;
REFRESH MATERIALIZED VIEW CONCURRENTLY public.genotyping_projectsXplots;
REFRESH MATERIALIZED VIEW CONCURRENTLY public.genotyping_projectsXseedlots;
REFRESH MATERIALIZED VIEW CONCURRENTLY public.genotyping_projectsXtrait_components;
REFRESH MATERIALIZED VIEW CONCURRENTLY public.genotyping_projectsXtrial_designs;
REFRESH MATERIALIZED VIEW CONCURRENTLY public.genotyping_projectsXtrial_types;
