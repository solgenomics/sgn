...
sub new {
    ...
    $self->set_sql(); #### This SQL should really be in the CXGN::People::Person object!
...
sub get_login_status {
    ...
    $logins{detailed} = {};
    while(my ($user_type, $username, $contact_email) = $sth->fetchrow_array()) {	
	$logins{detailed}->{$user_type}->{$username}->{contact_email} = $contact_email;
    }
...
sub verify_session {
    ...
    if($person_id) { #if they have a session 
        if($user_must_be_type) { #if there is a type that they must be to view this page
        
            if($user_must_be_type ne $user_type) {#if they are not the required type, send them away
    ...
    if(wantarray) {#if they are trying to get both pieces of info, give it to them, in array context
...
sub has_session {
	...
	my ($person_id,$user_type,$user_prefs,$expired)= $self->query_from_cookie($cookie);
...
sub query_from_cookie {
	...
	if(scalar(@result) && $sth->fetchrow_array()){
		die "Duplicate cookie_string entries found for cookie string '$cookie_string'";
	}
...
sub login_allowed {
	...
	#we haven't decided whether it's a good idea to comment this next line by default -- Evan
        and !($self->{conf_object}->get_conf('dbname')=~/sandbox/ and $self->{conf_object}->get_conf('production_server'))
    )
    {
        return 1;
    }
...
sub login_user {
    ...
    if($self->login_allowed())
    {
        ...
        if($num_rows > 1){die "Duplicate entries found for username '$username'";}
        if($disabled)
        {
	    ...
	    $login_info->{user_prefs} = $user_prefs;
            if($person_id) {
                ...
                if($sth->fetchrow_array())#very unlikely--or we need a new random string generator
                {
...
sub login_page_and_exit {
    my $self=shift;
    Apache2::RequestUtil->request()->internal_redirect($LOGIN_PAGE) unless $self->{no_redirect};
    exit 0;
}
