...
sub get_phenotype_matrix_prepare {
    my $self = shift;
    my $include_pedigree_parents = $self->include_pedigree_parents();
    my $include_timestamp = $self->include_timestamp;
    ...
    my @traits = ();
    foreach my $d (@$data) {
	push @traits, $d->{trait_name};
    }

    my @unique = sort uniq(@traits);

    my $unique_traits = \@unique;
    ...
    if ($self->search_type eq 'MaterializedViewTable'){
        $result_ref = $self->format_phenotype_data_materialized_view($data, $unique_traits, \@metadata_headers);
    }
    
    elsif ($self->search_type eq 'Native') {
	$result_ref = $self->format_phenotype_data_native($data, $unique_traits, \@metadata_headers);
    }


    print STDERR "UNIQUE TRAITS: ".Dumper($unique_traits);
    
    return ($result_ref, $unique_traits);
}


sub format_phentoype_data_materialized_view {
    my $self = shift;
    my $data = shift;
...
sub format_phenotype_data_materialized_view_old
{
    my $self = shift;
    my $dataref = shift;
    my $unique_traits = shift;
	...
	foreach my $trait (@$unique_traits) {
	    push @line, $trait_observations{$trait};
	}
	push @line, $obs_unit->{notes};
	push @info, \@line;
...
sub format_matrix_long {
    my $self = shift;
    my $dataref = shift;
    
    my $header = unshift(@$dataref);

    my @data;

    print STDERR "FORMAT MATRIX LONG DATAREF = ".Dumper($dataref);

    my $operator = "[unavailable]";
    
    foreach my $r (@$dataref) {
	my $obs_unit_name = $r->[22];
	my $accession_name = $r->[18];
	
	for(my $i = 0; $i<@$unique_traits; $i++) {
	    my $raw_value = $r->[$i + 30];  # 30 general info columns, then traits start
	    my ($value, $timestamp) = split /\,/, $raw_value;

	    push @data, [ $obs_unit_name, $accession_name, $unique_traits->[$i], $value, $operator, $timestamp ];
...
sub get_phenotype_matrix_long {
    ...
    my @data = $self->get_phenotype_matrix();
    #print STDERR "DATAREF = ".Dumper($data[0]);

    my $data_long_ref = $self->format_matrix_long(\@data);
