...
sub get_phenotype_matrix {
    ...
    my $data_str = substr(Dumper($data), 0, 6000);
    print STDERR "DATA RETRIEVED: ".$data_str;
#    print STDERR "TRAITS EXTRACTED = ". Dumper($data->[0]->[30..-1]);
#    $unique_traits = \@{$data->[0]->[30..-1]};
...
sub get_phenotype_matrix_long {
    ...
    my ($dataref, $unique_traits) = $self->get_phenotype_matrix();

    my $data_long_ref = $self->format_matrix_long($dataref, $unique_traits);
