package CXGN::Phenotypes::ParseUpload::Plugin::PhenotypeSpreadsheetNIRS;
...
sub name {
    return "phenotype spreadsheet nirs";
}
                            ...
                            if (defined($column_name)) {
                                if ($column_name ne '' && ($column_name =~ /[A-Z0-9_]+:\d{7}/ || $column_name eq 'notes')){
                                    ...
                                    if ( defined($trait_value) && defined($timestamp) ) {
                                        if ($trait_value ne '.'){
                                            $data{$observationunit_name}->{$column_name} = [$trait_value, $timestamp];
                                        }
                                    }
                                }
                                if ($column_name ne '' && $column_name ne 'notes' && $column_name !~ /[A-Z0-9_]+:\d{7}/){
                                    ...
                                    if ( defined($nir_value) && $nir_value ne '.') {
                                        $data{$observationunit_name}->{'nirs'}->{$column_name} = $nir_value;
                                    }
