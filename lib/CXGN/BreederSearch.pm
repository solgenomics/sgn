...
sub refresh_matviews {
  ...
  if ($refreshing) {
	...
	$async_refresh->run_async("perl bin/refresh_matviews.pl -H $dbhost -D $dbname -U $dbuser -P $dbpass -m $materialized_view -c");
      } else {
        ...
        $async_refresh = CXGN::Tools::Run->new();
	$async_refresh->run_async("perl bin/refresh_matviews.pl -H $dbhost -D $dbname -U $dbuser -P $dbpass -m $materialized_view");
      }
