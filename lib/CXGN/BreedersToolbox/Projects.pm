...
sub get_autogenerated_name_metadata_by_breeding_program {
    ...
    my @type_ids = ($transformant_autogenerated_name_metadata_cvterm_id,$plot_autogenerated_name_metadata_cvterm_id);

    my $program = $schema->resultset('Project::Project')->find({ project_id => $breeding_program_id});

    my $name_metadata_hashes;
    my $name_metadata_projectprop_rs = $program->projectprops({type_id => { -in => \@type_ids}});
    if ($name_metadata_projectprop_rs->count > 0){
    print STDERR "PROJECTPROP RS =".Dumper($name_metadata_projectprop_rs)."\n";
