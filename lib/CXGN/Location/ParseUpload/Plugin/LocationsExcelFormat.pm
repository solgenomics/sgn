...
sub validate {
    ...
    if (($col_max - $col_min)  < 1 || ($row_max - $row_min) < 1 ) { #must have header with at least plot_name and one trait, as well as one row of phenotypes
        $parse_result{'error'} = "Location file is missing header and/or location data";
        print STDERR $parse_result{'error'};
        return \%parse_result;
    }
