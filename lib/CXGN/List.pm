...
sub available_lists {
    ...
    my $q = "SELECT list.list_id, list.name, description, count(distinct(list_item_id)), type_id, cvterm.name, is_public, list.create_date, list.modified_date, first_item.content AS first_item
    FROM sgn_people.list
    LEFT JOIN sgn_people.list_item using(list_id)
    LEFT JOIN cvterm ON (type_id=cvterm_id)
    LEFT JOIN (
        SELECT DISTINCT ON (list_id) list_id, content
        FROM sgn_people.list_item
        ORDER BY list_id, list_item_id ASC
    ) AS first_item ON first_item.list_id = list.list_id
    WHERE owner=?
    GROUP BY list.list_id, list.name, description, type_id, cvterm.name, is_public, list.create_date, list.modified_date, first_item.content
    ORDER BY list.name";
    my $h = $dbh->prepare($q);
    $h->execute($owner);

    my @lists = ();
    while (my ($id, $name, $desc, $item_count, $type_id, $type, $public, $timestamp, $modify_timestamp, $first_item) = $h->fetchrow_array()) {
        if ($requested_type) {
            if ($type && ($type eq $requested_type)) {
            push @lists, [ $id, $name, $desc, $item_count, $type_id, $type, $public, $timestamp, $modify_timestamp, $first_item ];
            }
        }
        else {
            push @lists, [ $id, $name, $desc, $item_count, $type_id, $type, $public, $timestamp, $modify_timestamp, $first_item ];
...
sub available_public_lists {
    ...
    my $q = "SELECT list.list_id, list.name, description, count(distinct(list_item_id)), type_id, cvterm.name, sp_person.username, list.create_date, list.modified_date, first_item.content
    FROM sgn_people.list
    LEFT JOIN sgn_people.sp_person AS sp_person ON (sgn_people.list.owner=sp_person.sp_person_id)
    LEFT JOIN sgn_people.list_item using(list_id)
    LEFT JOIN cvterm ON (type_id=cvterm_id)
    LEFT JOIN (
        SELECT DISTINCT ON (list_id) list_id, content
    ...
    WHERE is_public='t'
    GROUP BY list.list_id, list.name, description, type_id, cvterm.name, sp_person.username, list.create_date, list.modified_date, first_item.content
    ORDER BY list.name";
    my $h = $dbh->prepare($q);
    $h->execute();

    my @lists = ();
    while (my ($id, $name, $desc, $item_count, $type_id, $type, $username, $timestamp, $modify_timestamp, $first_item) = $h->fetchrow_array()) {
        if ($requested_type) {
            if ($type && ($type eq $requested_type)) {
                push @lists, [ $id, $name, $desc, $item_count, $type_id, $type, $username, $timestamp, $modify_timestamp, $first_item ];
            }
        }
        else {
            push @lists, [ $id, $name, $desc, $item_count, $type_id, $type, $username, $timestamp, $modify_timestamp, $first_item ];
