...
sub available_public_lists {
    ...
    my $q = "SELECT list_id, list.name, description, count(distinct(list_item_id)), type_id, cvterm.name, sp_person.username, timestamp FROM sgn_people.list LEFT JOIN sgn_people.sp_person AS sp_person ON (sgn_people.list.owner=sp_person.sp_person_id) LEFT JOIN sgn_people.list_item using(list_id) LEFT JOIN cvterm ON (type_id=cvterm_id) WHERE is_public='t' GROUP BY list_id, list.name, description, type_id, cvterm.name, sp_person.username ORDER BY list.name";
    my $h = $dbh->prepare($q);
    $h->execute();

    my @lists = ();
    while (my ($id, $name, $desc, $item_count, $type_id, $type, $username, $timestamp) = $h->fetchrow_array()) {
        if ($requested_type) {
            if ($type && ($type eq $requested_type)) {
                push @lists, [ $id, $name, $desc, $item_count, $type_id, $type, $username, $timestamp ];
            }
        }
        else {
            push @lists, [ $id, $name, $desc, $item_count, $type_id, $type, $username, $timestamp ];
