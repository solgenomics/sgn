...
sub retrieve {
    ...
    while (my($plot_number, $design_info) = each %design){
        ...
        my $acc_female_parent = '';
        if (exists($selected_cols{'female_parent'})){
            $acc_female_parent = $pedigree_strings->{$design_info->{"accession_name"} . "_female_parent"};
        }
        my $acc_male_parent = '';
        if (exists($selected_cols{'male_parent'})){
            $acc_male_parent = $pedigree_strings->{$design_info->{"accession_name"} . "_male_parent"};
        }
        $design_info->{synonyms} = $acc_synonyms;
        $design_info->{pedigree} = $acc_pedigree;
        $design_info->{female_parent} = $acc_female_parent;
        $design_info->{male_parent} = $acc_male_parent;
