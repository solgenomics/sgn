...
sub validate_design {
    ...
    if (defined $design_type){
        if ($design_type ne 'CRD' && $design_type ne 'Alpha' && $design_type ne 'MAD' && $design_type ne 'Lattice' && $design_type ne 'Augmented' && $design_type ne 'RCBD' && $design_type ne 'RCD' && $design_type ne 'p-rep' && $design_type ne 'splitplot' && $design_type ne 'greenhouse' && $design_type ne 'Westcott' && $design_type ne 'Analysis'){
            $error .= "Design $design_type type must be either: CRD, Alpha, Augmented, Lattice, RCBD, RCD, MAD, p-rep, greenhouse, Westcott or splitplot";
            return $error;
        }
    }
