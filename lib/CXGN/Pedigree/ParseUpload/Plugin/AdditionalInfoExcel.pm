...
sub _validate_with_plugin {
    ...
    foreach my $info_field(@additional_info_header){
    ...
    for my $row (1 .. $row_max){
        ...
        for my $column (1 .. $col_max ){
            if ($worksheet->get_cell($row,$column)) {
                my $column_info = $info_columns{$column};
                $additional_info{$column_info}{$cross_name} = $worksheet->get_cell($row,$column)->value();
                my $info_type = $worksheet->get_cell(0,$column)->value();
                $parsed_result{$info_type} = $additional_info{$column_info};
            }
        }
    }
