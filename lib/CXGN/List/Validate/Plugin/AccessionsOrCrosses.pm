...
sub validate {
    ...
    my $q = "SELECT stock.uniquename, stockprop.value, stockprop.type_id FROM stock LEFT JOIN stockprop USING(stock_id) WHERE (stock.type_id=$accession_type_id OR stock.type_id=$cross_type_id) AND stock.is_obsolete = 'F';";
