...
sub create_names {
    ...
    my $number_of_names = $self->number_of_names();
    ...
    my $prefix_projectprop_rs = $program->projectprops({type_id => $autogenerated_name_prefix_cvterm->cvterm_id});
    if ($prefix_projectprop_rs->count == 1){
        $stored_prefix = $prefix_projectprop_rs->first->value();
        print STDERR "STORED PREFIX =".Dumper($stored_prefix)."\n";
    } elsif ($prefix_projectprop_rs->count > 1) {
        print STDERR "More than one found!\n";
        return;
    }

    my $prefix_metadata = $name_metadata_hash->{$stored_prefix};
    ...
    foreach my $n (1..$number_of_names) {
        push @new_autogenerated_names, $prefix. "$n + $current_serial_number";
    }
    print STDERR "NEW AUTOGENERATED NAMES =".Dumper(\@new_autogenerated_names)."\n";

    return \@autogenerated_names;
