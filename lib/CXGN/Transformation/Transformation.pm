...
sub get_transformation_info {
    ...
    my $q = "SELECT plant.stock_id, plant.uniquename, vector.stock_id, vector.uniquename
    ...
    $h->execute($plant_material_of_type_id, $accession_type_id, $vector_construct_of_type_id, $vector_construct_type_id, $transformation_stock_id);

    my @transformation_info = ();
    while (my ($plant_id, $plant_name, $vector_id, $vector_name) = $h->fetchrow_array()){
        push @transformation_info, [$plant_id, $plant_name, $vector_id, $vector_name]
    }
