...
sub pagination_tokens_response {
	my $self = shift;
	my $data_count = shift;
	my $page_size = shift;
	my $page = shift;
	...
	my %pagination = (pageSize=>$page_size, currentPageToken=>$page, nextPageToken=>$nextpage, prevPageToken=>$prevpage, totalCount=>$data_count, totalPages=>$total_pages);
	return \%pagination;
}
