...
sub search {
    ...
    foreach (@$result){
        ...
        push @data, {
            ...
            documentationURL=>$_->{'PUI'} || "/stock/$_->{stock_id}/view",
            donors=>\@donors,
            ...
            germplasmPUI=>$_->{'PUI'} || "/stock/$_->{stock_id}/view",     
...
sub store {
    ...
    my $refresh = $bs->refresh_matviews($c->config->{dbhost}, $c->config->{dbname}, $c->config->{dbuser}, $c->config->{dbpass}, 'stockprop', 'nonconcurrent', $c->config->{basepath});
