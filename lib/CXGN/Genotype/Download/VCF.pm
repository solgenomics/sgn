...
sub download {
    ...
    foreach (@$genotypes) {
        ...
        $unique_stocks{$_->{stock_id}} = $_->{selected_genotype_hash};
    }
    ...
    @all_marker_objects = sort { $a->{chrom} <=> $b->{chrom} || $a->{pos} <=> $b->{pos} } @all_marker_objects;
    ...
    open($F, ">", $filename) || die "Can't open file $filename\n";
        ...
        foreach my $m (@all_marker_objects) {
            my $format = $m->{format};
            my @row = ($m->{chrom}, $m->{pos}, $m->{name}, $m->{ref}, $m->{alt}, $m->{qual}, $m->{filter}, $m->{info}, $format);
            my @format = split ':', $format;
            foreach my $s (@sorted_stock_ids) {
                my $g = $unique_stocks{$s}->{$m};
                my @geno;
