...
has 'protocolprop_top_key_select' => (
    isa => 'ArrayRef[Str]',
    is => 'ro',
    default => sub {['reference_genome_name', 'species_name', 'header_information_lines', 'sample_observation_unit_type_name', 'assay_type', 'marker_names', 'markers', 'markers_array']} #THESE ARE ALL POSSIBLE TOP LEVEL KEYS IN PROTOCOLPROP
);

has 'protocolprop_marker_hash_select' => (
    isa => 'ArrayRef[Str]',
    is => 'ro',
    default => sub {['name', 'chrom', 'pos', 'alt', 'ref', 'qual', 'filter', 'info', 'format']} #THESE ARE ALL POSSIBLE PROTOCOLPROP MARKER HASH KEYS
);
...
sub download {
    ...
    my $forbid_cache = 1; # $self->forbid_cache;
    ...
    if ($compute_from_parents) {
        print STDERR Dumper "Computing genotype dosages HapMap From Parents......";
        return $genotypes_search->get_cached_file_HapMap_compute_from_parents(@required_config);
    }
    else {
        return $genotypes_search->get_cached_file_HapMap(@required_config);
    }
}
