...
sub _validate_with_plugin {
    ...
    my @accessions_already_present = @{$accession_validator->validate($schema,'accessions',\@accessions)->{'valid'}};

    if (scalar(@accessions_already_present) > 0) {
        push @error_messages, "This upload is for seedlots that do not have accessions in the database, and accessions will be created automatically. The following accessions are already in the database as uniquenames or synonyms: ".join(',',@accessions_already_present);
        $errors{'already_present'} = \@accessions_already_present;
    }
