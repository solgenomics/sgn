...
=head1 EXAMPLE
  ...
  my $si = CXGN::Stock::SequencingInfo->new( { bcs_schema => $schema, $stockprop_id });
...
has 'blast_link' => (isa => 'Maybe[Str]', is => 'rw');
...
has 'allowed_fields' => (isa => 'Ref', is => 'ro', default =>  sub {  [ qw | organization website genbank_accession funded_by funder_project_id contact_email sequencing_year publication jbrowse_link blast_link ftp_link ncbi_link stockprop_id stock_id sp_person_id timestamp | ] } );
...
sub BUILD {
    ...
    print STDERR "Building SequencingInfo object...\n";
...
=cut
 ...
 #     my @stockprops = $class->_get_props($schema, $stock_id, "sequencing_project_info");
