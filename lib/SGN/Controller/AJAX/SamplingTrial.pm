...
sub parse_sampling_trial_file_POST : Args(0) {
    ...
    my $sample_tissue_types = $c->controller('solGS::Utils')->get_sample_tissue_types($c);
    ...
    $parser = CXGN::Trial::ParseUpload->new(chado_schema => $chado_schema, filename => $archived_filename_with_path, sample_tissue_types => $sample_tissue_types);
