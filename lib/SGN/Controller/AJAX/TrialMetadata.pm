...
#   - type: plot order download type ('planting' or 'harvest')
...
sub get_trial_plot_order : Path('/ajax/breeders/trial_plot_order') : Args(0) {
    ...
    if ( $type ne 'planting' && $type ne 'harvest' ) {
    ...
    # Parse each trial
    # Get the min and max row and col
    my @plot_details;
        ...
        for my $i (0 .. $#outer_array) {
            ...
            if (scalar @keys > 0) {
                ...
                if ( !defined($row) || !defined($col) ) {
                    $c->stash->{rest} = { error => "One or more plots do not have a row and/or column defined!" };
                    return;
                }

                # Check for duplicate positions (plots with the same row / col positions)
                if ( exists $seen_row_cols{$key} ) {
                    $c->stash->{rest} = { error => "One or more plots share the same row and column position!" };
                    return;
                }
