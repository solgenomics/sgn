                            ...
                            if ($d->{vegetative_index_tgi_stitched_image}) {
                                ...
                                if ($d->{background_removed_tgi_stitched_image}) {
                                    ...
                                    if ($d->{background_removed_stitched_image}) {
                                        $drone_run_band_table_html .= '<div class="well well-sm"><div class="row"><div class="col-sm-3"><h5>Background Removed TGI Vegetative Index Mask on Denoised Image&nbsp;&nbsp;&nbsp;<span class="glyphicon glyphicon-remove-sign text-danger" name="drone_image_remove" data-image_id="'.$d->{background_removed_stitched_image_id}.'"></span></h5><b>By</b>: '.$d->{background_removed_stitched_image_username}.'</br><b>Date</b>: '.$d->{background_removed_stitched_image_modified_date}.'</div><div class="col-sm-3">'.$d->{background_removed_stitched_image}.'</div><div class="col-sm-6">';
                                        ...
                                        if ($d->{plot_polygon_images}) {
                                            ...
                                            $plot_polygon_images .= "</span>";
                                            $plot_polygon_images .= '<br/><br/>';
                                            $plot_polygon_images .= '<button class="btn btn-primary btn-sm" name="project_drone_imagery_get_phenotypes" data-field_trial_id="'.$v->{trial_id}.'" data-drone_run_project_id="'.$k.'" data-drone_run_band_project_id="'.$drone_run_band_project_id.'" data-drone_run_band_project_type="'.$d->{drone_run_band_project_type}.'" data-plot_polygons_type="original_background_removed_tgi_mask" >Calculate Phenotypes</button>';
                                        } else {
                                            $plot_polygon_images = 'No Plot Polygons Assigned';
                                        }
                                        $drone_run_band_table_html .= $plot_polygon_images;
...
sub raw_drone_imagery_stitch_GET : Args(0) {
    ...
    my $status = system('python '.$c->config->{rootpath}.'/DroneImageScripts/ImageStitching/PanoramaStitch.py --images_urls \''.$image_urls_string.'\' --outfile_path \''.$archive_stitched_temp_image.'\'');
...
sub drone_imagery_rotate_image_GET : Args(0) {
    ...
    my $cmd = 'python '.$c->config->{rootpath}.'/DroneImageScripts/ImageProcess/Rotate.py --image_path \''.$image_fullpath.'\' --outfile_path \''.$archive_rotate_temp_image.'\' --angle '.$angle_rotation;
    print STDERR $cmd."\n";
...
sub drone_imagery_get_contours_GET : Args(0) {
    ...
    my $status = system('python '.$c->config->{rootpath}.'/DroneImageScripts/GetContours.py --image_url \''.$main_production_site.$image_url.'\' --outfile_path \''.$archive_contours_temp_image.'\'');
...
sub drone_imagery_assign_plot_polygons_POST : Args(0) {
    ...
    foreach my $stock_name (keys %$polygon_objs) {
        ...
        my $cmd = "python ".$c->config->{rootpath}."/DroneImageScripts/CropToPolygon.py --inputfile_path '$image_fullpath' --outputfile_path '$archive_plot_polygons_temp_image' --polygon_json '$polygons'";
...
sub drone_imagery_fourier_transform_GET : Args(0) {
    ...
    my $status = system('python '.$c->config->{rootpath}.'/DroneImageScripts/ImageProcess/FourierTransform.py --image_path \''.$image_fullpath.'\' --outfile_path \''.$archive_fourier_temp_image.'\'');
...
sub drone_imagery_denoise_GET : Args(0) {
    ...
    my $status = system('python '.$c->config->{rootpath}.'/DroneImageScripts/ImageProcess/Denoise.py --image_path \''.$image_fullpath.'\' --outfile_path \''.$archive_denoise_temp_image.'\'');
...
sub drone_imagery_remove_background_display_POST : Args(0) {
    ...
    my $status = system('python '.$c->config->{rootpath}.'/DroneImageScripts/ImageProcess/RemoveBackground.py --image_path \''.$image_fullpath.'\' --outfile_path \''.$archive_remove_background_temp_image.'\' --threshold '.$threshold);
...
sub drone_imagery_crop_image_GET : Args(0) {
    ...
    my $cmd = "python ".$c->config->{rootpath}."/DroneImageScripts/CropToPolygon.py --inputfile_path '$image_fullpath' --outputfile_path '$archive_temp_image' --polygon_json '$polygons'";
    my $status = system($cmd);
...
sub drone_imagery_calculate_rgb_vegetative_index_POST : Args(0) {
    ...
    my $cmd = "python ".$c->config->{rootpath}."/DroneImageScripts/VegetativeIndex/$index_script.py --image_path '$image_fullpath' --outfile_path '$archive_temp_image'";
    my $status = system($cmd);
...
sub drone_imagery_mask_remove_background_POST : Args(0) {
    ...
    my $cmd = "python ".$c->config->{rootpath}."/DroneImageScripts/ImageProcess/MaskRemoveBackground.py --image_path '$image_fullpath' --mask_image_path '$mask_image_fullpath' --outfile_path '$archive_temp_image'";
    my $status = system($cmd);
...
sub drone_imagery_merge_bands_POST : Args(0) {
    ...
    my $cmd = "python ".$c->config->{rootpath}."/DroneImageScripts/ImageProcess/MergeChannels.py --image_path_band_1 '".$image_filesnames[0]."' --image_path_band_2 '".$image_filesnames[1]."' --image_path_band_3 '".$image_filesnames[2]."' --outfile_path '$archive_temp_image'";
    print STDERR Dumper $cmd;
    my $status = system($cmd);
...
sub drone_imagery_calculate_phenotypes_POST : Args(0) {
    ...
    my $cmd = 'python '.$c->config->{rootpath}.'/DroneImageScripts/ImageProcess/'.$calculate_phenotypes_script.' --image_paths \''.$image_paths_string.'\' '.$out_paths_string.' --results_outfile_path \''.$archive_temp_results.'\''.$calculate_phenotypes_extra_args;
    print STDERR Dumper $cmd;
    my $status = system($cmd);
        ...
        if ($phenotype_method eq 'zonal') {
            ...
            if ($drone_run_band_project_type eq 'RGB Color Image') {
                if ($image_band_selected eq '0') {
                    $drone_run_band_project_type_cvterm_id = $rgb_band_1_cvterm_id;
                }
                if ($image_band_selected eq '1') {
                    $drone_run_band_project_type_cvterm_id = $rgb_band_2_cvterm_id;
                }
                if ($image_band_selected eq '2') {
                    $drone_run_band_project_type_cvterm_id = $rgb_band_3_cvterm_id;
                }
            }

            if ($drone_run_band_project_type eq 'Merged 3 Bands') {
                if ($plot_polygons_type ne 'tgi' && $plot_polygons_type ne 'ndvi' && $plot_polygons_type ne 'vari') {
                    if ($image_band_selected eq '0') {
                        $drone_run_band_project_type_cvterm_id = $merged_3_bands_band_1_cvterm_id;
                    }
                    if ($image_band_selected eq '1') {
                        $drone_run_band_project_type_cvterm_id = $merged_3_bands_band_2_cvterm_id;
                    }
                    if ($image_band_selected eq '2') {
                        $drone_run_band_project_type_cvterm_id = $merged_3_bands_band_3_cvterm_id;
                    }
                } else {
                    if ($plot_polygons_type eq 'tgi') {
                        $drone_run_band_project_type_cvterm_id = $tgi_cvterm_id;
                    }
                    if ($plot_polygons_type eq 'vari') {
                        $drone_run_band_project_type_cvterm_id = $vari_cvterm_id;
                    }
                    if ($plot_polygons_type eq 'ndvi') {
                        $drone_run_band_project_type_cvterm_id = $ndvi_cvterm_id;
                    }
                }
            }
