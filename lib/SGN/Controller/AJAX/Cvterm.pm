...
sub autocomplete_GET :Args(0) {
    ...
    my @db_names = split(',', $c->request->param('db_names'));
    # trim and regularize whitespace
    #$term =~ s/(^\s+|\s+)$//g;
    #$term =~ s/\s+/ /g;
    my $term_name = $c->request->param("term");

    my $db_names_ph = join ( ',', ('?') x @db_names );
    ...
    my $query = "SELECT distinct cvterm.cvterm_id as cvterm_id  , cv.name as cv_name , cvterm.name as cvterm_name , db.name || ':' || dbxref.accession as accession
               ...
               WHERE db.name IN ($db_names_ph) AND (cvterm.name ilike ? OR cvtermsynonym.synonym ilike ? OR cvterm.definition ilike ?) AND cvterm.is_obsolete = 0 AND is_relationshiptype = 0
