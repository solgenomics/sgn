...
sub get_genotype_data_type_select : Path('/ajax/html/select/genotype_data_type') Args(0) {
    my $self = shift;
    my $c = shift;

    my $id = $c->req->param("id") || "genotype_data_type_select";
    my $name = $c->req->param("name") || "genotype_data_type_select";
    my $empty = $c->req->param("empty") || "";

    my @data_types = qw(SNP SSR);

    if ($empty) { unshift @data_types, [ "", "Select Data Type" ] }

    my $default = $c->req->param("default") || @data_types[0]->[0];
