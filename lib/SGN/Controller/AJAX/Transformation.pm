...
sub add_transformants_POST :Args(0){
    ...
    my $autogenerated_names = CXGN::Transformation::CreateAutogeneratedNames->new({schema => $schema, dbh => $dbh, breeding_program_id => $program_id, name_format => $name_format, number_of_names => $new_name_count });
...
sub set_autogenerated_name_format_POST :Args(0){
    ...
    if ($projectprop_rs->count == 1){
        ...
        $project->create_projectprops({$autogenerated_name_prefix_cvterm->name() => $name_format});
    }
