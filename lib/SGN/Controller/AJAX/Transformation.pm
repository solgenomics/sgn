...
sub add_transformants_POST :Args(0){
    ...
    my $autogenerated_names = CXGN::Transformation::CreateAutogeneratedNames->new({schema => $schema, dbh => $dbh, breeding_program_id => $program_id, prefix => $prefix, number_of_names => $new_name_count });
    ...
    if ($metadata_projectprop_rs->count == 1){
        ...
        $name_metadata_hash->{$prefix}->{'last_serial_number'} = $new_last_serial_number;
...
#    my $number_of_digits = $c->req->param('number_of_digits');
    ...
    if (! ( (exists($has_roles{$program_name}) && exists($has_roles{submitter})) || exists($has_roles{curator}))) {
        $c->stash->{rest} = { error => "You need to be either a curator, or a submitter associated with breeding program $program_name to add autogenerated name info." };
        return;
    }
...
#         number_of_digits => $number_of_digits,
    ...
    my $return = $add_autogenerated_name_info->add_info();
...
sub set_autogenerated_name_prefix_POST :Args(0){
    ...
    if (! ( (exists($has_roles{$program_name}) && exists($has_roles{submitter})) || exists($has_roles{curator}))) {
        $c->stash->{rest} = { error => "You need to be either a curator, or a submitter associated with breeding program $program_name to set autogenerated name prefix for new transformants." };
        return;
    }
...
sub add_autogenerated_name_metadata_POST :Args(0){
    ...
    my $last_serial_number = $c->req->param('last_serial_number');
    my $name_attributes = $c->req->param('name_attributes');
    print STDERR "PROGRAM =".Dumper($breeding_program)."\n";
    print STDERR "FORMAT NAME =".Dumper($format_name)."\n";
    print STDERR "DESCRIPTION =".Dumper($description)."\n";
    print STDERR "NAME TYPE =".Dumper($name_type)."\n";
    print STDERR "LAST NUMBER =".Dumper($last_serial_number)."\n";
    print STDERR "NAME ATTRIBUTES =".Dumper($name_attributes)."\n";
