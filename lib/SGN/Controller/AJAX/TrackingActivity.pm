...
sub activity_info_save_POST : Args(0) {
    ...
    if (($selected_type eq 'number_of_transformants') && $tracking_transformation) {
        ...
        my $autogenerated_name_prefix_cvterm_id = SGN::Model::Cvterm->get_cvterm_row($schema, 'autogenerated_name_prefix', 'project_property')->cvterm_id();
        my $material_of_type_id  =  SGN::Model::Cvterm->get_cvterm_row($schema, 'material_of', 'stock_relationship')->cvterm_id;
        if ($linked_project_id) {
            my $new_names_info;
            my $stored_prefix = $schema->resultset('Project::Projectprop')->find({ project_id => $linked_project_id, type_id => $autogenerated_name_prefix_cvterm_id});
            if ($stored_prefix) {
                my $prefix = $stored_prefix->value();
                my $autogenerated_names = CXGN::Transformation::CreateAutogeneratedNames->new({schema => $schema, dbh => $dbh, breeding_program_id => $program_id, prefix => $prefix, number_of_names => $number_of_transformants });
                $new_names_info = $autogenerated_names->create_names();
                print STDERR "NEW NAMES INFO=".Dumper($new_names_info)."\n";
                my $new_names = $new_names_info->{'new_autogenerated_names'};
                my $new_current_serial_number = $new_names_info->{'new_current_serial_number'};
                ...
                if ($metadata_projectprop_rs->count == 1){
                    ...
                    $name_metadata_hash->{$prefix}->{'current_serial_number'} = $new_current_serial_number;
