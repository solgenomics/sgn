		...
		my @marker_permission_errors;
			...
			$c->stash->{rest} = { error => "These marker names in your file are not in the selected protocol. $marker_name_error", missing_markers => \@mismatch_marker_names };
                ...
                if (scalar(@protocol_match_errors)) {
                    push @warning_groups, { title => 'Markers with mismatched pos, name, ref, or alt compared to the previously stored protocol (chromosome reassignment is not supported)', count => scalar(@protocol_match_errors), messages => \@protocol_match_errors };
                }
	    }
