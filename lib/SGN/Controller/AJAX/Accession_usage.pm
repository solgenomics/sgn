...
__PACKAGE__->config(
  ...
  sub accession_usage_trials: Path('/ajax/accession_usage_trials') :Args(0){
      ...
      my $q = "SELECT DISTINCT stock.stock_id, stock.uniquename, COUNT (DISTINCT project.project_id) AS trials, COUNT (DISTINCT stock_relationship.subject_id)
               FROM stock JOIN stock_relationship on (stock.stock_id=stock_relationship.object_id) AND stock_relationship.type_id=?
               JOIN nd_experiment_stock ON (stock_relationship.subject_id=nd_experiment_stock.stock_id) JOIN nd_experiment_project USING (nd_experiment_id)
               JOIN project USING (project_id) WHERE stock.type_id =? GROUP BY stock.stock_id ORDER BY trials DESC";

      my $h = $dbh->prepare($q);
      $h->execute($plot_of_type_id, $accession_type_id);
    ...
    sub accession_usage_female: Path('/ajax/accession_usage_female') :Args(0){
        ...
        my $q = "SELECT DISTINCT female_parent.stock_id, female_parent.uniquename, COUNT (DISTINCT cross_id.stock_id) AS cross_number
                 ...
                 JOIN stock AS cross_id ON (cross_id.stock_id=stock_relationship.object_id) GROUP BY female_parent.stock_id ORDER BY cross_number DESC";
        ...
        push @female_parents, [qq{<a href="/stock/$female_parent_id/view">$female_parent_name</a}, $cross_number];
        }
  ...
  sub accession_usage_male: Path('/ajax/accession_usage_male') :Args(0){
      ...
      my $q = "SELECT DISTINCT male_parent.stock_id, male_parent.uniquename, COUNT (DISTINCT cross_id.stock_id) AS cross_number
               ...
               JOIN stock AS cross_id ON (cross_id.stock_id=stock_relationship.object_id) GROUP BY male_parent.stock_id ORDER BY cross_number DESC";
      ...
      push @male_parents, [qq{<a href="/stock/$male_parent_id/view">$male_parent_name</a}, $cross_number];
      }
