...
sub get_autogenerated_name_metadata :Chained('ajax_breeding_program') PathPart('autogenerated_name_metadata') Args(0){
    ...
    my $projects = CXGN::BreedersToolbox::Projects->new({schema => $schema});
    my $name_types = $projects->get_autogenerated_name_metadata_by_breeding_program($program_id);
    my @all_name_types = @$name_types;

    my @autogenerated_name_metadata;

    foreach my $name_metadata (@all_name_types) {
        foreach my $format_name (keys %$name_metadata) {
            ...
            push @autogenerated_name_metadata, [$name_type, $format_name, $description, $attributes_string, $last_serial_number, $person_name, $created_date];
        }
    }
