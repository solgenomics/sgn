...
sub get_autogenerated_name_metadata :Chained('ajax_breeding_program') PathPart('autogenerated_name_metadata') Args(0){
    ...
    foreach my $prefix (keys %$name_metadata) {
        my $latest_serial_number = $name_metadata->{$prefix}->{'current_serial_number'};
        my $description = $name_metadata->{$prefix}->{'description'};
        my $added_by = $name_metadata->{$prefix}->{'added_by'};
        my $created_date = $name_metadata->{$prefix}->{'date'};

        my $person = CXGN::People::Person->new($dbh, $added_by);
        my $person_name = $person->get_first_name()." ".$person->get_last_name();

        push @autogenerated_name_metadata, [$prefix, $latest_serial_number, $description, $person_name, $created_date];
    }
