...
package SGN::Controller::AJAX::AlignViewer;
...
sub view_alignment :Path('/ajax/tools/align_viewer/view') Args(0) { 
    my $self = shift;
    my $c = shift;
	...
	"show_domains", "temp_file", "cds_temp_file",
        "family_nr", "i_value", "family_id"
	);
	...
	= map { $c->req->param($_) } @arguments;
    
    
    
    our ( $image_content, $sum_content, $seq_sum_content, $sv_content, $al_content,
	  $combined_content );
    ...
    unless ( $seq_data
	     ...
	     || ( $family_nr && $i_value ) )
    {
	$c->stash->{rest} = { error =>  "No sequence data provided!\n"};
	return;
    }
	...
	format         => $format,
        ...
        path           => $c->path_to( $c->tempfiles_subdir('align_viewer')),
        html_root_path => $c->config->{'basepath'},
        cluster_shared_tempdir => $c->config->{'cluster_shared_tempdir'},
    );
  
    my $params = $self->prepare_files($c, \%PARAM);    
...
#die $temp_file;
    ...
    close $tmp_image;
    $tmp_image =~ s/$PARAM{html_root_path}//;
...
$combined_content = get_combined_content($alignment, $cds_alignment);
...
# );
 ...
 if ($show_breakdown) {
     $seq_sum_content = get_seq_sum_content($alignment, \%PARAM);
 }
 else {
...
# #Find splice variants
 ...
 if ($show_sv) {
     ( $sv_content, $al_content ) = get_sv_al_content($alignment, \%PARAM);
 }
 else {
...
$c->stash->{rest} = { image_html => $image_content };

}

sub get_combined_content {
    my $alignment       = shift;
    my $cds_alignment   = shift;
    my $params = shift;
    ...
    if ($seq_ref) {
        my $link;
        ( $seq_file, $link ) =
	    build_seq_file( $params, $id_list_ref, $seq_ref, "All Sequences (No Gaps)");
        $file_links .= $link . "<br />";
    }
    ...
    if ($align_seq_ref) {
        my $link;
        ( $seq_align_file, $link ) =
          build_seq_file( $params, $id_list_ref, $align_seq_ref,
    ...
    if ( $ol_nr > 0 ) {
        ($file_links) .=
	    build_seq_file( $params, $id_list_ref, $ol_seq_ref, "Overlap of Sequences");
    }
    ...
    if ($cds_seq_ref) {
        my $link;
        ( $cds_file, $link ) =
          build_seq_file( $params, $id_list_ref, $cds_seq_ref,
            "Coding Sequences (No Gaps)" );
        $cds_file_links .= $link . "<br />";
    }
    if ($cds_align_seq_ref) {
        $cds_file_links .=
          build_seq_file($params, $id_list_ref, $cds_align_seq_ref, "Coding Aligned Sequences (Gapped)" )
          . "<br />";
    }
    if ( $ol_nr > 0 && $cds_ol_seq_ref ) {
        $cds_file_links .=
          build_seq_file( $params, $id_list_ref, $cds_ol_seq_ref,
    ...
    $combined_content .=
      "<tr><th>Selected Range</th><td>$params->{start_value} - $params->{end_value}</td></tr>";
	...
	<tr><td colspan=2><br /> 
	<a href='index.pl?temp_file=$tmp_seq_name&maxiters=$params->{maxiters}&type=$params->{type}&format=fasta_unaligned&title=$params->{title}'>&gt;&gt; Re-run Alignment On Selected Range ($available_seq_count)</a>
...
HTML
    ...
    #hidden parameters
    $combined_content .= qq| <input type="hidden" name="title" value="$params->{title}" /> |;
    $combined_content .= qq| <input type="hidden" name="format" value="$params->{format}" /> |;
    $combined_content .= qq| <input type="hidden" name="temp_file" value="$params->{temp_file}" /> |;
    $combined_content .= qq| <input type="hidden" name="cds_temp_file" value="$params->{cds_temp_file}" /> |;
    $combined_content .= qq| <input type="hidden" name="type" value="$params->{type}" /> |;

    #start and end value
    $combined_content .= qq| <table style="border:0px;border-spacing:0px"><tr><th>Start Value</th><th>End Value</th></tr>|;
    $combined_content .= qq| <tr><td><input type="text" name="start_value" value="$params->{start_value}" /></td> |;
    $combined_content .= qq| <td><input type="text" name="end_value" value="$params->{end_value}" /></td></tr></table> |;

    #Sequences to leave out
    $combined_content .=
      "<b>IDs of Sequences to Leave Out</b> (separated by spaces)<br>";
    $combined_content .= qq | <textarea name="hide_seqs" id="hide_seqs" rows="2" cols="50">$params->{hide_seqs}</textarea><br> |;

    my $show_breakdown_checked = "";
    $show_breakdown_checked = "CHECKED" if ($params->{show_breakdown});
    $combined_content .= qq | <b><input type="checkbox" name="show_breakdown" $show_breakdown_checked> Calculate Coverage and Overlap Details</b> (Time Intensive)<br /> |;

    my $show_sv_checked = "";
    $show_sv_checked = "CHECKED" if ($params->{show_sv});
    $combined_content .= qq | <b><input type="checkbox" name="show_sv" $show_sv_checked> Calculate Splice Variants and Alleles</b> (Time Intensive) |;
...
"<tr><th>% Similarity</th><th></th><th>Shared</th><th></th><th>InDel Limit</th></tr>";
    $combined_content .= qq| <tr><td><input type="text" size="10" name="sv_similarity" value="$params->{sv_similarity}"></td><td width=3>&nbsp;</td> |;
    $combined_content .= qq| <td><input type="text" size="10" name="sv_shared" value="$params->{sv_shared}'></td><td width="3">&nbsp;</td>|;
    $combined_content .= qq| <td><input type="text" size="10" name="sv_indel" value="$params->{sv_indel}"></td></tr></table> |;

    #submit
    $combined_content .= qq| <input type="submit" value="Change Alignment Details" /></form></td> |;
...
"<td>$first_key</td><td>$second_key</td><td>$ob{$first_key}{$second_key}</td><td>$pi{$first_key}{$second_key}</td></tr>";
        ...
        $sv_content .= hash2param( $params, { show_sv => 0 } );
...
"<td>$first_key</td><td>$second_key</td><td>$al_ob{$first_key}{$second_key}</td><td>$al_pi{$first_key}{$second_key}</td></tr>";
        ...
        $al_content .=
          "&nbsp;Reduce page load time by <a href=\"show_align.pl?";
        $al_content .= hash2param( $params, { show_sv => 0 } );
...
"<td>$head{$_} - $tail{$_}</td><td>$ng{$_}</td><td>$gap{$_}</td><td>$medium{$_}</td><td>$ov_score{$_}</td></tr>";
    ...
    $seq_sum_content .=
      "&nbsp;Reduce page load time by <a href=\"show_align.pl?";
    $seq_sum_content .= hash2param( $params, { show_breakdown => 0 } );
...
sub build_seq_file {
    ...
    close $temp_fh;
    $tempname =~ s/$params->{html_root_path}//;
    return ( $tempname,
...
sub fasta_check {
    ...
    unless ( $entry && $entry->id && $entry->seq ) {
        die "ERROR: FASTA needs IDs and Sequences [$filename]";
    }
    $count++;
    $maxlen = $entry->length;
    ...
    unless ( $entry && $entry->id && $entry->seq ) {
	die "ERROR: FASTA must have at least two valid sequences";
    }
    $count++;
    ...
    while ( $entry = $instream->next_seq() ) {
        unless ( $entry->id && $entry->seq ) {
            die "ERROR: Every entry must have ID AND sequence";
        }
        $maxlen = $entry->length if $entry->length > $maxlen;
        $total_length += $entry->length;
        $count++;
...
sub run_muscle :Path('/ajax/tools/align_viewer/align') Args(0) {
    my $self = shift;
    my $c = shift;

    my $params = $c->req->parameters();

    # adjust params and create necessary files...
    #
    $params = $self->prepare_files($c, $params);
    
    my ( $SEQ_COUNT, $MAX_LENGTH, $TOTAL_LENGTH ) = fasta_check( $params->{temp_file} );
    ...
    my @local_run_output = "";
    my $command_line     = "";
    my $old_temp_file    = $params->{temp_file};
    
    # RUN ON CLUSTER NODES 
    #
    my ( undef, $filename ) = tempfile(
	TEMPLATE => "jobXXXXXX",
	DIR      => $params->{cluster_shared_tempdir}
	);
    
    copy( $params->{temp_file}, $filename )
	|| die "Can't copy $params->{temp_file} to $filename";
    
    chdir $params->{cluster_shared_tempdir};
    
    print STDERR "Running on cluster: $command_line\n";
    
    CXGN::Tools::Run->temp_base($params->{cluster_shared_tempdir});
    ...
    my $job = CXGN::Tools::Run->new(  
		...
		#out_file => "$filename.aln",
                ...
                working_dir => $params->{cluster_shared_tempdir},
                temp_base   => $params->{cluster_shared_tempdir},
...
#    my ( undef, $job_file ) =
#	tempfile( DIR => $params->{path}, TEMPLATE => "object_XXXXXX" );
    
	
    return $job->jobid();
    
#         store( $job, $job_file ) || return "An error occurred in the serializaton of the job object\n";
...
sub check_job { 
    ...
    my $job = CXGN::Tools::Run->new( { jobid => $jobid });

    if ($job->alive()) { 
	sleep(1);
	$c->stash->{rest} = { message => "Processing job $jobid" };
	return;
    }
    else { 
	$c->stash->{rest} = { message => "Complete", jobid => "$jobid" };
	return;
    }
}

 
sub prepare_files :Path('/ajax/tools/align_viewer/prepare') Args(0) { 
    my $self = shift;
    my $c = shift;
    my $params = shift;

    if ( $params->{family_id} && !( $params->{family_nr} || $params->{i_value} ) ) {
	my $dbh = $c->dbc->dbh;
	my $sth =
	    $dbh->prepare(
		"SELECT family_nr, i_value FROM sgn.family LEFT JOIN sgn.family_build USING (family_build_id) WHERE family_id=?"
	    );
	$sth->execute($params->{family_id});
	( $params->{family_nr}, $params->{i_value} ) = $sth->fetchrow_array();
    }
    
    $params->{show_breakdown} = 1    if $params->{show_breakdown};
    $params->{show_sv}        = 1    if $params->{show_sv};
    $params->{maxiters}       = 1000 if ( $params->{maxiters} > 1000 );
    $params->{maxiters} ||= 2;
    
    $params->{show_domains} = 1 unless ( $params->{show_domains} eq '0' );
    
    #Run locally by default, cluster will run if the computation
    #is found to be intensive (later in script)
    unless ( $params->{run} =~ /^(local)|(cluster)$/ ) { $params->{run} = "local"; }
    
    #############################################################
    #Check if the input sequence is empty
    unless ( $params->{seq_data}
	     || $params->{id_data}
	     || $params->{temp_file}
	     || $params->{family_id}
	     || ( $params->{family_nr} && $params->{i_value} ) )
    {
	$c->stash->{rest} = { error =>  "No sequence data provided!\n"};
	return;
    }
    ...
    our $PATH     = $c->path_to( $c->tempfiles_subdir('align_viewer') );
    unless( -d $PATH ) {
	mkdir $PATH
    ...
    unless ( $params->{temp_file} =~ /\// ) {
	$params->{temp_file} = $PATH . "/" . $params->{temp_file};
    }
    unless ( $params->{cds_temp_file} =~ /\// ) {
	$params->{cds_temp_file} = $PATH . "/" . $params->{cds_temp_file};
	
	#If a temp file is set, we are definitely CDS
	#Fixes imagemap problem:
	$params->{type} = "cds" if ( -f $params->{cds_temp_file} );
    }
    
    if ( -f $params->{cds_temp_file} && $params->{force_gap_cds} ) {
	
	#The CDS temp file needs to be gapped
	$params->{cds_temp_file} = build_gapped_cds_file($PATH, $params->{temp_file}, $params->{cds_temp_file} );
    }
...
# print STDERR "Uploading file $args{upload_file}...\n";
    ...
    my @good_ids = ();
    if ( $params->{seq_data} =~ /\S+/ ) {
	$tmp_fh = File::Temp->new(
	    DIR    => $PATH,
	    UNLINK => 0,
	    );
	
	$params->{temp_file} = $tmp_fh->filename;

	print STDERR "Created temp file $params->{temp_file}...\n";

	print $tmp_fh "$params->{seq_data}";
	
	close $tmp_fh;
	
	# Convert the input file into fasta if it is clustal
	if ( $params->{format} eq 'clustalw' ) {
	    my $params->{temp_file_fasta} = $params->{temp_file} . ".fasta";
	    convert_clustal_fasta( $params->{temp_file}, $params->{temp_file_fasta} );
	    
	    # continue with the converted file...
	    $params->{temp_file} = $params->{temp_file_fasta};
	}
    }
    elsif ( $params->{id_data} =~ /\S+/ ) {
	unless ( -f $params->{temp_file} ) {
	...
	$params->{format} = "fasta_unaligned";
	
	open( my $temp_fh, '>', $params->{temp_file} ) or die $!;
	
	my @ids = split /\s+/, $params->{id_data};
	foreach my $id (@ids) {
	    if ( $id =~ /^gi:\d+$/i
		...
		my $database = "protein";
		$database = "nucleotide" if $params->{type} ne "pep";
		...
		eval {
		    ...
		    my $seqtype = "";
		    $seqtype = "protein" if $params->{type} eq "pep";
		    $seqtype = "cds"     if $params->{type} eq "cds";
		    $seqtype = "genomic" if $params->{type} eq "nt";
	...
	$params->{id_data} = join " ", @good_ids;
	close($temp_fh);
    }
    elsif ( $params->{family_nr} && $params->{i_value} ) {
	my $family_basedir = "/data/prod/public/family/$params->{i_value}";
	unless ( -d $family_basedir ) {
	    $c->stash->{rest} = { error => "No current family build corresponds to an i_value of $params->{i_value}" };
	    return;
	}
	unless ( -f "$family_basedir/pep_align/$params->{family_nr}.pep.aligned.fasta" ) {
	    $c->stash->{rest} = { error => "Alignment was not calculated for this family" };
	    return;
	}
	...
	$params->{format} = "fasta_aligned";
	$params->{type}   = "cds";
	
	#disable cds until kinks worked out
	if ( 0 && -f "$family_basedir/cds_align/$params->{family_nr}.cds.align.fasta" ) {
	    $params->{type} = "cds";
	    system
		"cp $family_basedir/cds_align/$params->{family_nr}.cds.align.fasta $params->{temp_file}";
	}
	else {
	    $params->{type} = "pep";
	    system
		"cp $family_basedir/pep_align/$params->{family_nr}.pep.aligned.fasta $params->{temp_file}";
	}
    }
    
    # Check that the input file is a good FASTA, throw error page otherwise
    #
    our( $SEQ_COUNT, $MAX_LENGTH, $TOTAL_LENGTH );
    eval { 
	( $SEQ_COUNT, $MAX_LENGTH, $TOTAL_LENGTH ) = fasta_check( $params->{temp_file} );
    };
    if ($@) { 
	$c->stash->{rest} = { error => $@ };
	return;
    ...
    #
    if (   !$NOCLUSTER && ( $params->{maxiters} * $SEQ_COUNT * $MAX_LENGTH > 30000 )
	   || !$params->{maxiters} )
    {
	$params->{run} = "cluster";
    }
    ...
    #
    if ( $params->{type} eq "cds" && $params->{temp_file} && !( -f $params->{cds_temp_file} ) ) {
	my $new_temp_fh = File::Temp->new(
	    DIR    => $PATH,
	    UNLINK => 0,
	    );
	my $new_temp = $new_temp_fh->filename;
	my $instream = Bio::SeqIO->new( -file => $params->{temp_file}, -format => 'fasta' );
	while ( my $in = $instream->next_seq() ) {
	    my $seq = $in->seq();
	    my $id  = $in->id();
	    my $len = length $seq;
	...
	$params->{cds_temp_file} = $params->{temp_file};
	$params->{temp_file}     = $new_temp;
    }
