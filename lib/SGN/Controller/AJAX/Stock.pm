...
sub add_stockprop :Path('/stock/prop/add') Args(3) { 
    my $self = shift;
    my $c = shift;
    my $stock_id = shift;
    my $value = shift;
	...
	$response = { error => "Login required for adding stockprops." };
	return;
    }
	...
	my $type_id = $schema->resultset("Cv::Cvterm")->find( { name=> $type } )->cvterm_id();	
	...
	$response = { 
	    message => "stock_id $stock_id and type_id $type_id added value $value", };

    }
    else { 
	$response = { error => "No stock $stock_id found." };
    }

    
}

=head2 remove_stockprop
...
sub remove_stockprop :Path('/stock/props/remove') Args(2) { 
    my $self = shift;
    my $c = shift;
    ...
    my $response = "";
    if (! ($c->user->check_roles("submitter") || $c->user->check_roles("curator"))) { 
	$response = { error => "Login required for deleting stockprops." };
	return;
    }

    my $schema = $c->dbic_schema('Bio::Chado::Schema', 'sgn_chado');

    my $stockprop_row = $schema->resultset("Stock::Stockprop")->find( { 
	    stockprop_id => $stockprop_id, })->first();
    
    $stockprop_row->delete();
...
sub get_stockprops : Path('/stock/prop/get') Args(2) { 
    my $self = shift;
    my $c = shift;
    my $stock_id = shift;
    my $type = shift;
    ...
    my $type_id = $schema->resultset("Cv::Cvterm")->find( { 'me.name'=>$type, 'cv.name'=>'local' }, { join => 'cv'})->cvterm_id();

    print STDERR "TYPE_ID: $type_id\n";
    
    my @stockprop_row = $schema->resultset("Stock::Stockprop")->search( 
	{ 
	stock_id => $stock_id, 
	type_id  => $type_id,
	});
    ...
    my @response = ();
    foreach my $spr (@stockprop_row) { 
	print STDERR ref($spr)."\n";
	push @response, { 
	    prop => $spr->value, 
	    stockprop_id=>$spr->stockprop_id 
	};
    }

    $c->stash->{rest} = { props => \@response };
}
