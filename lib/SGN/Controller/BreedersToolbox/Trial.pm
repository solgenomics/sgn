...
sub trial_info : Chained('trial_init') PathPart('') Args(0) {
    ...
    if ($design_type eq "genotyping_plate") {
        ...
        if($name_metadata_rs) {
            ...
            my @prefixes = keys %{$name_metadata_hash};
            $c->stash->{autogenerated_name_prefixes} = \@prefixes;
        }
        ...
        my $prefix = $transformation_project->get_autogenerated_name_prefix();
        my $default_plant_material = $transformation_project->get_default_plant_material();
        my $plant_material_id = $default_plant_material->[0];
        my $plant_material_name = $default_plant_material->[1];
        my $plant_material_link = qq{<a href="/stock/$plant_material_id/view">$plant_material_name</a>};

        $c->stash->{prefix} = $prefix;
        $c->stash->{default_plant_material} = $plant_material_link;
