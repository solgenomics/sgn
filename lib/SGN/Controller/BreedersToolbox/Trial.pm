...
sub trial_info : Chained('trial_init') PathPart('') Args(0) {
    ...
    if ($design_type eq "genotyping_plate") {
        ...
        my $program_id = $breeding_program_data->[0]->[0];
        my $program_name = $breeding_program_data->[0]->[1];
        my $locations = $program_object->get_all_locations_by_breeding_program();
        my @locations_by_program;
        foreach my $location_hashref (@$locations) {
        ...
        my $project_obj = CXGN::BreedersToolbox::Projects->new({schema => $schema});
        my $name_type = 'transformant';
        my $transformant_name_metadata = $project_obj->get_autogenerated_name_metadata_by_breeding_program($program_id, $name_type);
        my $name_metadata_hash = $transformant_name_metadata->[0];
        ...
        if ($plot_name_template_cvterm) {
            my $plot_name_template_rs = $schema->resultset("Project::Projectprop")->find({
                project_id => $c->stash->{trial_id},
                type_id    => $plot_name_template_cvterm->cvterm_id,
            });
            if ($plot_name_template_rs) {
                $c->stash->{plot_name_template} = $plot_name_template_rs->value;
            }
        }
