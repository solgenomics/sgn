...
sub gebvs_data : Path('/solgs/trait/gebvs/data') Args(0) {
    ...
    my $gebvs_data =
      $c->controller("solGS::Utils")->read_file_data($gebvs_file);
    my $gebvs_file_id =
      $c->controller('solGS::Files')->gebvs_file_id( $c, $type );
...
sub get_traits_selection_id : Path('/solgs/get/traits/selection/id') Args(0) {
    ...
    if ( @traits_ids > 1 ) {
        ...
        my $traits_selection_id =
          $self->create_traits_selection_id( \@traits_ids );
...
sub combine_gebvs_jobs_args {
    ...
    my @files_no =
      map { split(/\t/) } read_file( $gebvs_files, { binmode => ':utf8' } );
...
sub get_gebv_files_of_traits {
    ...
    if ($selection_pop_id) {
        $self->selection_pop_analyzed_traits( $c, $training_pop_id,
            $selection_pop_id );
        $gebv_files =
          join( "\t", @{ $c->stash->{selection_pop_analyzed_traits_files} } );
    }
    else {
        $self->training_pop_analyzed_traits($c);
        $gebv_files =
          join( "\t", @{ $c->stash->{training_pop_analyzed_traits_files} } );
        $valid_gebv_files = join( "\t",
    ...
    my $name2 =
      "gebv_files_of_valid_traits_${training_pop_id}${pred_file_suffix}";
    my $file2 =
      $c->controller('solGS::Files')->create_tempfile( $temp_dir, $name2 );
...
sub get_traits_selection_list {
    ...
    $self->traits_selection_catalogue_file($c);
    my $traits_selection_catalogue_file =
      $c->stash->{traits_selection_catalogue_file};

    my @combos = uniq(
        read_file( $traits_selection_catalogue_file, { binmode => ':utf8' } ) );
...
sub training_pop_analyzed_traits {
    ...
    my @selected_analyzed_traits = @{ $c->stash->{training_traits_ids} }
    ...
    foreach my $trait_id (@selected_analyzed_traits) {
        ...
        $c->controller('solGS::modelAccuracy')
          ->get_model_accuracy_value( $c, $training_pop_id, $trait );
