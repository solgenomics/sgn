...
sub load_list_selection_pop : Path('/solgs/load/genotypes/list/selection')
  Args(0) {
    my ( $self, $c ) = @_;
...
sub get_trial_id : Path('/solgs/get/trial/id') Args(0) {
    ...
    my $tr_rs = $c->controller('solGS::Search')->model($c)
      ->project_details_by_exact_name( \@trials_names );
...
sub get_trial_id_plots_list {
    ...
    my $pheno_file =
      $c->controller('solGS::Files')->phenotype_file_name( $c, $list_id );
...
sub get_genotypes_list {
    ...
    my $list =
      CXGN::List->new( { dbh => $c->dbc()->dbh(), list_id => $list_id } );
...
sub transform_genotypes_unqiueids {
    ...
    my $genotypes_id_hash =
      $transform->transform( $schema, $genotypes_t, $genotypes );
...
sub transform_uniqueids_genotypes {
    ...
    my $genotypes_hash =
      $transform->transform( $schema, $genotypes_t, $genotypes_ids );
...
sub get_genotypes_list_details {
    ...
    my $genotypes_ids =
      $self->transform_genotypes_unqiueids( $c, \@genotypes_names );
...
sub create_list_population_metadata {
    ...
    $metadata .= "\n"
      . 'description' . "\t"
      . 'Uploaded on: '
      . strftime "%a %b %e %H:%M %Y", localtime;
...
sub create_list_population_metadata_file {
    ...
    $c->controller('solGS::Files')
      ->population_metadata_file( $c, $tmp_dir, $list_pop_id );
...
sub predict_list_selection_pop_combined_pops_model {
    ...
    $c->controller('solGS::Download')
      ->selection_prediction_download_urls( $c, $training_pop_id,
        $selection_pop_id, $sel_pop_protocol_id );
...
sub user_selection_population_file {
    ...
    my ( $fh, $tempfile ) =
      tempfile( "selection_population_${pred_pop_id}-XXXXX", DIR => $list_dir );

    $c->controller('solGS::Files')
      ->genotype_file_name( $c, $pred_pop_id, $protocol_id );
...
sub map_plots_genotypes {
    ...
    if ( !@$plots ) {
        ...
        my $genotypes_rs = $c->controller('solGS::Search')->model($c)
          ->get_genotypes_from_plots($plots);
...
sub genotypes_list_genotype_file {
    ...
    $c->controller('solGS::AsyncJob')->submit_job_cluster( $c, $args );
...
sub genotypes_list_genotype_query_job {
    ...
    my $report_file = $c->controller('solGS::Files')
      ->create_tempfile( $temp_dir, "geno-data-query-report-args-${pop_id}" );
    ...
    my $config = $c->controller('solGS::AsyncJob')
      ->create_cluster_config( $c, $config_args );

    my $args_file = $c->controller('solGS::Files')
      ->create_tempfile( $temp_dir, "geno-data-query-job-args-file-${pop_id}" );
...
sub plots_list_phenotype_query_job {
    ...
    my $temp_data_files =
      $self->create_list_pop_data_files( $c, $data_dir, $file_id );
    ...
    my $args_file = $c->controller('solGS::Files')
      ->create_tempfile( $temp_dir, 'pheno-data-query-report-args' );
    $c->stash->{report_file} = $args_file;

    nstore $args, $args_file
      or croak
      "data query script: $! serializing data query details to $args_file ";
...
sub create_list_geno_data_query_jobs {
    ...
    if ( $list_type =~ /accessions/ ) {
        ...
        $c->stash->{list_geno_data_query_jobs} =
          $c->stash->{genotypes_list_genotype_query_job};
    }
    elsif ( $list_type =~ /trials/ ) {
        $self->get_list_trials_ids($c);
        my $trials_ids = $c->stash->{trials_ids};
...
sub get_list_training_data_query_jobs_file {
    ...
    nstore $query_jobs, $queries_args_file
      or croak
"list type training pop data query job : $! serializing selection pop data query details to $queries_args_file";
...
sub submit_list_training_data_query {
    ...
    if ( $list_type =~ /plots/ ) {
        ...
        $c->controller('solGS::AsyncJob')
          ->get_training_pop_data_query_job_args_file( $c, $trials,
            $protocol_id );
...
sub list_population_summary {
    ...
    if ( ( $user_role =~ /submitter|user/ && $user_id != $owner_id )
        || !$c->user )
    {
        my $page = "/" . $c->req->path;
        ...
        if ($file_id) {
            $c->controller('solGS::Files')
              ->population_metadata_file( $c, $tmp_dir, $file_id );
...
sub get_list_trials_ids {
    ...
    if ( $list_type =~ /trials/ ) {
        foreach my $t_name (@trials_names) {
            my $trial_id = $c->controller('solGS::Search')->model($c)
              ->project_details_by_name($t_name)->first->project_id;
...
#$c->controller('solGS::combinedTrials')->multi_pops_phenotype_data($c, $trials_ids);
    $c->controller('solGS::AsyncJob')
      ->submit_cluster_phenotype_query( $c, $trials_ids );
...
sub get_trials_list_geno_data {
    ...
    $c->controller('solGS::AsyncJob')
      ->submit_cluster_genotype_query( $c, $trials_ids, $protocol_id );
    $c->controller('solGS::combinedTrials')
      ->multi_pops_geno_files( $c, $trials_ids );
...
sub register_trials_list {
    ...
    if ($trials_ids) {
        $c->controller('solGS::combinedTrials')
          ->catalogue_combined_pops( $c, $trials_ids );
    }
