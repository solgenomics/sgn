...
sub get_dataset_trials : Path('/solgs/get/dataset/trials') Args(0) {
    ...
    $c->stash->{rest}{'genotyping_protocol_id'} =
      $c->stash->{genotyping_protocol_id};
}

sub check_predicted_dataset_selection :
  Path('/solgs/check/predicted/dataset/selection') Args(0) {
    my ( $self, $c ) = @_;
    ...
    $c->stash->{rest}{output} = $c->stash->{selection_prediction_download};
...
sub get_dataset_genotypes_list {
    ...
    my $genotypes_ids =
      $self->get_model($c)->get_genotypes_from_dataset($dataset_id);
    my $genotypes = $c->controller('solGS::List')
      ->transform_uniqueids_genotypes( $c, $genotypes_ids );
...
sub submit_dataset_training_data_query {
    ...
    if ( @{ $data->{categories}->{plots} } ) {
        ...
        $c->controller('solGS::List')
          ->get_list_training_data_query_jobs_file($c);
...
sub dataset_genotype_query_jobs {
    my ( $self, $c, $dataset_id, $protocol_id ) = @_;
...
sub dataset_population_summary {
    ...
    if ( !$c->user ) {
        ...
        my $protocol =
          $c->controller('solGS::genotypingProtocol')->create_protocol_url($c);
...
sub get_dataset_name {
    ...
    my $dataset_name =
      $c->controller('solGS::Search')->model($c)->get_dataset_name($dataset_id);
    return $dataset_name;
...
sub dataset_plots_list_phenotype_file {
    ...
    my $dataset_id = $c->stash->{dataset_id};
    my $plots_ids  = $c->controller('solGS::Search')->model($c)
      ->get_dataset_plots_list($dataset_id);
    my $file_id = $self->dataset_file_id($c);
    ...
    $c->controller('solGS::List')
      ->submit_list_phenotype_data_query( $c, $args );
