...
sub run_saved_analysis :Path('/solgs/run/saved/analysis/') Args(0) {
    ...
    my $file = '/export/prod/tmp/localhost/solgs/cache/gebv_kinship_PSH_143';
 
    $c->stash->{r_temp_file} = 'analysis-status';
    $c->controller('solGS::solGS')->create_cluster_acccesible_tmp_files($c);
    my $out_temp_file = $c->stash->{out_temp_file};
    ...
    { 
        my $r_process = CXGN::Tools::Run->run_cluster_perl({
            ...
            method => ['solGS::AnalysisProfile' => 'check_process_status'],
	    args => [$file],
	    run_opts => {
		out_file => $out_temp_file,
                working_dir =>$temp_dir,
		load_packages=>['solGS::AnalysisProfile', ],
...
sub run_analysis {
    ...
    $self->create_profiles_dir($c);
   # $c->stash->{on_completion} = 'test on completion'; 
