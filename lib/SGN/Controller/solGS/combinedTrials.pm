...
# $c->controller('solGS::solGS')->submit_cluster_training_pop_data_query($c, \@pops_ids);
        ...
        $c->controller('solGS::Search')
          ->compare_genotyping_platforms( $c, \@g_files );
...
sub combined_trials_page : Path('/solgs/populations/combined') Args() {
    ...
    if ($pops_list) {
        $cached = $c->controller('solGS::CachedResult')
          ->check_multi_trials_training_data( $c, $pops_list, $protocol_id );
    }
...
sub models_combined_trials : Path('/solgs/models/combined/trials') Args() {
    my ( $self, $c, $combo_pops_id, $tr_txt, $traits_selection_id, $gp,
        $protocol_id )
      = @_;
        ...
        if ( scalar(@traits_ids) == 1 ) {
            ...
            foreach my $trait_id (@traits_ids) {
                ...
                $c->controller('solGS::modelAccuracy')
                  ->create_model_summary( $c, $combo_pops_id, $trait_id );
...
{
    my ( $self, $c, $combo_pops_id, $trait_key, $trait_id, $gp, $protocol_id )
      = @_;
    ...
    my $cached =
      ...
      ->check_single_trial_model_output( $c, $combo_pops_id, $trait_id,
        $protocol_id );
    ...
    if ( !$cached ) {
        ...
        my $training_pop_page =
          $c->controller('solGS::Path')->training_page_url($args);
        $training_pop_page =
          qq | <a href="$training_pop_page">training population page </a>|;

        my $msg = "Cached output for this model does not exist anymore.\n"
          . " Please go to the $training_pop_page and run the analysis.";
...
# $protocol_id = $c->stash->{genotyping_protocol_id};
    ...
    if ( $selection_pop_id =~ /list/ ) {
        ...
        $c->controller('solGS::List')
          ->list_population_summary( $c, $selection_pop_id );
    ...
    my $sel_pop_mr_cnt =
      $c->controller('solGS::solGS')->get_markers_count( $c, $mr_cnt_args );
    ...
    my $training_model_page =
      $c->controller('solGS::Path')->model_page_url($args);
    my $model_link =
      qq | <a href="$training_model_page">$training_pop -- $trait_abbr </a>|;
    $c->stash->{model_page_url} = $model_link;

    $c->controller('solGS::Files')
      ->rrblup_selection_gebvs_file( $c, $model_id, $selection_pop_id,
        $trait_id );
...
sub combine_populations : Path('/solgs/combine/populations/trait') Args() {
    my ( $self, $c, $trait_id, $protocol_id ) = @_;
...
sub combine_populations_confrim :
  Path('/solgs/combine/populations/trait/confirm') Args() {
    my ( $self, $c, $trait_id, $protocol_id ) = @_;
    ...
    foreach my $pop_id (@pop_ids) {
        my $markers = $c->controller('solGS::Search')->model($c)
          ->get_project_genotyping_markers($pop_id);
        my @markers     = split( /\t/, $markers );
        my $markers_num = scalar(@markers);

        $c->controller('solGS::Search')
          ->trial_compatibility_table( $c, $markers_num );
        ...
        my $checkbox =
qq |<form> <input style="background-color: $match_code;" type="checkbox" checked="checked" name="project" value="$pop_id" /> </form> |;
...
qq | <div class=trial_code style="color: $match_code; background-color: $match_code; height: 100%; width:100%">code</div> |;
        ...
        push @selected_pops_details,
          [
            $checkbox,
            qq|<a href="$model_page" onclick="solGS.waitPage()">$pop_name</a>|,
            $pop_desc,
            $pop_location,
            $pop_year,
            $match_code
    ...
    $c->stash->{template}              = $c->controller('solGS::Files')
      ->template('/search/result/confirm/populations.mas');
...
sub get_combined_pops_list {
    ...
    my @combos =
      uniq( read_file( $combo_pops_catalogue_file, { binmode => ':utf8' } ) );
...
sub combined_pops_summary {
    ...
    my $tr_page_args = { 'genotyping_protocol_id' => $protocol_id, };
    ...
    foreach my $pop_id (@pops_ids) {
        ...
        if ($project_owner) {
            $projects_owners .=
              $projects_owners ? ', ' . $project_owner : $project_owner;
        }
    }
    ...
    my $markers_no =
      $c->controller('solGS::solGS')->get_markers_count( $c, $marker_args );
    ...
    if ($trait_id) {
        ...
        $model_link =
          $c->controller('solGS::Path')->model_page_url($model_page_args);
    }

    my $stocks_no =
      $self->count_combined_trials_lines( $c, $combo_pops_id, $trait_id );
...
sub cache_combined_pops_data {
    ...
    $c->controller('solGS::Files')
      ->genotype_file_name( $c, $combo_pops_id, $protocol_id );
...
sub build_model_combined_trials_trait {
    ...
    unless ( -s $gebv_file ) {
        ...
        $c->stash->{prerequisite_jobs} =
          $c->stash->{combine_populations_args_file};
...
sub combine_data_build_multiple_traits_models {
    ...
    foreach my $trait_id (@selected_traits) {
        $c->controller('solGS::Files')
          ->rrblup_training_gebvs_file( $c, $combo_pops_id, $trait_id );
        my $gebv_file = $c->stash->{rrblup_training_gebvs_file};
...
sub predict_selection_pop_combined_pops_model {
    ...
    if (@prediction_traits) {
        ...
        $c->controller('solGS::Files')
          ->selection_population_file( $c, $selection_pop_id );
...
sub r_combine_populations_args {
    ...
    my $cluster_files = $c->controller('solGS::AsyncJob')
      ->create_cluster_accessible_tmp_files( $c, $temp_file_template );
    ...
    my $job_config = $c->controller('solGS::AsyncJob')
      ->create_cluster_config( $c, $config_args );

    my $cmd =
      "Rscript --slave $in_file $out_file --args $input_files $output_files";
...
sub get_combine_populations_args_file {
    ...
    $c->controller('solGS::AsyncJob')
      ->training_pop_data_query_job_args( $c, $trials, $protocol_id );
    ...
    nstore $preq_jobs, $args_file
      or croak
"combine pops args file: $! serializing combine pops args  to $args_file ";
...
sub combined_pops_gs_input_files {
    ...
    my $input_files = join( "\t",
        $combined_pops_pheno_file, $combined_pops_geno_file, $model_info_file,
        $dataset_file, $selection_population_file, );

    my $input_file =
      $c->controller('solGS::Files')
      ->create_tempfile( $temp_dir,
        "input_files_${trait_abbr}_${training_pop_id}" );
...
sub count_combined_trials_lines {
    ...
    if ( $c->req->path =~ /solgs\/model\/combined\/populations\// ) {
        ...
        if ( -s $combined_pops_geno_file ) {
            ...
            $genos_cnt = $c->controller('solGS::solGS')
              ->count_predicted_lines( $c, $args );
        }
    }
    else {
        $self->get_combined_pops_list($c);
...
sub process_trials_list_details {
    ...
    if ( $pops_ids->[0] ) {
        foreach my $p_id (@$pops_ids) {
            my $pr_rs =
              $c->controller('solGS::Search')->get_project_details( $c, $p_id );
...
sub save_common_traits_acronyms {
    ...
    write_file( $traits_file, { binmode => ':utf8' }, $common_traits )
      if $traits_file;
...
# write_file($trait_file, {binmode => ':utf8'}, $trait_info);
    ...
    my $input_files =
      join( "\t", $pheno_files, $geno_files, $model_info_file, );
    ...
    my $tempfile_input =
      $c->controller('solGS::Files')
      ->create_tempfile( $temp_dir,
        "input_files_${trait_abbr}_${combo_pops_id}_combine" );
    write_file( $tempfile_input, { binmode => ':utf8' }, $input_files );

    my $tempfile_output =
      $c->controller('solGS::Files')
      ->create_tempfile( $temp_dir,
        "output_files_${trait_abbr}_${combo_pops_id}_combine" );
    ...
    $c->stash->{combine_r_temp_file} =
      "combine-pops-${combo_pops_id}_${trait_abbr}";
}
