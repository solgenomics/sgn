...
sub index_log_file_headers {
    ...
    my @headers =
      split( /\t/, ( read_file( $log_file, { binmode => ':utf8' } ) )[0] );
...
sub create_itemized_prediction_log_entries {
    ...
    foreach my $trait_id (@$trait_ids) {
        ...
        my $analysis_name =
          $analysis_log->{analysis_name} . ' -- ' . $trait_abbr;
        ...
        $entries .= join(
            "\t",
            (
                $analysis_log->{user_name}, $analysis_name,
                $analysis_page,             'Submitted',
                $args->{analysis_time},     $json->encode($args),
            )
        );
...
sub format_log_entry {
    ...
    my $traits_ids =
      $traits_args->{training_traits_ids} || $traits_args->{trait_id};
    ...
    my $entry = join(
        "\t",
        (
            $profile->{user_name}, $profile->{analysis_name},
            $analysis_page,        'Submitted',
            $time,                 $args
        )
    );
    ...
    if ( @traits_ids > 1 && $analysis_type =~ /model|selection/ ) {
        my $traits_entries =
          $self->create_itemized_prediction_log_entries( $c, $profile );
...
sub analysis_report_job_args {
    ...
    my $report_file = $c->controller('solGS::Files')
      ->create_tempfile( $temp_dir, 'analysis-report-args' );
    ...
    $status_check_duration =
      ...
      if $status_check_duration;

    my $cmd =
        'mx-run solGS::AnalysisReport'
      . ' --output_details_file '
      . $report_file
      . $status_check_duration;
...
sub email_analysis_report {
    ...
    my $job =
      $c->controller('solGS::AsyncJob')->submit_job_cluster( $c, $job_args );
...
sub structure_output_details {
    ...
    if ( $analysis_page =~ m/$match_pages/ ) {
        ...
        $output_details =
          $self->structure_training_combined_pops_data_output($c);
    }
    elsif ( $analysis_page =~
m/solgs\/selection\/(\d+|\w+_\d+)\/model\/|solgs\/combined\/model\/\d+\/selection\//
      )
    {
...
sub structure_kinship_analysis_output {
    ...
    my $coef_files = $c->controller('solGS::Kinship')
...
sub structure_training_single_pop_data_output {
    ...
    if ( $pop_id =~ /list/ ) {
        my $files =
          $c->controller('solGS::List')->create_list_pop_data_files($c);
        $pheno_file = $files->{pheno_file};
        $geno_file  = $files->{geno_file};

        $c->controller('solGS::List')
          ->create_list_population_metadata_file( $c, $pop_id );
        $c->controller('solGS::List')->list_population_summary($c);
        $pop_name = $c->stash->{project_name};
    }
    elsif ( $pop_id =~ /dataset/ ) {
        my $files = $c->controller('solGS::Dataset')
          ->create_dataset_pop_data_files( $c, );
        $pheno_file = $files->{pheno_file};
        $geno_file  = $files->{geno_file};

        $c->controller('solGS::Dataset')
          ->create_dataset_population_metadata_file($c);
...
sub structure_training_combined_pops_data_output {
    ...
    $c->controller('solGS::combinedTrials')
      ->multi_pops_pheno_files( $c, \@combined_pops_ids );
    $c->controller('solGS::combinedTrials')
      ->multi_pops_geno_files( $c, \@combined_pops_ids, $protocol_id );
    ...
    foreach my $pop_id (@combined_pops_ids) {
        ...
        $c->controller('solGS::Files')
          ->phenotype_file_name( $c, $pop_id, $protocol_id );
        my $geno_file = $c->stash->{genotype_file_name};
    ...
    foreach my $trait_id (@traits_ids) {
        ...
        if ( $data_set_type =~ /combined_populations/ ) {
            ...
            if ( $tr_pop_id =~ /list/ ) {
                ...
                $c->controller('solGS::Dataset')
                  ->dataset_population_summary($c);
                $tr_pop_name = $c->stash->{project_name};
            }
            else {
                $c->controller('solGS::Search')
                  ->get_project_details( $c, $tr_pop_id );
                $tr_pop_name = $c->stash->{project_name};
            }
        ...
        if ( $sel_pop_id =~ /list/ ) {
            ...
            $c->controller('solGS::List')
              ->list_population_summary( $c, $sel_pop_id );
            $c->controller('solGS::List')
              ->create_list_population_metadata_file( $c, $sel_pop_id );
            ...
            $c->controller('solGS::Search')
              ->get_project_details( $c, $sel_pop_id );
            $sel_pop_name = $c->stash->{project_name};
        }

        $c->controller('solGS::Files')
          ->rrblup_selection_gebvs_file( $c, $tr_pop_id, $sel_pop_id,
            $trait_id );
        my $gebv_file = $c->stash->{rrblup_selection_gebvs_file};

        $c->controller('solGS::Files')
          ->genotype_file_name( $c, $sel_pop_id, $protocol_id );
...
sub create_training_data {
    ...
    if ( $analysis_page =~ /solgs\/population\// ) {
        ...
        if ( $pop_id =~ /list/ ) {
            ...
            $c->controller('solGS::Dataset')
              ->submit_dataset_training_data_query($c);
            $c->controller('solGS::Dataset')
              ->create_dataset_population_metadata_file($c);
        }
        else {
            $c->controller('solGS::AsyncJob')
              ->submit_cluster_training_pop_data_query( $c, [$pop_id],
                $protocol_id );
        }
    }
        ...
        $c->controller('solGS::AsyncJob')
          ->submit_cluster_training_pop_data_query( $c, $trials, $protocol_id );
    }
...
sub predict_selection_traits {
    ...
    if ( $selection_pop_id =~ /list/ ) {
        ...
        $c->controller('solGS::List')
          ->create_list_population_metadata_file( $c, $selection_pop_id );
    }
...
sub get_user_solgs_analyses {
    ...
    if ($log_file) {
        ...
        foreach my $row (@user_analyses) {
            ...
            my $row = [
                $analysis_name,   $analysis_type, $submitted_on,
                $analysis_status, $result_page
            ];
            push @panel_data, $row;
        }
    }
