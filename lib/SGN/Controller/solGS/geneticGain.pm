...
sub get_selection_pop_gebvs : Path('/solgs/get/gebvs/selection/population/')
  Args(0) {
    my ( $self, $c ) = @_;
...
sub get_gebv_arrayref {
    ...
    $c->stash->{gebv_arrayref} =
      $c->controller('solGS::Utils')->read_file_data($file);
}

sub check_population_type {
    my ( $self, $c, $pop_id ) = @_;

    $c->stash->{population_type} =
      $c->controller('solGS::Search')->model($c)->get_population_type($pop_id);
}
