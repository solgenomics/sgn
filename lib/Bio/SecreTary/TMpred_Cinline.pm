...
double max_index_in_range( SV * terms, int start, int stop ) {
        ...
        long i;
//printf("start, stop: %i %i \n", start, stop);
        for (i = start+1; i <= stop; i++) {
                double thisval = SvNV(* av_fetch((AV *)SvRV(terms), i, 0));
// printf("max, thisval: %g %g \n", max, thisval);
                if(thisval > max){
                        max = thisval;
                        max_index = i;
// printf("max, max_index: %g %i \n", max, max_index); 
                }
//printf("thisval, max, max_index: %g %g %i \n", thisval, max, max_index);
        }
//printf("returned max_index: %i \n", max_index); 
       return max_index;
}
...
sub make_curve {
  ...
  for ( my $i = 0 ; $i < $length ; $i++ ) {
    ...
    #  my ( $p1, $s1 ) = findmax1( $n_profile, $n_start, $i - $min_halfw );
    #  my ( $p2, $s2 ) = findmax1( $c_profile, $i + $min_halfw, $c_end );
...
sub findmax {
  ...
  my $start = shift;
  my $stop = shift;
  my $position =  max_index_in_range($profile, $start, $stop);
  return ( $position, $profile->[$position] );
}

sub findmax1 {			# looking between start and stop,
  # return the maximum value in $profile and its position.
  my $profile = shift;		# array ref
  my $start   = shift;
  my $stop    = shift;
  # print "Tprofile: ", join(";", @$profile), "\n";
  ...
  while ( $io_count < $n_io or $oi_count < $n_oi ) {
	...
	$self->_io_oi_correspondence_string( $io_helices_ref->[$io_count],
      ...
      $io_count++;
	      ...
	      $oi_helices_ref->[$oi_count]->center()->[0] ) { # io center is first (or same pos)
      if ( $oi_helices_ref->[$oi_count]->sh_nterm()->[0] <=
	   $io_helices_ref->[$io_count]->sh_cterm()->[0] ) { # cores overlap
	...
	$io_count++;
      ...
      if ( $io_helices_ref->[$io_count]->sh_nterm()->[0] <=
	   $oi_helices_ref->[$oi_count]->sh_cterm()->[0] ) { # cores overlap
	$long_out .= $self->_io_oi_correspondence_string(
							 $io_helices_ref->[$io_count],
							 $oi_helices_ref->[$oi_count]
							);
